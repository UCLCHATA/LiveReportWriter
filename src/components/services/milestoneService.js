...
export class MilestoneService {
    constructor() {
        Object.defineProperty(this, "gsheetId", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    async saveToGSheets(milestones) {
        if (!this.gsheetId) {
    ...
    async loadFromGSheets() {
        if (!this.gsheetId) {
            return this.loadFromLocal();
        }
        try {
            const response = await fetch(`https://script.google.com/macros/s/${this.gsheetId}/exec?action=getMilestones`);
            if (!response.ok) {
