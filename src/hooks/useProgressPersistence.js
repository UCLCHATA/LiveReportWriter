...
export const useProgressPersistence = (validatedChataId) => {
    ...
    const saveProgress = (formProgress, carouselProgress) => {
        if (!validatedChataId)
            return;
        const newProgress = {
            formProgress,
        ...
        localStorage.setItem(`${PROGRESS_STORAGE_KEY}_${validatedChataId}`, JSON.stringify(newProgress));
    };
    // Clear progress
    const clearProgress = () => {
        if (!validatedChataId)
