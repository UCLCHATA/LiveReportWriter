...
function formatDomainData(domain) {
    const defaultData = {
        value: 0,
        label: 'Skipped',
        observations: []
...
function formatDomainScore(domain) {
    if (!domain || typeof domain.value === 'undefined') {
        return 'Not assessed';
    }
...
function prepareSensoryProfileData(profile) {
    ...
    domains.forEach(domain => {
        ...
        const header = SHEETY_HEADERS.SENSORY[domain];
...
function prepareSocialCommunicationData(profile) {
    ...
    domains.forEach(domain => {
        const domainData = profile.domains[domain];
        const header = SHEETY_HEADERS.SOCIAL[domain];
        formattedData[header.score] = formatDomainScore(domainData);
        formattedData[header.observations] = formatDomainData(domainData);
    });
...
function prepareBehaviorInterestsData(profile) {
    ...
    domains.forEach(domain => {
        const domainData = profile.domains[domain];
        const header = SHEETY_HEADERS.BEHAVIOR[domain];
        formattedData[header.score] = formatDomainScore(domainData);
        formattedData[header.observations] = formatDomainData(domainData);
    });
...
function formatMilestoneStatus(expectedAge, achievedAge) {
    if (!achievedAge)
        return ''; // Don't show unplaced milestones
    if (achievedAge === expectedAge)
        return `Placed at: ${achievedAge}m (On Time)`;
...
function prepareMilestoneData(milestones) {
    ...
    const placedMilestones = milestones.milestones.filter((m) => {
    ...
    placedMilestones.forEach((milestone) => {
...
function prepareAssessmentLogData(assessmentLog) {
    ...
    const formattedEntries = assessmentLog.selectedAssessments.map((assessment) => {
...
function prepareClinicianData(clinician) {
...
export function prepareFormDataForSubmission(globalState) {
