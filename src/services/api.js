...
function formatSensoryScores(profile) {
    if (!profile?.domains)
        return {};
    const scores = {};
...
function formatSocialScores(profile) {
    if (!profile?.domains)
        return {};
    const scores = {};
    for (const [key, domain] of Object.entries(profile.domains)) {
...
function formatBehaviorScores(profile) {
    if (!profile?.domains)
        return {};
    const scores = {};
    for (const [key, domain] of Object.entries(profile.domains)) {
...
function formatMilestoneData(milestones) {
    ...
    const formatMilestoneStatus = (milestone, expectedMonth) => {
        if (!milestone || !milestone.achievedAge)
            return `Expected ${expectedMonth}m (Not yet achieved)`;
        const achievedAge = milestone.achievedAge;
        const delay = achievedAge - expectedMonth;
        if (delay === 0)
            return `Expected ${expectedMonth}m (Achieved on time)`;
        if (delay > 0)
            return `Expected ${expectedMonth}m (Delayed by ${delay} months)`;
...
function formatAssessmentLogData(assessmentLog) {
    if (!assessmentLog)
        return '';
...
function chunkImageData(base64Data) {
    if (!base64Data) {
        return {
            chunk1: '{{NOT INCLUDE}}',
            chunk2: '{{NOT INCLUDE}}',
