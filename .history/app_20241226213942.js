...
// Function to handle Create New Report button
function handleCreateReport() {
    console.log('Create Report button clicked'); // Debug log
    ...
    const clinicianName = document.querySelector('#clinician-name');
    const clinicianEmail = document.querySelector('#clinician-email');
    
    if (!dialogBackdrop || !dialogContainer) {
        console.error('Dialog elements not found:', { dialogBackdrop, dialogContainer });
        return;
    }
    ...
    if (dialogSubmit) {
        dialogSubmit.onclick = () => {
            if (!clinicianName || !clinicianName.value.trim()) {
                alert('Please enter clinician\'s name');
                clinicianName?.focus();
                return;
            }
            if (!clinicianEmail || !clinicianEmail.value.trim() || !isValidEmail(clinicianEmail.value)) {
                alert('Please enter a valid email address');
                clinicianEmail?.focus();
                return;
            }
            
            // Store clinician details
            window.clinicianDetails = {
                name: clinicianName.value.trim(),
                email: clinicianEmail.value.trim()
            };
    ...
    if (dialogCancel) {
        dialogCancel.onclick = () => {
            ...
            dialogBackdrop.classList.remove('active');
            dialogContainer.classList.remove('active');
            if (clinicianName) clinicianName.value = '';
            if (clinicianEmail) clinicianEmail.value = '';
        };
    }
    
    // Handle backdrop click
    dialogBackdrop.onclick = (event) => {
...
// Function to initialize form after clinician details are entered
function initializeForm() {
    const formContent = document.querySelector('.form-content');
    const buttonGroup = document.querySelector('.button-group');
    
    if (formContent) formContent.classList.remove('muted');
    if (buttonGroup) buttonGroup.classList.remove('muted');
    
    // Load default text for all text areas
    document.querySelectorAll('.text-box-container').forEach(container => {
        const textarea = container.querySelector('.text-area');
        const preview = container.querySelector('.field-preview');
        const fieldId = container.classList[1]; // Get the type (clinical, strengths, etc.)
        const defaultText = fieldConfig[fieldId]?.defaultText || '';
        
        if (textarea) textarea.value = defaultText;
        if (preview) preview.textContent = defaultText;
    });
}
...
function clearForm() {
    ...
    const remarksInput = document.querySelector('.referral-other-input');
    if (remarksInput) remarksInput.value = '';

    // Clear storage
    localStorage.removeItem(STORAGE_KEY);

    // Reset assessment container
    const overlay = document.querySelector('.overlay');
    const assessmentContainer = document.querySelector('.assessment-container');
    if (overlay && assessmentContainer) {
...
// Function to initialize text box layout
function initializeTextBoxLayout() {
    const textBoxes = document.querySelectorAll('.text-box-container');
    textBoxes.forEach(box => {
        // Set up double-click to expand
        const preview = box.querySelector('.field-preview');
        if (preview) {
            preview.addEventListener('dblclick', () => {
                const modal = document.querySelector('.modal-container');
                const modalHeader = modal.querySelector('.modal-header h4');
                const textarea = modal.querySelector('.expandable-text');
                const fieldId = box.classList[1]; // Get the type (clinical, strengths, etc.)
                
                modalHeader.textContent = box.querySelector('h4').textContent;
...
// Function to setup form layout
function setupFormLayout() {
    const mainContainer = document.querySelector('.main-container');
    const formContainer = document.querySelector('.form-container');
    
    if (mainContainer && formContainer) {
        // Ensure form container stays on the right
        formContainer.style.flex = '1';
        formContainer.style.minWidth = '45%';
        formContainer.style.maxWidth = '50%';
        
        // Setup grid layout for text boxes
        const middleSectionsGrid = document.querySelector('.middle-sections-grid');
        if (middleSectionsGrid) {
            middleSectionsGrid.style.display = 'grid';
            middleSectionsGrid.style.gridTemplateColumns = 'repeat(2, 1fr)';
            middleSectionsGrid.style.gap = '12px';
        }
        
        // Setup referrals grid
        const referralsGrid = document.querySelector('.referrals-grid');
        if (referralsGrid) {
            referralsGrid.style.display = 'grid';
            referralsGrid.style.gridTemplateColumns = 'repeat(3, 1fr)';
            referralsGrid.style.gap = '0.75rem';
        }
    }
}
...
async function initializeApp() {
    try {
        ...
        // Setup Create New Report button
        const createReportButton = document.querySelector('.create-report-button');
        if (createReportButton) {
            console.log('Setting up Create Report button'); // Debug log
            createReportButton.removeEventListener('click', handleCreateReport); // Remove any existing listeners
