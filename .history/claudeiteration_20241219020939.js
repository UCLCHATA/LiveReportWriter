...
const MAX_TOKENS = 4096;  // Claude 3 Haiku's maximum output tokens
const LOGS_FOLDER_ID = process.env.LOGS_FOLDER_ID || '1GOFrL5mOdknr8LNgQr0FCM_OiDEvVseI';
...
function processAnalysis(chataId) {
  Logger.log(`\n=== Processing Analysis for CHATA_ID: ${chataId} ===`);
  ...
  try {
    ...
    // Process each assessment stage
    const result = await analyzeAssessments(assessmentData);
    ...
    logToSheet({
      ...
      status: result.documentUrl || 'Completed'
    });
