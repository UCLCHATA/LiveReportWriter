...
async function handleChataIdChange(event) {
    const selectedChataId = event.target.value;
    const formContent = document.querySelector('.form-content');
    const pdfContainer = document.querySelector('.pdf-container');
    const buttonGroup = document.querySelector('.button-group');
    const refreshButton = document.getElementById('refresh-data');
    const contextSections = document.getElementById('no-chata-context');
    ...
    if (selectedChataId) {
        ...
        if (selectedChataId === 'NO_CHATA') {
            ...
            try {
                ...
                await loadPdfViewers(selectedChataId);
            } catch (error) {
                console.error('Error loading data:', error);
                showError('Error loading assessment data. Please try again.');
            }
        }
        ...
        // Save mode selection
        localStorage.setItem('selected_mode', selectedChataId);
    } else {
        // Reset state when no mode is selected
...
// Update handleSubmit function to handle NO_CHATA option
async function handleSubmit() {
    try {
        ...
        if (!selectedChataId) {
            alert('Please select a CHATA ID or choose "Use w/o CHATA-ID"');
            return;
        }

        // 2. Validate ASC and ADHD status
        ...
        if (!ascStatus || !adhdStatus) {
            alert('Please select both ASC and ADHD status before proceeding');
            return;
        }

        // 3. Get the selected data including the child's name (if available)
        const selectedData = selectedChataId === 'NO_CHATA' ? 
            { id: 'NO_CHATA', name: 'Anonymous' } : 
            chataData.find(item => item.id === selectedChataId);
        ...
        const requiredFields = ['clinical', 'strengths', 'priority', 'support'];
        const emptyFields = requiredFields.filter(field => {
            const textarea = document.querySelector(`.text-box-container.${field} .text-area`);
            return !textarea?.value?.trim();
        });

        if (emptyFields.length > 0) {
            alert('Please fill in all required text fields before proceeding');
            return;
        }
        ...
        // 6. Show progress notification (this will persist until explicitly closed)
        const notification = createProgressNotification();
        
        try {
            // 7. Submit form data to Sheety
            ...
            // 8. Wait for data sync (with retries)
            ...
            // 9. Sequential Apps Script calls
            try {
                // Template stage
                ...
                if (result?.success && result.progress?.details) {
                    ...
                    // Update progress but don't close the overlay
...
function createProgressNotification() {
    ...
    notification.className = 'progress-notification active'; // Add active class immediately
    notification.innerHTML = `
        ...
        <div class="progress-content">
            ...
            <div class="progress-stages">
                <span class="stage" data-stage="submission">●</span>
    ...
    // Add close button handler (only shown after completion or error)
...
function updateProgress(notification, stage, message, details = {}) {
    ...
    const stages = ['submission', 'template', 'analysis', 'report', 'email'];
    ...
    // Show close button only after completion or error
...
function clearForm() {
    ...
    // Clear NO_CHATA context fields
    Object.values(CONTEXT_FIELDS).flat().forEach(field => {
        const element = document.querySelector(`[name="${field}"]`);
        if (element) {
            if (element.type === 'checkbox') {
                element.checked = false;
                if (field === 'has_previous_assessment') {
                    const dateInput = document.querySelector('input[name="previous_assessment_date"]');
    ...
    // Clear saved context data
    localStorage.removeItem('no_chata_context');

    // Show default placeholder
    const defaultPlaceholder = document.getElementById('default-placeholder');
    const noChataContext = document.getElementById('no-chata-context');
    const pdfViewerContainer = document.getElementById('pdf-viewer-container');
    
    defaultPlaceholder.classList.remove('hidden');
    noChataContext.classList.add('hidden');
    pdfViewerContainer.classList.add('hidden');
}
...
// Add CSS for the progress notification
const style = document.createElement('style');
style.textContent = `
...
const CONTEXT_FIELDS = {
    patient: ['age_range', 'gender', 'setting', 'assessment_date', 'has_previous_assessment', 'previous_assessment_date'],
    clinical: ['primary_concerns', 'duration_concerns', 'daily_impact', 'current_support'],
    family: ['family_composition', 'family_priorities', 'cultural_considerations', 'languages_home'],
    education: ['educational_setting', 'school_support', 'academic_profile', 'social_interactions']
};

// Add after other event listeners
document.getElementById('chata-id-select').addEventListener('change', function(e) {
    ...
    if (!selectedValue) {
        defaultPlaceholder.classList.remove('hidden');
    } else if (selectedValue === 'NO_CHATA') {
        noChataContext.classList.remove('hidden');
        setupNoChataContext();
    } else {
        pdfViewerContainer.classList.remove('hidden');
        loadPdfViewers(selectedValue);
    }
});

// Add new functions for NO_CHATA handling
function setupNoChataContext() {
    // Setup previous assessment date toggle
    const previousAssessmentCheckbox = document.querySelector('input[name="has_previous_assessment"]');
    const previousAssessmentDate = document.querySelector('input[name="previous_assessment_date"]');
    
    previousAssessmentCheckbox.addEventListener('change', function() {
        previousAssessmentDate.disabled = !this.checked;
        if (!this.checked) {
    ...
    setupContextAutoSave();
}

function loadSavedContextData() {
    const savedData = localStorage.getItem('no_chata_context');
    if (savedData) {
...
function setupContextAutoSave() {
    ...
    const saveContextData = debounce(() => {
        const data = {};
        Object.values(CONTEXT_FIELDS).flat().forEach(field => {
            const element = document.querySelector(`[name="${field}"]`);
            if (element) {
        ...
        localStorage.setItem('no_chata_context', JSON.stringify(data));
    }, 500);
    
    // Add change listeners to all context fields
    Object.values(CONTEXT_FIELDS).flat().forEach(field => {
        const element = document.querySelector(`[name="${field}"]`);
        if (element) {
            element.addEventListener('change', saveContextData);
            if (element.tagName === 'TEXTAREA') {
                element.addEventListener('input', saveContextData);
            }
        }
    });
}

// Update handleSubmit function to include context data
async function handleSubmit() {
    try {
        ...
        if (selectedChataId === 'NO_CHATA') {
            ...
            const requiredContextFields = [
                'age_range', 'gender', 'setting', 'assessment_date',
                'primary_concerns', 'duration_concerns'
            ];
            
            const missingFields = requiredContextFields.filter(field => {
                const element = document.querySelector(`[name="${field}"]`);
                return !element?.value;
            });
            
            if (missingFields.length > 0) {
                alert('Please fill in all required context fields before proceeding');
                return;
            }
            ...
            Object.values(CONTEXT_FIELDS).flat().forEach(field => {
                const element = document.querySelector(`[name="${field}"]`);
                if (element) {
                    contextData[field] = element.type === 'checkbox' ? element.checked : element.value;
                }
            });
            
            // Add context data to form data
            formData.r3Form = {
                ...formData.r3Form,
                context: contextData
            };
...
function initializeCollapsibleSections() {
    document.querySelectorAll('.context-section-header').forEach(header => {
        header.addEventListener('click', () => {
            ...
            // Save state
            const expandedSections = JSON.parse(localStorage.getItem('expanded_sections') || '[]');
    ...
    expandedSections.forEach(sectionId => {
        const section = document.querySelector(`.context-section[data-section="${sectionId}"]`);
        if (section) section.classList.add('expanded');
    });
}

// Update initializeApp to include collapsible sections
...
// Initialize text boxes with configuration
document.addEventListener('DOMContentLoaded', () => {
    const containers = document.querySelectorAll('.text-box-container');
    containers.forEach(container => {
        const type = container.classList.contains('clinical') ? 'clinical' :
                    container.classList.contains('strengths') ? 'strengths' :
                    container.classList.contains('priority') ? 'priority' :
                    container.classList.contains('support') ? 'support' : null;
        
        if (type) {
            new TextBox(container, {
                defaultText: container.dataset.defaultText || '',
                ...TEXT_FIELD_CONFIG[type]
            });
        }
    });
});

// Update initializeApp to include all new features
const originalInitializeApp = initializeApp;
initializeApp = async function() {
    await originalInitializeApp();
    ...
    const previousMode = localStorage.getItem('selected_mode');
    if (previousMode) {
        const chataSelect = document.getElementById('chata-id-select');
        if (chataSelect) {
            chataSelect.value = previousMode;
            await handleChataIdChange({ target: chataSelect });
        }
    }
    
    // Setup autosave for context fields
...
const PROGRESS_STEPS = {
    MODE_SELECTION: 0,
    CONTEXT_INFO: 1,
    CLINICAL_OBSERVATIONS: 2,
    STRENGTHS: 3,
    PRIORITY_AREAS: 4,
    SUPPORT_RECOMMENDATIONS: 5,
    REFERRALS: 6
};
...
class ProgressTracker {
    constructor() {
        ...
        this.currentStep = PROGRESS_STEPS.MODE_SELECTION;
        this.updateProgress();
    }
    ...
    createProgressElements() {
        ...
        Object.keys(PROGRESS_STEPS).forEach(() => {
        ...
        this.stepIndicators = progressSteps.children;
    }
    
    updateProgress() {
        const totalSteps = Object.keys(PROGRESS_STEPS).length;
        ...
        Array.from(this.stepIndicators).forEach((indicator, index) => {
    ...
    checkProgress() {
        const selectedMode = document.getElementById('chata-id-select')?.value;
        if (!selectedMode) {
            this.setStep(PROGRESS_STEPS.MODE_SELECTION);
            return;
        }
        
        if (selectedMode === 'NO_CHATA') {
            ...
            const hasAllContextInfo = requiredContextFields.every(field => {
                const element = document.querySelector(`[name="${field}"]`);
                return element?.value;
            });
            
            if (!hasAllContextInfo) {
                this.setStep(PROGRESS_STEPS.CONTEXT_INFO);
                return;
            }
        }
        
        // Check text areas
        const clinical = document.querySelector('.text-box-container.clinical .text-area')?.value;
        if (!clinical?.trim()) {
            this.setStep(PROGRESS_STEPS.CLINICAL_OBSERVATIONS);
            return;
        }
        
        const strengths = document.querySelector('.text-box-container.strengths .text-area')?.value;
        if (!strengths?.trim()) {
            this.setStep(PROGRESS_STEPS.STRENGTHS);
            return;
        }
        
        const priority = document.querySelector('.text-box-container.priority .text-area')?.value;
        if (!priority?.trim()) {
            this.setStep(PROGRESS_STEPS.PRIORITY_AREAS);
            return;
        }
        
        const support = document.querySelector('.text-box-container.support .text-area')?.value;
        if (!support?.trim()) {
            this.setStep(PROGRESS_STEPS.SUPPORT_RECOMMENDATIONS);
            return;
        }
        
        // Check referrals
        const hasReferrals = document.querySelectorAll('.referrals-grid input[type="checkbox"]:checked').length > 0;
        if (!hasReferrals) {
            this.setStep(PROGRESS_STEPS.REFERRALS);
            return;
        }
...
initializeApp = async function() {
    ...
    document.querySelectorAll('input, textarea, select').forEach(element => {
        element.addEventListener('change', () => progressTracker.checkProgress());
        element.addEventListener('input', debounce(() => progressTracker.checkProgress(), 500));
    });
...
const ASSESSMENT_SECTIONS = {
    ados: {
        social_speech: { min: 0, max: 3 },
        eye_contact: { min: 0, max: 3 },
        gesture_use: { min: 0, max: 3 },
        repetitive_play: { min: 0, max: 3 },
        repetitive_movements: { min: 0, max: 3 },
        sensory_exploration: { min: 0, max: 3 }
    },
    chata: {
        joint_attention: { min: 0, max: 10 },
        social_reciprocity: { min: 0, max: 10 },
        nonverbal_communication: { min: 0, max: 10 },
        stereotyped_behaviors: { min: 0, max: 10 },
        insistence_sameness: { min: 0, max: 10 },
        restricted_interests: { min: 0, max: 10 },
        sensory_hyper: { min: 0, max: 10 },
        sensory_hypo: { min: 0, max: 10 },
        sensory_interests: { min: 0, max: 10 }
    }
};

// Function to initialize assessment sections
function initializeAssessmentSections() {
...
function initializeScoreInputs() {
    ...
    document.querySelectorAll('.score-slider').forEach(slider => {
        ...
        slider.addEventListener('input', (e) => {
            ...
            saveAssessmentData(e.target.name, value);
            updateProgress();
        });
        
        // Set initial value
        const savedValue = getAssessmentData(slider.name);
        if (savedValue) {
            slider.value = savedValue;
            valueDisplay.textContent = `${savedValue}/10`;
        }
    });
}

// Function to save assessment data
function saveAssessmentData(field, value) {
...
function getAssessmentData(field) {
    const data = JSON.parse(localStorage.getItem('assessment_data') || '{}');
    return data[field];
}
...
function calculateSectionCompletion(section) {
    const fields = ASSESSMENT_SECTIONS[section];
    let completed = 0;
    let total = 0;
    
    Object.keys(fields).forEach(field => {
        total++;
        if (getAssessmentData(field)) completed++;
    });
...
function updateProgress() {
    ...
    if (progressBar) {
        progressBar.style.width = `${(totalCompleted / totalFields) * 100}%`;
    }
    
    // Update progress steps
    const steps = document.querySelectorAll('.progress-step');
    const currentStep = Math.floor((totalCompleted / totalFields) * steps.length);
...
// Function to create CHATA ID
async function createChataId() {
    const overlay = document.createElement('div');
    overlay.className = 'progress-overlay';
    overlay.innerHTML = `
        <div class="progress-content">
            <h3>Creating CHATA ID</h3>
            <div class="progress-spinner"></div>
            <p class="progress-message">Initializing assessment...</p>
        </div>
    `;
    document.body.appendChild(overlay);
    
    try {
        // Generate unique CHATA ID
        const timestamp = new Date().getTime();
        const random = Math.floor(Math.random() * 1000);
        const chataId = `CHATA_${timestamp}_${random}`;
        
        // Save CHATA ID
        localStorage.setItem('current_chata_id', chataId);
        
        // Update progress message
        const progressMessage = overlay.querySelector('.progress-message');
        progressMessage.textContent = 'CHATA ID created successfully!';
        ...
        setTimeout(() => {
            ...
            showNotification('CHATA ID created successfully', 'success');
            ...
            const firstSection = document.querySelector('.context-section');
        ...
        console.error('Error creating CHATA ID:', error);
        overlay.remove();
        showNotification('Error creating CHATA ID', 'error');
        throw error;
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add create CHATA ID button handler
    const createButton = document.getElementById('create-chata-id');
    if (createButton) {
    ...
    // Check for existing CHATA ID
    const existingChataId = localStorage.getItem('current_chata_id');
    if (existingChataId) {
        // Enable form sections
        document.querySelectorAll('.context-section').forEach(section => {
            section.classList.remove('disabled');
        ...
        // Disable form sections initially
        document.querySelectorAll('.context-section').forEach(section => {
