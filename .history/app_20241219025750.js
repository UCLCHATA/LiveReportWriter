...
async function handleChataIdChange(event) {
    ...
    const selectedData = selectedId === 'NO_CHATA' ? { id: 'NO_CHATA' } : chataData.find(item => item.id === selectedId);
    ...
    if (selectedData) {
        ...
        if (selectedId === 'NO_CHATA') {
            // Keep PDF container muted for NO_CHATA option
            pdfContainer?.classList.add('muted');
            // Clear and disable PDF viewers
            resetPdfViewers();
            // Hide refresh button for NO_CHATA option
            refreshButton?.style.display = 'none';
        } else {
            // Regular CHATA ID selected
            pdfContainer?.classList.remove('muted');
            refreshButton?.style.display = '';
            
            // Load PDFs and check for existing submission
