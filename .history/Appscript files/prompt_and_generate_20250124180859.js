    ...
    // Create JSON object from row data and handle special values
    const formData = {};
    headers.forEach((header, index) => {
        const value = row[index];
        // Check for special values that should be marked as skipped
        if (typeof value === 'string' && (
            value.includes('Typical, 0 out of 5 on the 5 point scale') ||
            value.includes('Age Appropriate, 0 out of 5 on the 5 point scale') ||
            value.includes('Not Present, 0 out of 5 on the 5 point scale')
        )) {
            formData[header] = 'skipped';
            Logger.log(`Marked ${header} as skipped due to special value: ${value}`);
        } else {
