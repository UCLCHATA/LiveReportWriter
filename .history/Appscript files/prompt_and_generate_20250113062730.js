...
function testPromptGeneration() {
  ...
  try {
    ...
    Logger.log(`[State] Testing with CHATA_ID: ${testChataId}`);
    
    // Get sample form data with validation
    ...
    // Get placeholder mapping with validation
    ...
    Logger.log('[Action] Constructing test prompt...');
    ...
    logs.push({
      ...
      state: {
        ...
        systemPromptPreview: prompt?.systemPrompt?.substring(0, 100) + '...',
        userPromptPreview: prompt?.userPrompt?.substring(0, 100) + '...'
      }
    });
    ...
    Logger.log(`[Validation] System Prompt Length: ${prompt.systemPrompt.length}`);
    Logger.log(`[Validation] User Prompt Length: ${prompt.userPrompt.length}`);
    Logger.log('[Validation] System Prompt contains key elements:', {
      hasInstructions: prompt.systemPrompt.includes('CRITICAL REQUIREMENTS'),
      hasFormatting: prompt.systemPrompt.includes('Format your response'),
      hasRestrictions: prompt.systemPrompt.includes('DO NOT')
    });
    
    // Save prompts to logs with detailed metadata
    const systemPromptDoc = saveDetailedLog('Test_SystemPrompt', prompt.systemPrompt, testChataId);
    const userPromptDoc = saveDetailedLog('Test_UserPrompt', prompt.userPrompt, testChataId);
    ...
    logs.push({
      ...
      state: {
        success: true,
        systemPromptSaved: !!systemPromptDoc,
        userPromptSaved: !!userPromptDoc,
        timestamp: new Date().toISOString()
      }
    });
    ...
    Logger.log('[ERROR] Prompt generation test failed:', {
      message: error.message,
