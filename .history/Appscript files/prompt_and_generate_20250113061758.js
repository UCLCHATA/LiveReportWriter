...
// Add folder management and improved logging
...
function logToSheetWithDetails(sheet, operation, section, content, details, chataId) {
  const timestamp = Utilities.formatDate(new Date(), "Asia/Kolkata", "yyyy-MM-dd HH:mm:ss");
  
  // Save large content to separate document if needed
  let contentToLog = content;
  let statusDetails = details;
  let documentUrl = '';
  
  // Save API prompts and responses to separate docs
  if (operation.includes('Prompt') || operation.includes('Response')) {
    try {
      ...
      if (folder) {
        ...
        // Update content and details for logging
...
function formatLogMessage(message, data = null) {
...
// Update the existing initializeLogsSheet function to use enhanced formatting
...
async function generateReport(chataId) {
  ...
  try {
    ...
    logToSheetWithDetails(logsSheet, 'Template Creation', 'All', 'Created template document', template.docUrl, chataId);

    // 2. Get form data
    const formData = getFormData(chataId);
    Logger.log('Retrieved form data');
    logToSheetWithDetails(logsSheet, 'Data Retrieval', 'Form Data', 'Retrieved assessment data', 'Success', chataId);
    ...
    logToSheetWithDetails(logsSheet, 'Data Retrieval', 'Placeholders', `Loaded ${Object.keys(placeholderMap).length} placeholders`, 'Success', chataId);

    // 4. Construct prompt
    const prompt = constructPrompt(formData, placeholderMap);
    Logger.log('Constructed LLM prompt');
    logToSheetWithDetails(logsSheet, 'Prompt Construction', 'All', 'Created LLM prompt', JSON.stringify(prompt), chataId);

    // 5. Call LLM API
    Logger.log('Calling Claude API...');
    logToSheetWithDetails(logsSheet, 'API Call', 'All', 'Initiating API call', 'In Progress', chataId);
    const llmResponse = await callClaudeAPI(
      [{ role: 'user', content: prompt.userPrompt }],
      prompt.systemPrompt,
      0.7
    );
    logToSheetWithDetails(logsSheet, 'API Call', 'All', 'API call completed', 'Success', chataId);
    ...
    if (!processedResponse.success) {
      ...
      logToSheetWithDetails(logsSheet, 'Response Processing', 'All', 'Processing failed', error, chataId);
      throw new Error(error);
    }
    logToSheetWithDetails(logsSheet, 'Response Processing', 'All', 'Processed LLM response', 'Success', chataId);

    // 7. Log final response
    const responseUrl = logToSheetWithDetails(
      logsSheet,
      'Final Response',
      'All',
      'Generated content',
    ...
    logToSheetWithDetails(logsSheet, 'Error', 'Process', error.message, error.stack, chataId);
    return {
      success: false,
      error: error.message,
      progress: {
...
function doGet(e) {
  ...
  try {
    ...
    if (e?.parameter?.test === 'true') {
      const ss = getSpreadsheet();
      const logsSheet = initializeLogsSheet(ss);
      logToSheetWithDetails(logsSheet, 'Test', 'All', 'Test request received', 'Success', chataId);
...
function testPromptGeneration() {
  ...
  try {
    ...
    logToSheetWithDetails(logsSheet, 'Test Prompt', 'System', prompt.systemPrompt, 'Generated for testing', testChataId);
    logToSheetWithDetails(logsSheet, 'Test Prompt', 'User', prompt.userPrompt, 'Generated for testing', testChataId);
...
async function testAPIConnection() {
  ...
  try {
    ...
    logToSheetWithDetails(logsSheet, 'API Test', 'Connection', 'Initiating API test', 'In Progress', testChataId);
    ...
    logToSheetWithDetails(logsSheet, 'API Test', 'Connection', 'API test completed', 'Success', testChataId);
    ...
    Logger.log('Stack trace:', error.stack);
    logToSheetWithDetails(logsSheet, 'API Test', 'Connection', 'API test failed', error.message, testChataId);
    return {
      success: false,
      error: error.message,
      details: { stack: error.stack }
...
async function testContentGeneration() {
  ...
  try {
    ...
    logToSheetWithDetails(logsSheet, 'Content Test', 'Generation', 'Starting content generation test', 'In Progress', testChataId);
...
##END##`
    ...
    logToSheetWithDetails(logsSheet, 'Content Test', 'Generation', 'Content generation completed', 'Success', testChataId);
    ...
    Logger.log('Stack trace:', error.stack);
    logToSheetWithDetails(logsSheet, 'Content Test', 'Generation', 'Content generation failed', error.message, testChataId);
    return {
      success: false,
      error: error.message,
      details: { stack: error.stack }
