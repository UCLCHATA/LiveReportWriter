...
// Test function for document conversion and email
function testDocumentProcessing() {
    try {
        ...
        // 2. Convert to Word
        const wordResult = convertToWord(doc.getId());
        if (!wordResult.success) {
            throw new Error(`Word conversion failed: ${wordResult.error}`);
        }
        Logger.log('Converted to Word:', wordResult.wordFileUrl);
        
        // 3. Set permissions
        const testEmail = Session.getEffectiveUser().getEmail();
        const gdocPermissions = setFilePermissions(doc.getId(), testEmail);
        const wordPermissions = setFilePermissions(wordResult.wordFileId, testEmail);
        
        if (!gdocPermissions.success || !wordPermissions.success) {
...
Word Document (Download): ${wordPermissions.viewUrl}

This is a test email.`;
        
        MailApp.sendEmail({
            to: testEmail,
            subject: 'Test Report Documents',
            body: emailBody
        });
        ...
        return {
            ...
            wordUrl: wordPermissions.viewUrl,
            testEmail: testEmail
        };
