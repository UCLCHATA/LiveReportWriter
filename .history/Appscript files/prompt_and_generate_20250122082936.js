...
function populateTemplate(templateDoc, responses) {
    ...
    responses.forEach(response => {
        ...
        Logger.log(`Original content length: ${content.length} chars`);
        
        // Enhanced content cleanup
        content = content
            // Remove JSON/programming artifacts
            ...
            .replace(/_(.*?)_/g, '$1')               // Remove underscore formatting
            .replace(/`(.*?)`/g, '$1')               // Remove code formatting
            .replace(/\[(.*?)\]/g, '$1')             // Remove square brackets
            
            // Clean up quotes and observations
            .replace(/["']([^"']+)["']/g, '$1')      // Remove quotes around phrases
            .replace(/"([^"]+)"/g, '$1')             // Remove any remaining double quotes
        ...
        Logger.log(`Cleaned content length: ${content.length} chars`);
        ...
        if (range) {
            try {
                ...
                // Insert new content with consistent formatting
...
function setupAllTriggers() {
    ...
    // Create change trigger for new rows
    ...
    Logger.log(`Current triggers: ${currentTriggers.length}`);
    currentTriggers.forEach(trigger => {
        Logger.log(`Trigger: ${trigger.getHandlerFunction()} - ${trigger.getEventType()}`);
    });
}

// Function to handle new form submissions and edits
function onFormSubmit(e) {
    ...
    try {
        ...
        // Handle both form submit and edit events
        let range;
        if (e && e.range) {
            // Edit event
            range = e.range;
            ...
            Logger.log('Processing latest row');
            range = sheet.getRange(sheet.getLastRow(), 1, 1, sheet.getLastColumn());
        }
        ...
        if (result.success) {
            ...
            sendEmailNotification(formData, result.templateUrl);
            Logger.log(`Report generated successfully for CHATA_ID: ${chataId}`);
        } else {
            Logger.log(`Failed to generate report for CHATA_ID: ${chataId}. Error: ${result.error}`);
            sendErrorNotification(formData, result.error);
        }
