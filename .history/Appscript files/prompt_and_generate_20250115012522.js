    ...
    // Add these functions before the replaceContentInBody function
    function reconstructImageFromChunks(formData, prefix) {
        // Find all chunks for this image type
        const chunkKeys = Object.keys(formData)
            .filter(key => key.startsWith(`${prefix}_Chunk_`))
            .sort((a, b) => {
        ...
        if (chunkKeys.length === 0) {
            Logger.log(`No chunks found for ${prefix}`);
            return null;
        }
        ...
        if (!shouldInclude) {
            Logger.log(`${prefix} marked as Not-Include`);
            return null;
        }
        ...
        chunkKeys.forEach(key => {
            ...
            const chunkData = chunk.split('|').slice(3).join('|'); // Get everything after the metadata
    ...
    function insertImagesIntoTemplate(doc, formData) {
        Logger.log('Processing images for template insertion...');
        
        // Process both image types
        const imageTypes = ['Milestone_Image', 'CombinedGraph_Image'];
        
        imageTypes.forEach(imageType => {
            const base64Data = reconstructImageFromChunks(formData, imageType);
            ...
            if (base64Data) {
                try {
                    ...
                    doc.appendParagraph(`${imageType}:`); // Add label
                    doc.appendImage(imageBlob);
                    doc.appendParagraph(''); // Add spacing
    ...
    // Replace the existing populateTemplate function
    function populateTemplate(templateDoc, responses) {
        ...
        // Add images at the end of the document
        if (responses.formData) {
            insertImagesIntoTemplate(doc, responses.formData);
        }
    ...
    async function testSinglePlaceholder() {
        ...
        try {
            ...
            Logger.log('Populating template with generated content...');
            const doc = DocumentApp.openById(template.docId);
            const body = doc.getBody();
            
            const content = processedResponse.content[0].content;
            const replacementCount = replaceContentInBody(body, placeholderData.id, content);
            
            if (replacementCount === 0) {
                throw new Error(`Failed to replace placeholder {{${placeholderData.id}}} in template`);
            }
            
            doc.saveAndClose();

            // Save the actual document content for verification
            const verificationDoc = saveDetailedLog(
                'Test_Document_Content',
                `Original placeholder: {{${placeholderData.id}}}\n\nReplaced with content:\n${content}`,
                testChataId
            );

            // 11. Log success with actual content
            const replacementLog = {
                placeholder: placeholderData.id,
                originalPlaceholder: placeholderData.id,
    ...
    // Add replaceContentInBody function from step3.js
    ...
    // Add after the CONFIG section and before the first function
    const IMAGE_PROCESSING = {
        reconstructFromChunks: function(formData, prefix) {
            return function(formData, prefix) {
                // Find all chunks for this image type
                const chunkKeys = Object.keys(formData)
                    .filter(key => key.startsWith(`${prefix}_Chunk_`))
                    .sort((a, b) => {
        ...
        insertIntoTemplate: function(doc, formData) {
            return function(doc, formData) {
                Logger.log('Processing images for template insertion...');
                ...
                imageTypes.forEach(imageType => {
                    const base64Data = IMAGE_PROCESSING.reconstructFromChunks(formData, imageType);
    ...
    // Update the populateTemplate function to use the new image processing
    function populateTemplate(templateDoc, responses) {
        Logger.log('Populating template with responses...');
        const doc = DocumentApp.openById(templateDoc);
        const body = doc.getBody();
        ...
        // Add images at the end of the document
        if (responses.formData) {
            IMAGE_PROCESSING.insertIntoTemplate(doc, responses.formData);
        }
