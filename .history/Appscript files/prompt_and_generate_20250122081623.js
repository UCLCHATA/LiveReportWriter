...
function populateTemplate(templateDoc, responses) {
    ...
    responses.forEach(response => {
        ...
        let cleanContent = content
            ...
            .replace(/_([^_]+)_/g, '$1')      // Remove underscore italics
        ...
        if (range) {
            try {
                ...
                // Insert new content
                const insertedText = textElement.insertText(startOffset, cleanContent);
    ...
    if (failed > 0) {
        Logger.log('Failed replacements:');
        replacementLog.filter(r => !r.success).forEach(r => {
            Logger.log(`- ${r.id}: ${r.error}`);
        });
    }
