    ...
    // Create JSON object from row data with preprocessing
    const formData = {};
    headers.forEach((header, index) => {
        let value = row[index];
        
        // Check for special values that indicate typical/age appropriate behavior
        if (typeof value === 'string') {
            const normalizedValue = value.trim().toLowerCase();
            if (
                normalizedValue === 'typical, 0 out of 5 on the 5 point scale' ||
                normalizedValue === 'age appropriate, 0 out of 5 on the 5 point scale' ||
                normalizedValue === 'not present, 0 out of 5 on the 5 point scale'
            ) {
                value = 'skipped';
                Logger.log(`Preprocessed ${header}: Original value "${row[index]}" replaced with "skipped"`);
            }
        }
