...
async function generateReport(chataId) {
  ...
  try {
    ...
    const promptDoc = saveDetailedLog('Complete_Prompt', 
      JSON.stringify({
    ...
    Logger.log(`Saved complete prompt to: ${promptDoc}`);
    logToSheet(logsSheet, 'Prompt Construction', 'All', 'Created LLM prompt', promptDoc, chataId);
    ...
    const responseDoc = saveDetailedLog('LLM_Response', llmResponse, chataId);
    Logger.log(`Saved LLM response to: ${responseDoc}`);
    logToSheet(logsSheet, 'API Call', 'All', 'API call completed', responseDoc, chataId);
    ...
    const processedDoc = saveDetailedLog('Processed_Response', 
      JSON.stringify(processedResponse.content, null, 2),
      chataId
    );
    Logger.log(`Saved processed response to: ${processedDoc}`);
    logToSheet(logsSheet, 'Response Processing', 'All', 'Processed LLM response', processedDoc, chataId);
