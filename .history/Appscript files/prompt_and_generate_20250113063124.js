...
function testPromptGeneration() {
  ...
  try {
    ...
    Logger.log('[Info] Test CHATA ID state:', {
      ...
      value: testChataId,
      allProperties: PropertiesService.getScriptProperties().getProperties()
    });
    
    if (!testChataId) {
      const error = 'TEST_CHATA_ID not found in script properties. Please run setup() first.';
      Logger.log('[FAIL] Missing TEST_CHATA_ID:', {
        availableProperties: PropertiesService.getScriptProperties().getProperties()
      });
      throw new Error(error);
    }
    
    // 4. Get sample form data
    Logger.log('[Step 4] Preparing sample form data...');
    ...
    Logger.log('[Info] Sample form data validation:', {
      hasMetadata: !!sampleFormData.metadata,
      hasSensoryProfile: !!sampleFormData.sensory_profile,
      hasSocialComm: !!sampleFormData.social_communication,
      metadataFields: Object.keys(sampleFormData.metadata)
    });
    
    // 5. Get placeholder mapping
    ...
    Logger.log('[Info] Placeholder map validation:', {
      ...
      placeholders: Object.keys(placeholderMap),
      hasDescriptions: Object.values(placeholderMap).every(p => p.description),
      hasWordCounts: Object.values(placeholderMap).every(p => p.wordCount)
    });
    ...
    let prompt;
    try {
      prompt = constructPrompt(sampleFormData, placeholderMap);
      Logger.log('[Success] Prompt constructed');
    } catch (e) {
      Logger.log('[FAIL] Failed to construct prompt:', {
        error: e.message,
        stack: e.stack,
        formDataKeys: Object.keys(sampleFormData),
        placeholderKeys: Object.keys(placeholderMap)
      });
      throw e;
    }
    
    // 7. Validate prompt structure
    Logger.log('[Step 7] Validating prompt structure...');
    const promptValidation = {
      ...
      userPromptParseable: false,
      systemPromptContains: {
        criticalRequirements: prompt?.systemPrompt?.includes('CRITICAL REQUIREMENTS'),
        formatInstructions: prompt?.systemPrompt?.includes('Format your response'),
        restrictions: prompt?.systemPrompt?.includes('DO NOT')
      }
    };
    
    Logger.log('[Info] Prompt validation results:', promptValidation);
    
    try {
      JSON.parse(prompt.userPrompt);
      promptValidation.userPromptParseable = true;
      Logger.log('[Success] User prompt is valid JSON');
    } catch (e) {
      Logger.log('[FAIL] User prompt is not valid JSON:', {
        error: e.message,
        preview: prompt?.userPrompt?.substring(0, 100)
      });
      promptValidation.parseError = e.message;
    }
    
    if (!prompt || !prompt.systemPrompt || !prompt.userPrompt) {
      const error = 'Invalid prompt structure generated';
      Logger.log('[FAIL] Invalid prompt structure:', {
        hasPrompt: !!prompt,
        hasSystemPrompt: !!prompt?.systemPrompt,
        hasUserPrompt: !!prompt?.userPrompt
      });
    ...
    Logger.log('[Step 8] Saving detailed logs...');
    let systemPromptDoc, userPromptDoc;
    try {
      systemPromptDoc = saveDetailedLog('Test_SystemPrompt', prompt.systemPrompt, testChataId);
      userPromptDoc = saveDetailedLog('Test_UserPrompt', prompt.userPrompt, testChataId);
      Logger.log('[Success] Saved detailed logs');
    } catch (e) {
      Logger.log('[FAIL] Failed to save detailed logs:', {
        error: e.message,
        stack: e.stack
      });
      throw e;
    ...
    // 9. Log to sheet
    Logger.log('[Step 9] Logging to sheet...');
    try {
      logToSheet(
        logsSheet,
        'Test Prompt',
        'System',
        'Generated system prompt for testing',
        JSON.stringify({
          docUrl: systemPromptDoc,
      ...
      logToSheet(
        ...
        'Test Prompt',
        'User',
        'Generated user prompt for testing',
        JSON.stringify({
          docUrl: userPromptDoc,
          length: prompt.userPrompt.length,
          timestamp: new Date().toISOString()
        }),
        testChataId
      );
      ...
      Logger.log('[FAIL] Failed to log to sheet:', {
        error: e.message,
        stack: e.stack
      });
      throw e;
    ...
    Logger.log('[COMPLETE] Prompt generation test completed successfully');
    return {
      success: true,
    ...
    Logger.log('\n[ERROR] Prompt generation test failed:', {
      message: error.message,
      stack: error.stack,
    ...
    // Log error details to sheet if possible
    try {
      if (logsSheet) {
        logToSheet(
          logsSheet,
      ...
      Logger.log('[WARN] Failed to log error to sheet:', e.message);
    }
