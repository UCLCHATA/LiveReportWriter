...
export const App: React.FC = () => {
  ...
  const { formState, saveForm, updateProgress, submitForm, clearForm } = useFormPersistence(chataId);

  // Load stored form state when component mounts or chataId changes
  useEffect(() => {
    if (formState) {
      setClinicianInfo(formState.clinicianInfo);
      setFormProgress(formState.progress.formProgress);
      setCarouselProgress(formState.progress.carouselProgress);
      setShowForm(true);
    }
  }, [formState]);
  ...
  const handleClinicianSubmit = (info: ClinicianInfo) => {
    // Ensure we have a CHATA ID before proceeding
    if (!info.chataId) {
      console.error('No CHATA ID provided');
      return;
    }
    ...
    saveForm({
      clinicianInfo: info,
      progress: { formProgress: 0, carouselProgress: 0 }
    });
  };
  ...
  const handleFormProgressUpdate = (progress: number) => {
    ...
    if (chataId) {
      updateProgress(progress, carouselProgress);
    }
  };
  ...
  const handleCarouselProgressUpdate = (progress: number) => {
    ...
    if (chataId) {
      updateProgress(formProgress, progress);
    }
  };
          ...
          <>
            <div className="left-panel active">
              <AssessmentCarousel 
                ...
                formState={formState?.assessmentState}
                onStateChange={(state) => saveForm({ assessmentState: state })}
              />
            </div>
            <div className="right-panel active">
              <div className="form-container active">
                <AssessmentForm 
                  ...
                  onStateChange={(state) => saveForm({ formState: state })}
