  ...
  useEffect(() => {
    ...
    if (storedForm) {
      setGlobalState(prev => ({
        ...
        assessments: storedForm.assessments as Required<AssessmentData>,
        lastUpdated: new Date(storedForm.lastUpdated).toISOString(),
        status: storedForm.isSubmitted ? 'submitted' : 'draft'
      }));
    }
  }, []);
