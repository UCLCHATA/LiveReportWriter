...
// Operation timings can stay as a global object
...
function processMilestoneData(
  ...
  const processedMilestones = saved.milestones?.filter(isMilestone).map((m: Milestone) => ({
    ...m,
  ...
  const processedCustomMilestones = saved.customMilestones?.filter(isMilestone).map((m: Milestone) => ({
    ...m,
    actualAge: m.actualAge,
    stackPosition: m.stackPosition,
    status: m.status || 'pending'
...
function processAssessmentLogData(
  ...
  const processedSelectedAssessments = saved.selectedAssessments?.filter(isAssessmentEntry).map((assessment: AssessmentEntry) => ({
    id: assessment.id,
...
export const useFormState = () => {
  // Move all hook declarations inside
            ...
            sensoryProfile: {
              ...
              domains: Object.fromEntries(
                Object.entries(initialState.assessments.sensoryProfile.domains).map(([key, defaultDomain]) => {
                  ...
                  if (isSensoryDomain(savedDomain)) {
                    return [key, {
                      ...defaultDomain,
                      value: savedDomain.value,
                      observations: [...savedDomain.observations],
                      label: savedDomain.label
                    }];
                  }
                  return [key, defaultDomain];
                })
              ),
            ...
            socialCommunication: {
              ...
              domains: Object.fromEntries(
                Object.entries(initialState.assessments.socialCommunication.domains).map(([key, defaultDomain]) => {
                  ...
                  if (isSocialCommunicationDomain(savedDomain)) {
                    return [key, {
                      ...defaultDomain,
                      value: savedDomain.value,
                      observations: [...savedDomain.observations],
            ...
            behaviorInterests: {
              ...
              domains: Object.fromEntries(
                Object.entries(initialState.assessments.behaviorInterests.domains).map(([key, defaultDomain]) => {
                  ...
                  if (isBehaviorDomain(savedDomain)) {
                    return [key, {
                      ...defaultDomain,
                      value: savedDomain.value,
                      observations: [...savedDomain.observations],
  ...
  useEffect(() => {
    return () => {
      ...
      // Log cleanup
      console.log(`🧹 [${hookId.current}] Cleaning up ${componentName.current}:`, {
        pendingSaveCleared: !!saveTimeoutRef.current,
        operationLockReleased: true,
        finalSaveAttempts: saveAttemptCountRef.current
      });
    };
  }, []);
