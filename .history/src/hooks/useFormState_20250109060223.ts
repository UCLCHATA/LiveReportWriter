...
function processMilestoneData(
  ...
  const processedMilestones = saved.milestones?.map(m => {
    if (!isMilestone(m)) return null;
    return {
      ...m,
      actualAge: typeof m.actualAge === 'number' ? m.actualAge : undefined,
      stackPosition: typeof m.stackPosition === 'number' ? m.stackPosition : undefined,
      status: m.status || 'pending'
    };
  }).filter((m): m is Milestone => m !== null) || initial.milestones;

  // Validate and process custom milestones
  const processedCustomMilestones = saved.customMilestones?.map(m => {
    if (!isMilestone(m)) return null;
    return {
      ...m,
      actualAge: typeof m.actualAge === 'number' ? m.actualAge : undefined,
