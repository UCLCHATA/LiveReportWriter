          ...
          assessments: Object.entries(state.assessments).reduce((acc, [key, value]) => {
            ...
            switch (value.type) {
              ...
              case 'behaviorInterests':
                // These types have domains
                const domains = Object.entries(value.domains).reduce((domainAcc, [domainKey, domain]) => {
              ...
              case 'milestoneTracker':
                // Milestone tracker has milestones and customMilestones
                acc[key as keyof AssessmentData] = {
                  ...value,
                  milestones: value.milestones || [],
                  customMilestones: value.customMilestones || []
                };
                break;
                
              case 'assessmentLog':
                // Assessment log has selectedAssessments and entries
                acc[key as keyof AssessmentData] = {
                  ...value,
                  selectedAssessments: value.selectedAssessments || [],
                  entries: value.entries || {}
                };
                break;
                
              case 'summary':
                // Summary just needs basic fields
                acc[key as keyof AssessmentData] = {
                  ...value
                };
                break;
