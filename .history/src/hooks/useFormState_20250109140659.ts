...
function isValidState(state: GlobalFormState): boolean {
...
function hasValidContent(state: GlobalFormState): boolean {
  ...
  const hasAssessmentContent =
    Object.values(state.assessments.sensoryProfile.domains).some(d => d.value > 0 || d.observations.length > 0) ||
    Object.values(state.assessments.socialCommunication.domains).some(d => d.value > 0 || d.observations.length > 0) ||
    Object.values(state.assessments.behaviorInterests.domains).some(d => d.value > 0 || d.observations.length > 0) ||
    state.assessments.milestones.milestones.some(m => m.actualAge !== undefined) ||
    state.assessments.assessmentLog.selectedAssessments.length > 0;
            ...
            sensoryProfile: {
              ...
              domains: Object.fromEntries(
                Object.entries(initialState.assessments.sensoryProfile.domains).map(([key, defaultDomain]) => {
                  const savedDomain = validation.state.assessments?.sensoryProfile?.domains?.[key];
                  return [key, {
                    ...defaultDomain,
            ...
            socialCommunication: {
              ...
              domains: Object.fromEntries(
                Object.entries(initialState.assessments.socialCommunication.domains).map(([key, defaultDomain]) => {
                  const savedDomain = validation.state.assessments?.socialCommunication?.domains?.[key];
                  return [key, {
                    ...defaultDomain,
                    value: savedDomain?.value ?? defaultDomain.value,
                    observations: savedDomain?.observations ?? [],
            ...
            behaviorInterests: {
              ...
              domains: Object.fromEntries(
                Object.entries(initialState.assessments.behaviorInterests.domains).map(([key, defaultDomain]) => {
                  const savedDomain = validation.state.assessments?.behaviorInterests?.domains?.[key];
                  return [key, {
                    ...defaultDomain,
                    value: savedDomain?.value ?? defaultDomain.value,
                    observations: savedDomain?.observations ?? [],
