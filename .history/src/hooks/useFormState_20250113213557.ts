...
export const getStorageKey = (chataId: string = '') => {
  ...
  if (!chataId) {
    ...
    return `chata-draft-${draftId}`;
  }
  return `chata-form-${chataId}`;
};
...
export const useFormState = () => {
  ...
  const [globalState, setGlobalState] = useState<GlobalFormState>(() => {
    try {
      ...
      if (!saved) {
        // Try to get chataId from URL params
        const urlParams = new URLSearchParams(window.location.search);
        const chataIdFromUrl = urlParams.get('chataId');
        if (chataIdFromUrl) {
          return { ...initialState, chataId: chataIdFromUrl };
        }
        return initialState;
      }
      ...
      if (!isValidState(parsed)) {
        // Even if state is invalid, try to preserve chataId
        const chataId = parsed?.chataId || '';
        return { ...initialState, chataId };
      }
      
      // Ensure we preserve chataId and all progress values and assessment states
      const restoredState: GlobalFormState = {
        ...initialState,
        ...parsed,
        chataId: parsed.chataId || '', // Explicitly preserve chataId
        formData: {
  ...
  const saveState = useCallback(
    debounce((state: GlobalFormState) => {
      try {
        // Always use the current chataId for storage key, fallback to draft
