...
export const useFormState = () => {
  const [globalState, setGlobalState] = useState<GlobalFormState>(() => {
    ...
    try {
      ...
      console.log('Attempting to restore state from localStorage:', saved);
      if (saved) {
        const parsed = JSON.parse(saved);
        if (parsed.status === 'draft') {
          console.log('Successfully restored state:', parsed);
          return parsed;
        }
      }
    } catch (error) {
  ...
  const updateAssessment = (type: keyof AssessmentData, data: any) => {
    console.log('Updating assessment:', type, data);
    setGlobalState(prev => ({
      ...prev,
      assessments: {
        ...prev.assessments,
