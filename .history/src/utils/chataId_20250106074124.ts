...
export const validateChataId = (chataId: string): boolean => {
  ...
  if (!pattern.test(chataId)) {
    console.log('CHATA ID validation failed: Invalid format', { chataId, expectedFormat: 'XXX-XXX-NNN' });
    return false;
  }
  
  // Additional validation if needed
  const [prefix, childCode, number] = chataId.split('-');
  ...
  console.log('CHATA ID validation passed:', { chataId, prefix, childCode, number });
  return true;
};
