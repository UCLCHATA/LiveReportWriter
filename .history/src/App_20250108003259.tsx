...
export const App: React.FC = () => {
  ...
  const [showForm, setShowForm] = useState(() => {
    // Show form immediately if we have saved state
    const hasState = localStorage.getItem('chata-form-state') !== null;
    console.log('Initial showForm state:', hasState);
    return hasState;
  });
  ...
  useEffect(() => {
    console.log('App mounted, globalState:', globalState);
    if (globalState.formData?.progress || globalState.assessments?.progress) {
      console.log('Found saved progress, showing form');
      setShowForm(true);
      setFormProgress(globalState.formData?.progress || 0);
      setCarouselProgress(globalState.assessments?.progress || 0);
    }
  }, [globalState]);
  ...
  const handleFormProgressUpdate = (progress: number) => {
    console.log('Form progress update:', progress);
    setFormProgress(progress);
    updateFormData({ progress });
  };

  const handleCarouselProgressUpdate = (progress: number) => {
    console.log('Carousel progress update:', progress);
