...
export const App: React.FC = () => {
  const { 
    globalState, 
    setClinicianInfo: setGlobalClinicianInfo,
    clearState,
    updateFormData,
    checkExistingDraft,
    restoreDraft,
  ...
  const handleClinicianSubmit = (info: ClinicianInfo) => {
    if (info.chataId) {
      const existingDraft = checkExistingDraft(info.chataId);
      if (existingDraft) {
        restoreDraft(info.chataId);
      }
    }
    setGlobalClinicianInfo(info);
    setShowModal(false);
    setShowForm(true);
  };
  ...
  const handleCarouselProgressUpdate = (progress: number) => {
    ...
    updateFormData({ carouselProgress: progress });
  };
