...
export const App: React.FC = () => {
  ...
  const [showForm, setShowForm] = useState(() => {
    const hasState = localStorage.getItem('chata-form-state') !== null;
    console.log('🔄 Initial state:', hasState ? 'Found' : 'Not found');
    return hasState;
  });

  const [showModal, setShowModal] = useState(false);
  const [formProgress, setFormProgress] = useState(() => globalState.formData?.formProgress || 0);
  const [carouselProgress, setCarouselProgress] = useState(() => globalState.assessments?.sensoryProfile.progress || 0);
  const [lastCelebrated, setLastCelebrated] = useState<number>(0);

  // Load form state when component mounts
  useEffect(() => {
    const hasProgress = globalState.formData?.formProgress || globalState.assessments?.sensoryProfile.progress;
    const hasContent = 
    ...
    if (!showForm && (hasProgress || hasContent)) {
      console.log('📊 Restoring state:', {
        form: globalState.formData?.formProgress,
        carousel: globalState.assessments?.sensoryProfile.progress,
        hasContent: !!hasContent
      });
      setShowForm(true);
      setFormProgress(globalState.formData?.formProgress || 0);
      setCarouselProgress(globalState.assessments?.sensoryProfile.progress || 0);
    }
  ...
  const handleClinicianSubmit = (info: Partial<ClinicianInfo>) => {
    console.log('👤 Setting clinician info');
    setClinicianInfo({
      name: info.name || '',
      email: info.email || '',
