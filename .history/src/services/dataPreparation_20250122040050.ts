...
function formatDomainData(domain: any): string {
    console.log('🔍 Raw domain data for observations:', JSON.stringify(domain, null, 2));
    ...
    console.log('Domain observations check:', {
        ...
        isUnused,
        shouldBeSkipped: isUnused
    });

    if (isUnused) {
        console.log('✓ Skipping observations - domain is unused or has no value');
        return '';
    }
...
function formatDomainScore(domain: any): string {
    console.log('🔍 Raw domain data:', JSON.stringify(domain, null, 2));
    ...
    const isUnused = !domain || 
                    ...
                    domain.value === null || 
                    domain.value === 0 || 
                    !domain.label;  // Also check if label exists
    ...
    if (isUnused) {
        console.log('✓ Marking as Skipped - domain is unused or has no value');
        return 'Skipped';
    }
