...
function prepareMilestoneData(milestones: any) {
    ...
    communicationMilestones.forEach((milestone: any) => {
        const name = milestone.milestone || milestone.name || 'Unnamed Milestone';
        ...
        formattedString += `${name}: Expected ${milestone.expectedAge}m (${status})\n`;
    });
    formattedString += '\n';
    ...
    concerns.forEach((concern: any) => {
        const name = concern.name || concern.milestone || 'Unnamed Concern';
        const status = concern.status || 'Not Present';
        formattedString += `${name}: ${status}\n`;
    });
