...
export async function submitFormData(formData: any): Promise<SheetyResponse> {
  try {
    ...
    // Format the data with explicit column mapping
    const formDataToSubmit = {
      r3Form: {
        ...
        referrals: Array.isArray(formData.referrals) 
          ? formData.referrals.filter(Boolean).join(', ')
          : Object.entries(formData.referrals)
        ...
        milestone_history: formData.milestoneHistory || '',
        status: 'submitted',
        last_updated: new Date().toISOString()
      }
    };

    console.log('Submitting data to Sheety with columns:', Object.keys(formDataToSubmit.r3Form));
    
    // Submit to Sheety API with retry logic
    const response = await submitToSheetyAPI(R3_FORM_API, formDataToSubmit);
    
    console.log('✓ Form submission completed successfully:', {
      responseId: response.r3Form?.id,
      chataId: response.r3Form?.chata_id,
      timestamp: response.r3Form?.timestamp
    });
