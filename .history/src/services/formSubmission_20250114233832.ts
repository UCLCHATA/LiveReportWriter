...
export const submitFormData = async (data: FormSubmissionData) => {
  ...
  // Get the radar chart image from the summary data
  const radarChartImage = data.globalState.assessments.summary?.radarChartImage || '';
  const timelineImage = data.globalState.assessments.milestones?.timelineImage || '';
  ...
  const rawData: Record<string, any> = {
    ...
    milestoneImage: `${data.globalState.assessments.milestones?.includeTimelineInReport ? '{{Include}}' : '{{Not-Include}}'} ${timelineImage}`,
    ...
    // Combined Graph
    combinedGraphImage: `${data.globalState.assessments.summary?.includeInReport ? '{{Include}}' : '{{Not-Include}}'} ${radarChartImage}`
  };
