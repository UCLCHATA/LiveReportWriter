...
class FormPersistenceService {
  ...
  saveForm(data: FormData): void {
    ...
    // Remove any older forms with the same CHATA ID
    const cleanedForms = Object.entries(forms).reduce((acc, [id, form]) => {
      // Keep forms with different CHATA IDs and submitted forms
      if (id !== data.chataId || form.isSubmitted) {
        acc[id] = form;
      }
      return acc;
    ...
    // Add the new form
    cleanedForms[data.chataId] = {
      ...data,
      lastUpdated: Date.now()
    };

    console.log('Updated forms:', cleanedForms);
