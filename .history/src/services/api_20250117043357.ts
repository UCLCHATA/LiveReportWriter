...
function formatFormData(formData: any) {
    ...
    console.log('📝 Assessment Form Texts:', {
        ...
        supportAreas: formData.prioritySupportAreas || ''
    });

    // Log profile scores with proper field names
    console.log('🎯 Sensory Profile Scores:', {
        visual: formData.visualScore,
        auditory: formData.auditoryScore,
        tactile: formData.tactileScore,
        vestibular: formData.vestibularScore,
        proprioceptive: formData.proprioceptiveScore,
        oral: formData.oralScore
    });

    console.log('🤝 Social Communication Scores:', {
        jointAttention: formData.jointAttentionScore,
        nonverbalCommunication: formData.nonverbalCommunicationScore,
        verbalCommunication: formData.verbalCommunicationScore,
        socialUnderstanding: formData.socialUnderstandingScore,
        playSkills: formData.playSkillsScore,
        peerInteractions: formData.peerInteractionsScore
    });

    console.log('🔄 Behavior Interests Scores:', {
        repetitiveBehaviors: formData.repetitiveBehaviorsScore,
        routinesRituals: formData.routinesRitualsScore,
        specialInterests: formData.specialInterestsScore,
        sensoryInterests: formData.sensoryInterestsScore,
        emotionalRegulation: formData.emotionalRegulationScore,
    ...
    console.log('📊 Assessment Log:', {
        entries: formData.assessmentLogData ? 
            `${Object.keys(JSON.parse(formData.assessmentLogData)).length} entries` : 'No entries'
    });

    console.log('🖼️ Images:', {
        radarChartIncluded: Boolean(formData.radarChartImage?.include),
        milestoneTimelineIncluded: Boolean(formData.milestoneTimelineImage?.include)
    });
