...
export function validateDataFormat(data: Record<string, any>): ValidationResult {
    ...
    const requiredClinicianFields = [
        SHEETY_HEADERS.CLINICIAN.name,
        SHEETY_HEADERS.CLINICIAN.email,
        SHEETY_HEADERS.CLINICIAN.clinic
    ];
    
    requiredClinicianFields.forEach(field => {
        if (!data[field]?.trim()) {
            result.missingFields.push(field);
        }
    });

    // Optional clinician fields don't need validation
