...
export function prepareFormDataForSubmission(globalState: GlobalFormState) {
    ...
    // Log image data sizes
    if (assessments?.milestones?.timelineImage) {
        console.log('📏 Timeline image submission size:', Math.round(assessments.milestones.timelineImage.length / 1024), 'KB');
    }
    if (assessments?.summary?.radarChartImage) {
        console.log('📏 Radar chart submission size:', Math.round(assessments.summary.radarChartImage.length / 1024), 'KB');
    }

    // Combine main data (excluding images)
