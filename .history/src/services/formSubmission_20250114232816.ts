...
export const submitFormData = async (data: FormSubmissionData) => {
  ...
  // Get the radar chart image from the summary data if it should be included
  const radarChartImage = data.globalState.assessments.summary?.includeInReport ? 
    data.globalState.assessments.summary?.radarChartImage || '' : '';
  const timelineImage = data.globalState.assessments.milestones?.includeTimelineInReport ? 
    data.globalState.assessments.milestones?.timelineImage || '' : '';

  // Add debug logs for image data and toggle flags
  console.log('Image toggle flags:', {
    includeRadarChart: data.globalState.assessments.summary?.includeInReport,
    includeTimeline: data.globalState.assessments.milestones?.includeTimelineInReport
  });

  console.log('Raw image data:', {
    radarChartImage: data.globalState.assessments.summary?.radarChartImage ? 'Present' : 'Not present',
    timelineImage: data.globalState.assessments.milestones?.timelineImage ? 'Present' : 'Not present'
  });
  ...
  const rawData: Record<string, any> = {
    ...
    milestoneImage: data.globalState.assessments.milestones?.includeTimelineInReport ? timelineImage : '',
    ...
    // Combined Graph
    combinedGraphImage: data.globalState.assessments.summary?.includeInReport ? radarChartImage : ''
  };
