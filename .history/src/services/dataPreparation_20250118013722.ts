...
function prepareMilestoneData(milestones: any) {
    ...
    console.log('📝 Communication milestones:', communicationMilestones);
    
    communicationMilestones.forEach((milestone: any) => {
        console.log('Processing milestone:', milestone);
        const milestoneName = milestone.description || milestone.milestone || milestone.name || 'Unnamed Milestone';
        const status = milestone.achievedAge !== null && milestone.achievedAge !== undefined
            ? milestone.achievedAge < milestone.expectedAge
                ? `Early by ${milestone.expectedAge - milestone.achievedAge} months`
                : milestone.achievedAge > milestone.expectedAge
                    ? `Delayed by ${milestone.achievedAge - milestone.expectedAge} months`
                    : 'On Time'
            : 'Not yet achieved';
        formattedString += `${milestoneName}: Expected ${milestone.expectedAge}m (${status})\n`;
    });
    ...
    console.log('📝 Motor milestones:', motorMilestones);
    ...
    console.log('📝 Social milestones:', socialMilestones);
    ...
    concerns.forEach((concern: any) => {
        const concernName = concern.description || concern.name || concern.milestone || 'Unnamed Concern';
        formattedString += `${concernName}: ${concern.status || 'Not Present'}\n`;
    });
    ...
    const formattedData = {
        ...
        metadata: {
            totalMilestones: milestones.milestones.length,
            categories: ['COMMUNICATION', 'MOTOR', 'SOCIAL', 'CONCERNS'],
            lastUpdated: new Date().toISOString()
        }
    };

    console.log('📦 Final formatted milestone data:', formattedData);
