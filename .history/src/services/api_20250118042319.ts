...
export const setMilestoneImageHeader = async (chunk: MilestoneImageChunk): Promise<void> => {
  console.log('[API] Setting milestone image header');
  try {
    // Set the custom header with the image data
    const headers = new Headers();
    headers.append('Milestone_Image_Chunk_1', JSON.stringify(chunk));
    
    // Make a request to update headers
    const response = await fetch('/api/milestone/header', {
      method: 'POST',
      headers,
      body: JSON.stringify({ action: 'set_header' })
    });

    if (!response.ok) {
      throw new Error(`Failed to set header: ${response.statusText}`);
    }

    console.log('[API] Successfully set milestone image header');
  } catch (error) {
    console.error('[API] Error setting milestone image header:', error);
    throw error;
  }
};

export const clearMilestoneImageHeader = async (): Promise<void> => {
  console.log('[API] Clearing milestone image header');
  try {
    // Make a request to clear the header
    const response = await fetch('/api/milestone/header', {
      method: 'DELETE',
    });

    if (!response.ok) {
      throw new Error(`Failed to clear header: ${response.statusText}`);
    }

    console.log('[API] Successfully cleared milestone image header');
  } catch (error) {
    console.error('[API] Error clearing milestone image header:', error);
    throw error;
  }
}; 