...
export const submitFormData = async (data: FormData) => {
  ...
  try {
    const response = await submitToSheetyAPI(formattedData);
...
export const submitToSheetyAPI = async (data: SheetyFormData, endpoint: string = R3_FORM_API) => {
  try {
    const response = await fetch(endpoint, {
      ...
      body: JSON.stringify({ r3form: data })
    });
    ...
    console.error('Error in submitToSheetyAPI:', error);
    throw error;
  }
}; 