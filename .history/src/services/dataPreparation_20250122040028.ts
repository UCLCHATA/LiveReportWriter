...
function formatDomainData(domain: any): string {
    console.log('🔍 Raw domain data for observations:', domain);
    
    // Check if the domain has been actively set (value > 0)
    const isUnused = !domain || 
                    typeof domain.value === 'undefined' || 
                    domain.value === null || 
                    domain.value === 0;  // If value is 0, consider it unused
                    
    console.log('Domain observations check:', {
        domain: !!domain,
        hasValue: typeof domain?.value !== 'undefined',
        value: domain?.value,
        hasObservations: Array.isArray(domain?.observations),
        isUnused
    });

    if (isUnused) {
        console.log('✓ Skipping observations - domain value is 0 or undefined');
        return '';
    }

    const formatted = Array.isArray(domain.observations) ? domain.observations.join('. ') : '';
    console.log('✓ Using formatted observations:', formatted);
    return formatted;
}

// Helper to format domain score
function formatDomainScore(domain: any): string {
    console.log('🔍 Raw domain data:', domain);
    
    // Check if the domain has been actively set (value > 0)
    const isUnused = !domain || 
                    typeof domain.value === 'undefined' || 
                    domain.value === null || 
                    domain.value === 0;  // If value is 0, consider it unused regardless of label
                    
    console.log('Domain usage check:', {
        domain: !!domain,
        hasValue: typeof domain?.value !== 'undefined',
        value: domain?.value,
        label: domain?.label,
    ...
    if (isUnused) {
        console.log('✓ Marking as Skipped - domain value is 0 or undefined');
        return 'Skipped';
    }

    const formattedScore = `${domain.label}, ${domain.value} out of 5 on the 5 point scale`;
    console.log('✓ Using formatted score:', formattedScore);
...
function prepareSensoryProfileData(profile: any) {
    ...
    console.log('🔍 Raw sensory domains:', JSON.stringify(profile?.domains, null, 2));
    ...
    domains.forEach(domain => {
        ...
        console.log(`🔍 Processing ${domain} domain:`, JSON.stringify(domainData, null, 2));
...
function prepareSocialCommunicationData(profile: any) {
    ...
    console.log('🔍 Raw social domains:', JSON.stringify(profile?.domains, null, 2));
...
function prepareBehaviorInterestsData(profile: any) {
    ...
    console.log('🔍 Raw behavior domains:', JSON.stringify(profile?.domains, null, 2));
