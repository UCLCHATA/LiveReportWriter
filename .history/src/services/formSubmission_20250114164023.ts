...
export const prepareSheetyData = ({ formData, globalState, radarChartImage }: FormSubmissionData) => {
  ...
  const milestoneTimelineData = formData.milestones
    ? formData.milestones
        .map(milestone => 
          `${milestone.title}|${milestone.description}|${milestone.expectedAge}|${milestone.actualAge}|${milestone.status}`
        )
        .join(' || ')
    : '';
  ...
  const sheetyData = {
    r3Form: {
      ...
      assessmentLogData: Object.entries(globalState.assessments.assessmentLog.entries)
