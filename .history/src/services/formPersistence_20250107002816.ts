...
class FormPersistenceService {
  constructor() {
    ...
    // Clean up submitted forms on initialization
    this.cleanupSubmittedForms();
  ...
  private cleanupSubmittedForms(): void {
    const forms = this.getStoredForms();
    const unsubmittedForms = Object.entries(forms).reduce((acc, [id, form]) => {
      if (!form.isSubmitted) {
        acc[id] = form;
      }
      return acc;
    }, {} as Record<string, FormData>);
    this.setStoredForms(unsubmittedForms);
  }
  ...
  saveForm(data: FormData): void {
    ...
    // Remove any existing forms with the same CHATA ID
    delete forms[data.chataId];
  ...
  getForm(chataId: string): FormData | null {
    ...
    // Only return the form if it exists and is not submitted
    if (form && !form.isSubmitted) {
      console.log('Found unsubmitted form:', form);
      return form;
    }
    console.log('No unsubmitted form found for CHATA ID:', chataId);
    return null;
  }
  ...
  markAsSubmitted(chataId: string): void {
    ...
    if (forms[chataId]) {
      ...
      this.setStoredForms(forms);
      // Clean up other submitted forms
      this.cleanupSubmittedForms();
    }
  }
