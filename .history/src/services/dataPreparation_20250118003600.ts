...
function formatDomainScore(domain: any): string {
    ...
    // For zero values, return 'Skipped'
    if (domain.value === 0) {
        return 'Skipped';
    }
...
function prepareMilestoneData(milestones: any) {
    ...
    const groupedMilestones = milestones.milestones?.reduce((acc: any, milestone: any) => {
    ...
    let formattedData = '';
    Object.entries(groupedMilestones).forEach(([category, categoryMilestones]: [string, any]) => {
        formattedData += `=== ${category.toUpperCase()} ===\n`;
        categoryMilestones.forEach((milestone: any) => {
            const status = milestone.achievedAge ? 
            ...
            formattedData += `${milestone.name}: Expected ${milestone.expectedAge}m (${status})\n`;
        });
        formattedData += '\n';
    });

    // Add concerns section
    formattedData += '=== CONCERNS ===\n';
    const concerns = milestones.concerns || [];
    concerns.forEach((concern: any) => {
        formattedData += `${concern.name}: ${concern.status || 'Not Present'}\n`;
    });

    return {
        [SHEETY_HEADERS.MILESTONE.timelineData]: formattedData.trim(),
...
function prepareClinicianData(clinician: any) {
    ...
    const formattedData = {
        ...
        Child_SecondName: clinician?.childLastName?.trim() || '',
