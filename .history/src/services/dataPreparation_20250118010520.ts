...
function prepareMilestoneData(milestones: any) {
    if (!milestones) return {
        [SHEETY_HEADERS.MILESTONE.timelineData]: JSON.stringify({
            sections: [],
            concerns: []
        }),
    ...
    const sections = ['COMMUNICATION', 'MOTOR', 'SOCIAL', 'CONCERNS'].map(category => {
        const milestoneList = milestones.milestones
            ?.filter((m: any) => m.category.toUpperCase() === category)
            ?.map((milestone: any) => {
                let status = 'Skipped';
        ...
        return {
            category,
            milestones: milestoneList.sort((a: any, b: any) => a.expectedAge - b.expectedAge)
        };
    });
    ...
    const concerns = milestones.concerns?.map((concern: any) => ({
        name: concern.name,
        status: concern.achievedAge !== null && concern.achievedAge !== undefined ? 
            `Present at ${concern.achievedAge} months` : 
            'Not Present'
    })) || [
        { name: 'Rigid play patterns', status: 'Not Present' },
        { name: 'Limited social engagement', status: 'Not Present' },
        { name: 'Sensory seeking/avoiding', status: 'Not Present' }
    ];
