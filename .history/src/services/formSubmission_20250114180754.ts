...
export const prepareSheetyData = ({ formData, globalState, radarChartImage }: FormSubmissionData) => {
  ...
  if (formData.milestones && formData.milestones.length > 0) {
    ...
    milestoneTimelineData = formData.milestones
      ...
      .map(milestone => {
        const formatted = `${milestone.title}|${milestone.category}|${milestone.expectedAge}|${milestone.actualAge}|${milestone.status}|${milestone.difference > 0 ? `Delayed by ${milestone.difference}m` : milestone.difference < 0 ? `Advanced by ${Math.abs(milestone.difference)}m` : 'On Track'}|${milestone.id.startsWith('custom-concern-') ? 'custom' : 'standard'}`;
