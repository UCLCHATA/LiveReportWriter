...
export const submitToSheetyAPI = async (url: string, data: any): Promise<SheetyResponse> => {
  const submitFn = async () => {
    try {
      ...
      // Ensure all keys are properly formatted for column names
      const formattedData = {
      ...
      // For image chunks, ensure they update existing row instead of creating new one
      if (data.r3Form.chataId && (data.r3Form.radarChartImage || data.r3Form.timelineImage)) {
        // Find existing row ID first
        const searchResponse = await fetch(`${url}?filter[chata_id]=${data.r3Form.chataId}`, {
          headers: {
            'Authorization': 'Bearer CHATAI'
          }
        });
        const searchResult = await searchResponse.json();
        const existingRow = searchResult.r3Form?.[0];

        if (existingRow?.id) {
          // Update existing row instead of creating new one
          url = `${url}/${existingRow.id}`;
