...
export function prepareFormDataForSubmission(globalState: GlobalFormState) {
    ...
    const imageData = {
        'Milestone_Image': assessments?.milestones?.timelineImage || '',
        'Image1_Include': assessments?.milestones?.includeTimelineInReport ? 'true' : 'false',
        'Combinedradar_Image': assessments?.summary?.radarChartImage || '',
        'Image2_Include': assessments?.summary?.includeInReport ? 'true' : 'false'
    };
