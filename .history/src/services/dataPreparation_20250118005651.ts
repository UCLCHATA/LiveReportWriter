...
function prepareMilestoneData(milestones: any) {
    ...
    milestones.milestones?.forEach((milestone: any) => {
        ...
        if (groupedMilestones[category]) {
            // Only add if it has been achieved or has an achieved age
    ...
    // Format concerns section
    const concerns = [
        'Rigid play patterns',
        'Limited social engagement',
        'Sensory seeking/avoiding'
    ].map(concern => `${concern}: Not Present`).join('\n');

    // Build the formatted string
    let formattedData = '';

    // Add each category
    Object.entries(groupedMilestones).forEach(([category, categoryMilestones]) => {
        formattedData += `=== ${category} ===\n`;
        
        // Sort milestones by expected age
        categoryMilestones
            .sort((a, b) => a.expectedAge - b.expectedAge)
            .forEach(milestone => {
                formattedData += `${milestone.name}: Expected ${milestone.expectedAge}m (${milestone.status})\n`;
            });
    ...
    return {
        [SHEETY_HEADERS.MILESTONE.timelineData]: formattedData,
