...
function prepareMilestoneData(milestones: any) {
    ...
    const categorizedMilestones = milestones.milestones.reduce((acc: Record<string, any[]>, milestone: any) => {
    ...
    const sections = Object.entries(categorizedMilestones)
        ...
        .map(([category, items]: [string, Array<any>]) => {
            const sortedItems = [...items].sort((a, b) => a.expectedAge - b.expectedAge);
