...
const IMAGE_CONFIG = {
    MAX_SIZE: 45000,     // Reduced to 45K character limit
    INITIAL_QUALITY: 0.9, // Start with slightly lower quality
    MIN_QUALITY: 0.5,     // Allow lower quality if needed
    QUALITY_STEP: 0.1,    // Larger steps for faster adjustment
    SCALE: 0.75          // Reduced scale to help with size
};
...
export function chunkImageData(base64Data: string | null, include: boolean = false): { chunk1: string, chunk2: string, chunk3: string } {
    if (!base64Data) {
        console.log('[API] No image data provided for chunking');
        return {
            chunk1: '{{NOT INCLUDE}}',
            chunk2: '{{NOT INCLUDE}}',
            chunk3: '{{NOT INCLUDE}}'
    ...
    if (!include) {
        console.log('[API] Include flag is false, returning NOT INCLUDE markers');
        return {
            chunk1: '{{NOT INCLUDE}}',
            chunk2: '{{NOT INCLUDE}}',
            chunk3: '{{NOT INCLUDE}}'
    ...
    // Add include flag as prefix
    const prefixedData = `{{INCLUDE}}${base64Data}`;
    console.log(`[API] Image data prefixed with include flag, total size: ${prefixedData.length} characters`);

    const chunkSize = Math.ceil(prefixedData.length / 3);
    const chunks = {
        chunk1: prefixedData.slice(0, chunkSize),
        chunk2: prefixedData.slice(chunkSize, chunkSize * 2),
        chunk3: prefixedData.slice(chunkSize * 2)
    };

    console.log(`[API] Image chunked into sizes: ${chunks.chunk1.length}, ${chunks.chunk2.length}, ${chunks.chunk3.length}`);
    return chunks;
}
    ...
    try {
        ...
        console.log('Main form data formatted successfully. Payload:', {
            ...
            clinicianInfo: {
                ...
                clinicName: formattedData.r3Form.clinicName
            },
            childInfo: {
        ...
        // Check image flags
        const shouldSubmitImages = formData.includeImages || formData.milestoneImage?.include;
        console.log('Checking image submission flags:', {
            includeImages: formData.includeImages,
            milestoneImageInclude: formData.milestoneImage?.include,
            shouldSubmitImages
        });
        
        // If images are included, submit them separately
        if (shouldSubmitImages) {
            ...
            console.log('Image submissions skipped (no images to submit)');
        }
