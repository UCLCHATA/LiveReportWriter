...
const IMAGE_CONFIG = {
    MAX_SIZE: 150000,    // Increased max size to 150KB
    INITIAL_QUALITY: 1.0, // Start with maximum quality
    MIN_QUALITY: 0.7,     // Don't go below 70% quality
    QUALITY_STEP: 0.05,   // Smaller steps for finer control
    SCALE: 1.0           // Full scale for better resolution
};
...
export async function generateImage(element: HTMLElement | null): Promise<string | null> {
    ...
    try {
        while (quality >= IMAGE_CONFIG.MIN_QUALITY) {
            console.log(`[API] Attempting image generation with quality: ${quality}`);
            ...
            console.log(`[API] Generated image size: ${size} characters`);
            ...
            console.log(`[API] Image too large, reducing quality to ${quality}`);
        }
        
        if (!imageData || imageData.length > IMAGE_CONFIG.MAX_SIZE) {
            console.error('[API] Failed to generate image within size limit');
            return null;
        }
