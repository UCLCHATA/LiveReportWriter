...
function prepareMilestoneData(milestones: any) {
    ...
    // Ensure history of concerns is a valid string without special markers
    const historyOfConcerns = milestones.historyOfConcerns || '';
    const cleanHistory = historyOfConcerns.replace(/===\s*CONCERNS\s*===/, '').trim();

    return {
        [SHEETY_HEADERS.MILESTONE.timelineData]: JSON.stringify(timelineData),
        [SHEETY_HEADERS.MILESTONE.historyOfConcerns]: cleanHistory
    };
}
