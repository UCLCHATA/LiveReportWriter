...
function prepareMilestoneData(milestones: any) {
    ...
    // Log each milestone's complete data
    milestones.milestones.forEach((m: any, index: number) => {
        console.log(`Milestone ${index + 1}:`, JSON.stringify(m, null, 2));
    });

    // Filter to only placed milestones - check multiple possible properties
    const placedMilestones = milestones.milestones.filter((m: any) => {
        // Log the placement check for each milestone
        console.log('Checking milestone placement:', {
            name: m.name,
            achievedAge: m.achievedAge,
            month: m.month, // New property to check
            placement: m.placement, // New property to check
            y: m.y, // UI might use this for placement
            position: m.position
        });

        const isPlaced = 
            (m.achievedAge !== null && m.achievedAge !== undefined && m.achievedAge !== '') ||
            (m.month !== null && m.month !== undefined && m.month !== '') ||
            (m.y !== null && m.y !== undefined && m.y !== '');
        
        if (isPlaced) {
            console.log('✓ Found placed milestone:', m.name, {
                achievedAge: m.achievedAge,
                month: m.month,
                y: m.y
            });
    ...
    placedMilestones.forEach((milestone: any) => {
        ...
        // Format milestone text based on whether it has an expected age
        let milestoneText;
        if (milestone.expectedAge !== null && milestone.expectedAge !== undefined) {
            const difference = milestone.expectedAge - milestone.achievedAge;
            const status = difference === 0 ? 'On Time' :
                          difference > 0 ? `Early by ${difference} months` :
                          `Delayed by ${Math.abs(difference)} months`;
            
            milestoneText = `${milestone.name}: Placed at ${milestone.achievedAge}m (Expected: ${milestone.expectedAge}m) - ${status}`;
        } else {
            // For developmental concerns that don't have expected ages
            milestoneText = `${milestone.name}: Placed at ${milestone.achievedAge}m`;
        }
    ...
    // Add milestone categories
    Object.entries(groupedMilestones).forEach(([category, entries]) => {
        if (entries.length > 0) {
    ...
    const presentConcerns = milestones.concerns?.filter((c: any) => c.isPresent)
        .map((c: any) => `${c.name}: Present`) || [];

    if (presentConcerns.length > 0) {
        sections.push('=== CONCERNS ===');
        sections.push(...presentConcerns);
    }
