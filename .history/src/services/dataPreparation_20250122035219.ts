...
function formatDomainData(domain: any): string {
    console.log('🔍 Formatting domain data for:', domain);
    
    // Log the exact state of the domain
    const domainState = {
        isDefined: domain !== undefined && domain !== null,
        hasValue: domain?.value !== undefined,
        rawValue: domain?.value,
        hasObservations: Array.isArray(domain?.observations),
        observationsLength: domain?.observations?.length
    };
    console.log('Domain data state:', domainState);

    // Check if domain should be skipped
    if (!domainState.isDefined || !domainState.hasValue || !domainState.rawValue) {
        console.log('✓ Skipping observations because domain is unused');
        return '';
    }
...
function formatDomainScore(domain: any): string {
    console.log('🔍 Formatting domain score for:', domain);
    ...
    const domainState = {
        ...
        rawValue: domain?.value,
        label: domain?.label,
        isDefaultOrUnused: !domain?.value || domain?.value === 0
    };
    console.log('Domain state:', domainState);

    // Check if domain should be marked as skipped
    if (!domainState.isDefined || !domainState.hasValue || domainState.isDefaultOrUnused) {
        console.log('✓ Domain marked as Skipped because:', {
            notDefined: !domainState.isDefined,
            noValue: !domainState.hasValue,
            isDefaultOrUnused: domainState.isDefaultOrUnused
        });
        return 'Skipped';
    }
