...
function formatDomainData(domain: any): string {
    console.log('🔍 Formatting domain data:', {
        domain,
        hasValue: domain?.value !== undefined,
        value: domain?.value,
        isDefault: domain?.value === 0 || domain?.value === null,
        hasObservations: Array.isArray(domain?.observations)
    });

    if (!domain || typeof domain.value === 'undefined' || domain.value === null || domain.value === 0) {
        console.log('✓ Skipping observations for unused domain');
        return '';
    }

    const formatted = Array.isArray(domain.observations) ? domain.observations.join('. ') : '';
    console.log('✓ Formatted observations:', formatted);
    return formatted;
}

// Helper to format domain score
function formatDomainScore(domain: any): string {
    console.log('🔍 Formatting domain score:', {
        domain,
        hasValue: domain?.value !== undefined,
        value: domain?.value,
        isDefault: domain?.value === 0 || domain?.value === null
    ...
    // Consider both undefined and value of 0 as unused/default
    if (!domain || typeof domain.value === 'undefined' || domain.value === null || domain.value === 0) {
        console.log('✓ Marking domain as Skipped');
        return 'Skipped';
    }

    console.log('✓ Using formatted score:', `${domain.label}, ${domain.value} out of 5 on the 5 point scale`);
