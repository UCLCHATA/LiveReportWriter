  ...
  try {
    ...
    const validatedData = {
      r3form: {
        ...
        chataId: sheetyData.r3form.chataId || '',
        timestamp: sheetyData.r3form.timestamp || new Date().toISOString(),
        // Convert any null/undefined values to empty strings
        milestoneImage: sheetyData.r3form.milestoneImage || '',
        combinedGraphImage: sheetyData.r3form.combinedGraphImage || '',
        milestoneTimelineData: sheetyData.r3form.milestoneTimelineData || '',
        assessmentLogData: sheetyData.r3form.assessmentLogData || ''
      }
    };

    // Submit to Sheety API
    console.log('Submitting to Sheety API...');
    const response = await submitToSheetyAPI(R3_FORM_API, validatedData);
