...
function formatFormData(formData: any) {
    ...
    return {
        r3Form: {
            ...
            // Assessment data
            sensoryProfile: JSON.stringify(formData.assessments?.sensoryProfile || {}),
            socialCommunication: JSON.stringify(formData.assessments?.socialCommunication || {}),
            behaviorInterests: JSON.stringify(formData.assessments?.behaviorInterests || {}),
            milestoneTimelineData: JSON.stringify(formData.assessments?.milestones?.milestones || []),
            assessmentLogData: JSON.stringify(formData.assessments?.assessmentLog || {}),
