...
function prepareMilestoneData(milestoneData: any) {
    console.log('🔍 Raw milestone data:', JSON.stringify(milestoneData, null, 2));
    ...
    if (!milestoneData?.milestones) {
        ...
        return {
            ...
            [SHEETY_HEADERS.MILESTONE.historyOfConcerns]: milestoneData?.historyOfConcerns || ''
        };
    }

    // Filter to only placed milestones
    const placedMilestones = milestoneData.milestones.filter((m: any) => {
