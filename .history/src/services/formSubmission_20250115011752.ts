...
const splitBase64Image = (base64Data: string, includeFlag: boolean, maxChunkSize: number = 45000): string[] => {
  ...
  for (let i = 0; i < base64Data.length; i += maxChunkSize) {
    ...
    const isLastChunk = i + maxChunkSize >= base64Data.length;
    
    // Format: {{Include/Not-Include}}|CHUNK_INDEX|TOTAL_CHUNKS|CHUNK_DATA
    const chunkMarker = `${prefix}|${chunks.length + 1}|${Math.ceil(base64Data.length / maxChunkSize)}|`;
...
export const submitFormData = async (data: FormSubmissionData) => {
  ...
  const rawData = {
    ...
    ...splitBase64Image(timelineImage, !!data.globalState.assessments.milestones?.includeTimelineInReport)
      .reduce((acc, chunk, index) => ({
        ...acc,
        [`milestoneImage_${index + 1}`]: chunk
      }), {}),
    ...
    // Combined Graph
    ...splitBase64Image(radarChartImage, !!data.globalState.assessments.summary?.includeInReport)
      .reduce((acc, chunk, index) => ({
        ...acc,
        [`combinedGraphImage_${index + 1}`]: chunk
      }), {})
  };
