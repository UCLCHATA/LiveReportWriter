...
export const prepareSheetyData = ({ formData, globalState, radarChartImage }: FormSubmissionData) => {
  ...
  const formattedMilestones = milestonesToUse
    .filter(milestone => milestone.actualAge !== undefined)
    .map(milestone => {
      const isCustom = milestone.id.startsWith('custom-concern-');
      return `${milestone.title}|${milestone.category}|${milestone.expectedAge}|${milestone.actualAge}|${milestone.status}|${isCustom ? 'custom' : 'standard'}`;
    })
    .join(';');

  console.log('Formatted milestone data:', formattedMilestones);
