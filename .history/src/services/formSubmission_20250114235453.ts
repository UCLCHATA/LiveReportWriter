...
export const submitFormData = async (data: FormSubmissionData) => {
  ...
  // Log the include flags first
  console.log('Include flags:', {
    includeTimeline: data.globalState.assessments.milestones?.includeTimelineInReport,
    includeRadar: data.globalState.assessments.summary?.includeInReport,
    fromProps: {
      includeMilestoneImage: data.includeMilestoneImage,
      includeRadarChart: data.includeRadarChart
    }
  });
  ...
  console.log('Milestone data:', {
    ...
    imageIncluded: data.includeMilestoneImage || data.globalState.assessments.milestones?.includeTimelineInReport
  });
  ...
  console.log('Assessment data:', {
    ...
    chartIncluded: data.includeRadarChart || data.globalState.assessments.summary?.includeInReport
  });
  ...
  // Add debug logs for image data
  console.log('Raw image data:', {
    radarChartImage: radarChartImage ? 'Present' : 'Not present',
    timelineImage: timelineImage ? 'Present' : 'Not present',
    radarChartLength: radarChartImage.length,
    timelineLength: timelineImage.length
  });
  ...
  const rawData: Record<string, any> = {
    ...
    milestoneImage: data.globalState.assessments.milestones?.includeTimelineInReport 
      ? timelineImage 
      : '',
    ...
    // Combined Graph
    combinedGraphImage: data.globalState.assessments.summary?.includeInReport 
      ? radarChartImage 
      : ''
  };
  ...
  // Before submitting, log the image sizes
  console.log('Image sizes before submission:', {
    milestoneImage: stringifiedData.milestoneImage?.length || 0,
    combinedGraphImage: stringifiedData.combinedGraphImage?.length || 0
  });

  // Prepare the final payload
  const shouldIncludeTimeline = data.includeMilestoneImage || data.globalState.assessments.milestones?.includeTimelineInReport;
  const shouldIncludeRadar = data.includeRadarChart || data.globalState.assessments.summary?.includeInReport;

  console.log('Final image inclusion flags:', {
    shouldIncludeTimeline,
    shouldIncludeRadar,
    timelineLength: timelineImage.length,
    radarLength: radarChartImage.length
  });

  const payload = {
    r3Form: {
      ...stringifiedData,
      // Ensure image fields are properly named and formatted
      milestoneImage: shouldIncludeTimeline && timelineImage ? timelineImage : '',
      combinedGraphImage: shouldIncludeRadar && radarChartImage ? radarChartImage : ''
    }
  };
