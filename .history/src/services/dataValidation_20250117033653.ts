...
export function validateDataFormat(data: any): ValidationResult {
    ...
    const validateProfile = (profile: any, headerGroup: any) => {
        Object.values(headerGroup).forEach((domain: any) => {
            if (!data[domain.score] || !data[domain.observations]) {
                result.missingFields.push(domain.score);
            }
            if (!isValidJSON(data[domain.observations])) {
                result.invalidFormats.push(domain.observations);
            }
        });
    };

    validateProfile(data, SHEETY_HEADERS.SENSORY);
    validateProfile(data, SHEETY_HEADERS.SOCIAL);
    validateProfile(data, SHEETY_HEADERS.BEHAVIOR);
    ...
    if (data.includeImages) {
        const validateImageChunks = (imageType: any) => {
