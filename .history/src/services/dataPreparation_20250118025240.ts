...
function prepareMilestoneData(milestones: any) {
    console.log('🔍 Raw milestone data:', {
        ...
        concerns: milestones?.concerns,
        historyOfConcerns: milestones?.historyOfConcerns
    });

    // Log detailed milestone structure
    if (milestones?.milestones?.length) {
        console.log('📊 Detailed milestone structure:', 
            milestones.milestones.map((m: any) => ({
                name: m.name,
                category: m.category,
                achievedAge: m.achievedAge,
                expectedAge: m.expectedAge,
                isPlaced: m.isPlaced, // Check if this exists
                position: m.position, // Check if this exists
                // Log any other properties that might indicate placement
    ...
    const placedMilestones = milestones.milestones.filter((m: any) => {
        const isPlaced = 
            (m.achievedAge !== null && m.achievedAge !== undefined) ||
            (m.isPlaced === true) ||
            (m.position !== null && m.position !== undefined);
        
        if (isPlaced) {
            console.log('✓ Found placed milestone:', m);
        }
        return isPlaced;
    });
