...
export const prepareSheetyData = ({ formData, globalState, radarChartImage }: FormSubmissionData) => {
  ...
  console.log('Preparing Sheety data with:', {
    ...
    timelineImage: formData.timelineImage ? 'Present' : 'Not present',
  ...
  const formatMilestone = (milestone: Milestone) => {
    const difference = milestone.actualAge !== undefined ? milestone.actualAge - milestone.expectedAge : null;
    let status = 'Not Assessed';
    if (difference !== null) {
      if (difference > 0) {
        status = `Delayed by ${difference} months`;
      } else if (difference < 0) {
        status = `Advanced by ${Math.abs(difference)} months`;
      } else {
        status = 'On Track';
      }
    }
    ...
    return {
      ...
      Description: milestone.isCustom ? 'Custom milestone' : `Expected at ${milestone.expectedAge} months`,
      ExpectedMonth: milestone.isCustom ? 'NA' : ensureString(milestone.expectedAge),
      ActualMonth: milestone.actualAge !== undefined ? ensureString(milestone.actualAge) : 'Not Assessed',
      Status: status
    };
  };

  const joinMilestone = (formatted: Record<string, string>) => {
    return `${formatted.Title}|${formatted.Description}|${formatted.ExpectedMonth}|${formatted.ActualMonth}|${formatted.Status}`;
  };

  // Use either the direct milestone data or the pre-formatted timeline data
  const milestoneTimelineData = formData.milestones
    ? formData.milestones
        .map(formatMilestone)
        .map(joinMilestone)
        .join(' || ')
    : formData.milestoneTimelineData || '';
  ...
  const sheetyData = {
    r3Form: {
      ...
      assessmentLogData: assessmentLogData.map(entry => 
        `${ensureString(entry.name)} (${ensureString(entry.date)})${entry.notes ? ': ' + ensureString(entry.notes) : ''}`
      ).join(' | '),
