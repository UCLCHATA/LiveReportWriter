...
function prepareMilestoneData(milestones: any) {
    if (!milestones?.milestones?.length) return {
        [SHEETY_HEADERS.MILESTONE.timelineData]: JSON.stringify({ timelineEntries: '' }),
    ...
    // If we have no content, return a default message
    if (!formattedText) {
        return {
            [SHEETY_HEADERS.MILESTONE.timelineData]: JSON.stringify({ timelineEntries: 'No milestones placed on timeline' }),
            [SHEETY_HEADERS.MILESTONE.historyOfConcerns]: milestones.historyOfConcerns || ''
        };
    }
    
    return {
        [SHEETY_HEADERS.MILESTONE.timelineData]: JSON.stringify({ timelineEntries: formattedText }),
        [SHEETY_HEADERS.MILESTONE.historyOfConcerns]: milestones.historyOfConcerns || ''
    };
}
