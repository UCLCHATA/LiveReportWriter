...
const captureRadarChart = async (): Promise<string> => {
  ...
  const selectors = ['.recharts-wrapper', '.combined-radar-chart', '[data-testid="radar-chart"]'];
  ...
  try {
    const canvas = await html2canvas(chartElement, {
      ...
      onclone: (doc) => {
        ...
        const clonedElement = doc.querySelector(selectors[0]);
        if (clonedElement) {
...
const captureTimelineImage = async (): Promise<string> => {
  ...
  // Try multiple selectors in case the class names vary
  const selectors = ['.timelineWrapper', '.milestone-timeline', '[data-testid="milestone-timeline"]'];
...
const splitBase64Image = (base64Data: string, includeFlag: boolean, prefix: string, maxChunkSize: number = 45000): string[] => {
  if (!base64Data) return [];
  
  const includePrefix = includeFlag ? '{{Include}}' : '{{Not-Include}}';
  const chunks: string[] = [];
  
  // Add chunk markers to help reconstruct the image
  for (let i = 0; i < base64Data.length; i += maxChunkSize) {
    ...
    const chunkMarker = `${includePrefix}|${chunks.length + 1}|${Math.ceil(base64Data.length / maxChunkSize)}|`;
    chunks.push(chunkMarker + chunk);
  }
...
export const submitFormData = async (data: FormSubmissionData) => {
  ...
  const rawData = {
    ...
    ...splitBase64Image(timelineImage, !!data.globalState.assessments.milestones?.includeTimelineInReport, 'Milestone_Image')
      .reduce((acc, chunk, index) => ({
        ...acc,
        [`Milestone_Image_Chunk_${index + 1}`]: chunk
      }), {}),
    ...
    // Combined Graph
    ...splitBase64Image(radarChartImage, !!data.globalState.assessments.summary?.includeInReport, 'CombinedGraph_Image')
      .reduce((acc, chunk, index) => ({
        ...acc,
        [`CombinedGraph_Image_Chunk_${index + 1}`]: chunk
      }), {})
  };
