...
function prepareMilestoneData(milestones: any) {
    ...
    placedMilestones.forEach((milestone: any) => {
        ...
        let milestoneText;
        if (milestone.expectedAge !== null && milestone.expectedAge !== undefined && milestone.expectedAge !== 0) {
            ...
            milestoneText = `${milestone.title}: Placed at ${milestone.actualAge}m (Expected: ${milestone.expectedAge}m) - ${status}`;
        } else {
            // For developmental concerns that don't have expected ages
            milestoneText = `${milestone.title}: Placed at ${milestone.actualAge}m`;
        }
