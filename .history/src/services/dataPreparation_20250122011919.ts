...
export function prepareFormDataForSubmission(globalState: GlobalFormState) {
    ...
    console.log('📦 Raw form data:', globalState);
    ...
    // Log the base64 lengths for debugging
    ...
    console.log('📏 Milestone image base64 length:', milestoneTimelineImage.length);
    console.log('📏 Radar chart image base64 length:', radarChartImage.length);

    // Prepare milestone image data
    const milestoneImageData = {
        [SHEETY_HEADERS.IMAGES.milestoneImage]: milestoneTimelineImage,
        [SHEETY_HEADERS.IMAGES.milestoneInclude]: globalState.assessments?.milestones?.includeTimelineInReport ? 'true' : 'false'
    };

    // Prepare radar chart image data
    const radarChartImageData = {
        [SHEETY_HEADERS.IMAGES.radarChartImage]: radarChartImage,
        [SHEETY_HEADERS.IMAGES.radarChartInclude]: globalState.assessments?.summary?.includeInReport ? 'true' : 'false'
    };
