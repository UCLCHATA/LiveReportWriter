...
export const submitFormData = async (data: FormSubmissionData) => {
  ...
  // Only attempt to capture images if they should be included
  if (shouldIncludeTimeline) {
    ...
    if (timelineElement) {
      try {
        ...
        console.log('Timeline image captured successfully, length:', timelineImage.length);
      } catch (error) {
        console.error('Error capturing timeline:', error);
      }
    } else {
  ...
  if (shouldIncludeRadar) {
    ...
    if (chartElement) {
      try {
        ...
        console.log('Radar chart captured successfully, length:', radarChartImage.length);
      } catch (error) {
        console.error('Error capturing radar chart:', error);
      }
    } else {
