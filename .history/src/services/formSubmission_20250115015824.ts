...
export const submitFormData = async (data: FormSubmissionData) => {
  ...
  const rawData = {
    ...
    ...(() => {
      ...
      console.log('Created milestone image chunks:', chunks.map((_, i) => `Milestone_Image_Chunk_${i + 1}`));
    ...
    ...(() => {
      ...
      console.log('Created combined graph chunks:', chunks.map((_, i) => `CombinedGraph_Image_Chunk_${i + 1}`));
      return chunks.reduce((acc, chunk, index) => ({
        ...acc,
