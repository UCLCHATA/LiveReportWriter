...
export const submitFormData = async (data: FormSubmissionData) => {
  ...
  // Capture images first if they should be included
  const [radarChartImage, timelineImage] = await Promise.all([
    shouldIncludeRadar ? captureRadarChart() : Promise.resolve(''),
    shouldIncludeTimeline ? captureTimelineImage() : Promise.resolve('')
  ]);
  ...
  const payload = {
    r3Form: {
      ...stringifiedData,
      // Only include images if their respective toggles are enabled
      milestoneImage: shouldIncludeTimeline ? timelineImage : '',
      combinedGraphImage: shouldIncludeRadar ? radarChartImage : ''
    }
  };
