...
export function prepareFormDataForSubmission(globalState: GlobalFormState) {
    ...
    const clinicianData = prepareClinicianData(clinician, chataId); // Pass CHATA ID
    ...
    const mainSubmissionData = {
        ...
        ...clinicianData, // clinicianData already includes chataId
        ...formDataPrepped,
        ...sensoryData,
        ...socialData,
        ...behaviorData,
    ...
    // Prepare milestone image data if available
    const milestoneImageData = assessments?.milestones ? {
        [SHEETY_HEADERS.IMAGES.milestoneImage]: assessments.milestones.timelineImage || '',
        [SHEETY_HEADERS.IMAGES.milestoneInclude]: assessments.milestones.includeTimelineInReport ? 'true' : 'false'
    } : {
        [SHEETY_HEADERS.IMAGES.milestoneImage]: '',
        [SHEETY_HEADERS.IMAGES.milestoneInclude]: 'false'
    };

    // Log milestone image size if present
    if (assessments?.milestones?.timelineImage) {
        console.log('📏 Milestone image base64 length:', assessments.milestones.timelineImage.length);
    }

    // Prepare radar chart image data if available
    const radarChartImageData = assessments?.summary ? {
        [SHEETY_HEADERS.IMAGES.radarChartImage]: assessments.summary.radarChartImage || '',
        [SHEETY_HEADERS.IMAGES.radarChartInclude]: assessments.summary.includeInReport ? 'true' : 'false'
    } : {
        [SHEETY_HEADERS.IMAGES.radarChartImage]: '',
        [SHEETY_HEADERS.IMAGES.radarChartInclude]: 'false'
    };

    // Log radar chart image size if present
    if (assessments?.summary?.radarChartImage) {
        console.log('📏 Radar chart image base64 length:', assessments.summary.radarChartImage.length);
    }
    ...
    console.log('📦 Prepared submission data:', {
        ...
        hasMilestoneImage: !!assessments?.milestones?.timelineImage,
        hasRadarChartImage: !!assessments?.summary?.radarChartImage
    });

    return {
        mainData: mainSubmissionData,
        milestoneImageData,
        radarChartImageData
    };
} 