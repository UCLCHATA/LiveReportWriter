...
class FormPersistenceService {
  ...
  private cleanupFormsExcept(chataId: string): void {
    ...
    // Keep only the current CHATA ID's form and recent unsubmitted forms
    const cleanedForms = Object.entries(forms).reduce((acc, [id, form]) => {
      if (id === chataId || (!form.isSubmitted && now - form.lastUpdated < maxAge)) {
        acc[id] = form;
      }
      return acc;
    ...
    console.log('Cleaned up forms. Remaining forms:', Object.keys(cleanedForms));
  }
  ...
  private getStoredForms(): Record<string, FormData> {
    try {
      ...
      console.log('Retrieved stored forms:', {
        ...
        hasCurrentForm: this.currentChataId ? !!forms[this.currentChataId] : false
      });
      return forms;
    } catch (error) {
