...
function prepareAssessmentLogData(assessmentLog: Record<string, Assessment> | undefined) {
    ...
    // Filter out only the selected assessments and format them
    const formattedEntries = Object.values(assessmentLog)
        .filter(assessment => assessment.selected)
        .map(assessment => {
            const status = assessment.status || 'pending';
            const dateText = assessment.date ? 
                `Scheduled for ${assessment.date}` : 
                status === 'completed' ? 'Completed' : 'Not scheduled';

            const lines = [
                `Assessment: ${assessment.name} (${assessment.category})`,
                `Date: ${dateText}`,
                `Status: ${status}`,
    ...
    const formattedText = formattedEntries.length > 0 ? 
