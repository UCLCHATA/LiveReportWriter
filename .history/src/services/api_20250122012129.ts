...
function formatFormData(formData: any) {
    ...
    let milestoneImageChunks = { chunk1: '', chunk2: '', chunk3: '' };
    if (formData.milestoneImage?.data) {
        console.log('Processing milestone image data...');
        milestoneImageChunks = chunkImageData(formData.milestoneImage.data);
    }
    ...
    // Process history of concerns data
    const processedHistoryData = formData.historyOfConcerns
        ? { value: formData.historyOfConcerns }
        : { value: '' };

    // Return the data wrapped in r3Form object
    return {
        r3Form: {
            ...formData,
            chataId: formData.chataId || new URLSearchParams(window.location.search).get('chataId'),
            milestoneTimelineData: JSON.stringify(processedMilestoneData),
            historyOfConcerns: JSON.stringify(processedHistoryData),
            milestoneImageChunk1: milestoneImageChunks.chunk1,
            milestoneImageChunk2: milestoneImageChunks.chunk2,
            milestoneImageChunk3: milestoneImageChunks.chunk3,
            status: 'submitted',
...
export async function submitFormData(formData: any): Promise<SheetyResponse> {
    try {
        ...
        // Get CHATA ID from URL if not present in form data
        const urlParams = new URLSearchParams(window.location.search);
        const urlChataId = urlParams.get('chataId');
        formData.chataId = formData.chataId || urlChataId;
        
        if (!formData.chataId) {
            console.error('No CHATA ID found');
