...
function prepareMilestoneData(milestones: any) {
    ...
    if (!milestones?.milestones) {
        ...
        return {
            [SHEETY_HEADERS.MILESTONE.timelineData]: JSON.stringify({ entries: [] }),
    ...
    placedMilestones.forEach((milestone: any) => {
        ...
        // Format milestone entry as an object
        const milestoneEntry = {
            ...
            category: category,
            description: milestone.expectedAge !== null && milestone.expectedAge !== undefined && milestone.expectedAge !== 0
                ? `Placed at ${milestone.actualAge}m (Expected: ${milestone.expectedAge}m) - ${
                    milestone.actualAge === milestone.expectedAge ? 'On Time' :
                    milestone.actualAge < milestone.expectedAge ? `Early by ${milestone.expectedAge - milestone.actualAge} months` :
                    `Delayed by ${milestone.actualAge - milestone.expectedAge} months`
                }`
                : `Placed at ${milestone.actualAge}m`
        };

        groupedMilestones[category].push(milestoneEntry);
    });

    // Create a structured object for JSON
    const milestoneData = {
        categories: Object.entries(groupedMilestones).map(([category, entries]) => ({
            name: category,
            milestones: entries
        })),
        totalPlaced: placedMilestones.length,
        lastUpdated: milestones.lastUpdated || new Date().toISOString()
    };
    
    console.log('📝 Formatted milestone data:', milestoneData);

    return {
        [SHEETY_HEADERS.MILESTONE.timelineData]: JSON.stringify(milestoneData),
        [SHEETY_HEADERS.MILESTONE.historyOfConcerns]: milestones.history || ''
    };
}
