...
function prepareMilestoneData(milestones: any) {
    ...
    let communicationSection = "COMMUNICATION:\n";
    communicationMilestones.forEach((milestone: any) => {
        ...
        communicationSection += `${milestone.name}: Expected ${milestone.expectedAge}m (${status})\n`;
    });
    ...
    motorMilestones.forEach((milestone: any) => {
        ...
        motorSection += `${milestone.name}: Expected ${milestone.expectedAge}m (${status})\n`;
    });
    sections.push(motorSection);
    ...
    socialMilestones.forEach((milestone: any) => {
        ...
        socialSection += `${milestone.name}: Expected ${milestone.expectedAge}m (${status})\n`;
    });
    ...
    concernMilestones.forEach((concern: any) => {
        const status = concern.achievedAge !== null && concern.achievedAge !== undefined
            ...
            : 'Not Present';
        concernsSection += `${concern.name}: ${status}\n`;
    });
    sections.push(concernsSection);

    // Join all sections with double newlines
    const formattedString = sections.join('\n');
