...
export const AssessmentForm: React.FC<AssessmentFormProps> = ({ 
  ...
  const handleStatusChange = async (newStatus: 'draft' | 'submitted') => {
    if (newStatus === 'submitted') {
      ...
      if (!currentGlobalState?.formData) {
        console.error('No form data available');
        return;
      }

      // Show overlay immediately when submission starts
      ...
      try {
        ...
        if (chartRef.current) {
          ...
          updateFormData({ chartImage } as Partial<FormData>);
        }

        setSubmissionProgress(50);

        // Submit form data
        const result = await submitFormData(currentGlobalState);
