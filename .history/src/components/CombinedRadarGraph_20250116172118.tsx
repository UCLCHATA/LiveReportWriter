...
export const CombinedRadarGraph: React.FC<CombinedRadarGraphProps> = ({
  ...
  const processData = (): DataPoint[] => {
    ...
    const result = Array.from(allDomains).map(domain => ({
      ...dataPoints[domain],
      sensitivity: typeof dataPoints[domain].sensitivity === 'number' ? dataPoints[domain].sensitivity : 1,
      social: typeof dataPoints[domain].social === 'number' ? dataPoints[domain].social : 1,
      behavior: typeof dataPoints[domain].behavior === 'number' ? dataPoints[domain].behavior : 1
    }));
