...
export const AssessmentForm: React.FC<AssessmentFormProps> = ({ 
  ...
  const [validationItems, setValidationItems] = useState<Array<{ id: string; label: string; completed: boolean }>>([]);
  ...
  // Function to check text area word count and determine status
  const validateTextArea = (text: string): TextAreaValidation => {
  ...
  const checkComponentsCompletion = () => {
    // Add your logic to check if all assessment components are complete
    return Object.values(globalState?.assessments || {}).every(
  ...
  const validateForm = () => {
    const clinicalValidation = validateTextArea(globalState?.formData?.clinicalObservations || '');
    const strengthsValidation = validateTextArea(globalState?.formData?.strengths || '');
    const priorityValidation = validateTextArea(globalState?.formData?.priorityAreas || '');
    const recommendationsValidation = validateTextArea(globalState?.formData?.recommendations || '');
  ...
  const handleSubmitClick = () => {
    ...
    if (!isValid) {
      ...
      handleSubmit(); // Your existing submit function
    }
  };
