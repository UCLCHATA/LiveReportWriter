...
export const AssessmentForm: React.FC<AssessmentFormProps> = ({ onClear, onProgressUpdate }) => {
  ...
  // Update word counts whenever text fields change
  useEffect(() => {
    const newWordCounts: Record<string, WordCountState> = {};
    const textFields = {
    ...
    Object.entries(textFields).forEach(([field, text]) => {
      ...
      if (wordCount >= 180) status = 'over';
      else if (wordCount >= 120) status = 'good';
