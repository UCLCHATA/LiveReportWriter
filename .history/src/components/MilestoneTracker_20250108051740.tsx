...
const MonthMarker: React.FC<{ month: number }> = ({ month }) => {
  const { setNodeRef } = useDroppable({
    id: `month-${month}`,
  });

  return (
    <div ref={setNodeRef} className={styles.monthMarker}>
      <div className={styles.markerLine} />
      <span className={styles.markerLabel}>{month}</span>
    </div>
  );
};
...
const MilestoneTracker: React.FC<Props> = ({
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    handleDrop(draggedMilestone, month);
  };
