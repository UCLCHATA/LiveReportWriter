...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = React.memo(({
  ...
  const handleToggleInclude = useCallback(async () => {
    ...
    try {
      if (newIncludeState) {
        ...
        // When toggling off, preserve the existing image data
        const existingData = globalState.assessments.summary;
        const summaryData: AssessmentSummaryData = {
          type: 'summary',
          includeInReport: false,
          radarChartImage: existingData?.radarChartImage,
          lastUpdated: new Date().toISOString()
        };
        updateAssessment('summary', summaryData);
      }
  ...
  }, [includeInReport, updateAssessment, globalState.assessments.summary]);
