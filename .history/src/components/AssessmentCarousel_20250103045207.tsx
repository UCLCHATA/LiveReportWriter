...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = ({ onProgressUpdate }) => {
  ...
  const handleMarkComplete = (toolId: string) => {
    const button = document.querySelector(`.${styles.completeButton}`);
    if (button) {
      button.classList.add(styles.cut);
      setTimeout(() => {
    ...
    return (
      <div className={styles.completionControls}>
        <div className={styles.container}>
          ...
          <div className={styles.overlay} onClick={() => handleMarkComplete(currentTool.id)} />
        </div>
      </div>
    );
  };
