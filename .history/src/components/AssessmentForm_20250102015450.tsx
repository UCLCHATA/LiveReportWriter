...
export const AssessmentForm: React.FC<AssessmentFormProps> = ({ onClear, onProgressUpdate }) => {
  ...
  // Calculate form progress whenever relevant fields change
  useEffect(() => {
    let progress = 0;
    ...
    const textFields = [
      globalState.clinicalObservations,
      globalState.strengths,
      globalState.supportAreas,
      globalState.recommendations
    ];
    const textProgress = textFields.reduce((acc, field) => {
      return acc + (field && field.trim().length > 0 ? 10 : 0);
    }, 0);

    // Status contributes 5%
    const statusProgress = globalState.ascStatus ? 2.5 : 0;
    const adhdProgress = globalState.adhdStatus ? 2.5 : 0;

    // Referrals contribute 5% (optional)
    const referralsProgress = globalState.referrals.length > 0 ? 5 : 0;

    progress = textProgress + statusProgress + adhdProgress + referralsProgress;
    ...
    globalState.supportAreas,
    globalState.recommendations,
    globalState.ascStatus,
    globalState.adhdStatus,
    globalState.referrals,
    onProgressUpdate
  ]);
