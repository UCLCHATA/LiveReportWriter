...
export const MilestoneTracker: React.FC<{ 
  ...
  useEffect(() => {
    const updateData = async () => {
      console.log('MilestoneTracker updateData called:', { 
        includeInReport, 
      ...
      if (includeInReport) {
        ...
        const formattedMilestones = milestones
          ...
          .map((m: Milestone) => {
            console.log('Processing milestone:', m);
            const difference = m.actualAge ? m.actualAge - m.expectedAge : null;
            let status = 'Not Assessed';
            if (difference !== null) {
              if (difference > 0) status = `Delayed by ${difference} months`;
              else if (difference < 0) status = `Advanced by ${Math.abs(difference)} months`;
              else status = 'On Track';
            }

            return {
              title: m.title,
              description: m.category === 'concerns' ? 'Custom milestone' : `Expected at ${m.expectedAge} months`,
              expectedAge: m.expectedAge,
              actualAge: m.actualAge || 0,
              status,
              isCustom: m.category === 'concerns'
            };
          });
        ...
        console.log('Clearing form data:', updatedData);
        onChange(updatedData);
      }
    };
