...
export const BehavioralAssessment: React.FC = () => {
  ...
  return (
    <div className={carouselStyles.carouselContainer}>
      <header className={carouselStyles.carouselHeader}>
        <button className={carouselStyles.navButton}>
          <span>←</span>
        </button>
        
        <div className={carouselStyles.titleSection}>
          <h2 className={carouselStyles.title}>Behavioral Assessment</h2>
        ...
        <div className={carouselStyles.carouselIndicators}>
          {[1, 2, 3].map((_, index) => (
            <div 
              key={index} 
              className={`${carouselStyles.indicator} ${index === 0 ? carouselStyles.indicatorActive : ''}`}
            />
          ))}
        </div>
