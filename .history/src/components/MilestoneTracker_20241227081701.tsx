...
export const MilestoneTracker: React.FC = () => {
  ...
  const handleDragMove = (event: DragMoveEvent) => {
    ...
    const relativeX = (timelineX - timelineRect.left) / timelineRect.width;
    const currentMonth = Math.round(Math.max(0, Math.min(maxAge, maxAge * relativeX)));

    // Update position indicator
    const indicator = document.querySelector(`.${styles.currentAgeIndicator}`);
    if (indicator) {
      indicator.style.left = `${(currentMonth / maxAge) * 100}%`;
      indicator.textContent = `${currentMonth}m`;
    }

    // Show expected age line
    const expectedLine = document.querySelector(`.${styles.expectedAgeIndicator}`);
    if (expectedLine) {
      expectedLine.style.left = `${(activeMilestone.expectedAge / maxAge) * 100}%`;
    }
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const timelineX = mouseX + timelineRect.left;
    const relativeX = (timelineX - timelineRect.left) / timelineRect.width;
    const month = Math.round(Math.max(0, Math.min(maxAge, maxAge * relativeX)));

    // Update placed milestones
    setPlacedMilestones(prev => {
      ...
      Object.keys(newPlaced).forEach(key => {
        ...
        newPlaced[monthKey] = newPlaced[monthKey]?.filter(m => m.id !== event.active.id) || [];
        if (newPlaced[monthKey]?.length === 0) delete newPlaced[monthKey];
      });

      // Add to new position
      if (!newPlaced[month]) newPlaced[month] = [];
      newPlaced[month].push({
        ...activeMilestone,
        actualAge: month
      });

      // Sort milestones at this position by category
      newPlaced[month].sort((a, b) => a.category.localeCompare(b.category));
      ...
      <div className={styles.milestoneTrackerContainer}>
        ...
        <Droppable id="timeline">
          <div className={styles.timeline}>
            {/* Month markers */}
            {Array.from({ length: maxAge / 6 + 1 }, (_, i) => i * 6).map(month => (
              <div
                key={month}
            ...
            {/* Placed milestones */}
            {Object.entries(placedMilestones).map(([month, milestones]) => (
              <div
                key={month}
                className={styles.milestoneStack}
                style={{ left: `${(parseInt(month) / maxAge) * 100}%` }}
              >
                  ...
                  <Draggable key={milestone.id} id={milestone.id}>
                    <div
                      ...
                      style={{
                        ...
                        transform: `translate(-50%, ${-120 + index * 60}%)`
                      }}
                      onMouseEnter={() => setHoveredMilestone(milestone.id)}
                      onMouseLeave={() => setHoveredMilestone(null)}
                    >
