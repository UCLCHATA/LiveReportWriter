...
interface ComponentProps {
  ...
  onUpdate: (updates: Partial<AssessmentData[keyof AssessmentData]>) => void;
  isComplete: boolean;
  onMarkComplete: () => void;
...
type ComponentIndex = 0 | 1 | 2 | 3 | 4 | 5;
type ComponentName = 'sensory' | 'social' | 'behavior' | 'milestone' | 'assessment' | 'summary';

const componentNames: Record<ComponentIndex, ComponentName> = {
  0: 'sensory',
  1: 'social',
  2: 'behavior',
  3: 'milestone',
...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = ({
  ...
  const [currentIndex, setCurrentIndex] = useState<ComponentIndex>(0);
  ...
  const handleNext = () => {
    setCurrentIndex((prev) => ((prev + 1) % 6) as ComponentIndex);
  };

  const handlePrevious = () => {
    setCurrentIndex((prev) => (prev === 0 ? 5 : prev - 1) as ComponentIndex);
  };

  const handleGoToIndex = (index: number) => {
    setCurrentIndex(index as ComponentIndex);
  };
    ...
    <div className={styles.container}>
      <div className={styles.carouselContainer}>
        <div 
          ...
          data-component={componentNames[currentIndex]}
        >
        ...
        >
          <CurrentComponent
            ...
            onUpdate={(updates: Partial<AssessmentData[keyof AssessmentData]>) => 
              handleUpdateAssessment(currentTool.id, updates)
            }
            isComplete={completionStates[currentTool.id]?.isComplete}
            onMarkComplete={() => handleMarkComplete(currentTool.id)}
            onMarkIncomplete={() => handleMarkIncomplete(currentTool.id)}
          />
        </div>
      </div>
    </div>
