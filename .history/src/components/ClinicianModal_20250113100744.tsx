...
export const ClinicianModal: React.FC<ClinicianModalProps> = ({
  ...
  const [info, setInfo] = useState<ClinicianInfo>(() => ({
    name: initialInfo?.name || '',
    email: initialInfo?.email || '',
    clinicName: initialInfo?.clinicName || '',
    childFirstName: initialInfo?.childFirstName || '',
    childLastName: initialInfo?.childLastName || '',
    childAge: initialInfo?.childAge || '',
    childGender: initialInfo?.childGender || '',
    chataId: initialInfo?.chataId || ''
  }));
  ...
  const createNewForm = () => {
    ...
    const newChataId = generateChataId(info.name, info.childName);
    ...
    existingForms.forEach(form => {
      if (form.clinicianInfo.email === info.email) {
    ...
    onSubmit({ ...info, chataId: newChataId });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      onSubmit({
        ...info,
        childName: `${info.childFirstName} ${info.childLastName}`.trim()
      });
    }
  };
  ...
  const validateForm = () => {
    return info.name && info.email && info.clinicName && info.childFirstName && info.childLastName && info.childAge && info.childGender;
  };
              ...
              <div className={styles.formRow}>
                ...
                <div className={`${styles.formGroup} ${styles.compactField} ${styles.clinicField}`}>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setInfo((prev) => ({ ...prev, clinicName: e.target.value }))
                    }
                    required
                  />
                </div>
              ...
              <div className={styles.formRow}>
                <div className={`${styles.formGroup} ${styles.compactField}`}>
                  ...
                  <input
                    id="child-first-name"
                    type="text"
                    value={info.childFirstName}
                    onChange={(e) =>
                      setInfo((prev) => ({ ...prev, childFirstName: e.target.value }))
                    }
                    required
                  />
                </div>
                <div className={`${styles.formGroup} ${styles.compactField}`}>
                  ...
                  <input
                    id="child-last-name"
                    type="text"
                    value={info.childLastName}
                    onChange={(e) =>
                      setInfo((prev) => ({ ...prev, childLastName: e.target.value }))
                    }
                    required
                  />
                </div>
                <div className={`${styles.formGroup} ${styles.compactField} ${styles.ageField}`}>
                  ...
                  <Select
                    inputId="child-age"
                    options={ageOptions}
                    value={ageOptions.find(opt => opt.value === info.childAge)}
                    onChange={(selected) =>
                      setInfo((prev) => ({ 
                        ...prev, 
                ...
                <div className={`${styles.formGroup} ${styles.compactField} ${styles.genderField}`}>
                  ...
                  <select
                    id="child-gender"
                    value={info.childGender}
                    onChange={(e) =>
                      setInfo((prev) => ({ ...prev, childGender: e.target.value }))
                    }
                  >
