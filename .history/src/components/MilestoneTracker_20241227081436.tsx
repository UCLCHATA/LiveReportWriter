...
export const MilestoneTracker: React.FC = () => {
  ...
  const handleDragMove = (event: DragMoveEvent) => {
    ...
    if (event.over.id === 'timeline') {
      ...
      const mouseX = event.delta.x + timelineRect.left;
      // Calculate relative position within timeline
      const relativeX = (mouseX - timelineRect.left) / timelineRect.width;
      const age = Math.max(0, Math.min(maxAge, maxAge * relativeX));
      ...
      if (currentAge) {
        const position = Math.round(age);
        currentAge.style.left = `${(position / maxAge) * 100}%`;
        currentAge.textContent = `${position}m`;
      }
    }
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const mouseX = event.delta.x + timelineRect.left;
    const relativeX = (mouseX - timelineRect.left) / timelineRect.width;
    const droppedAge = Math.round(Math.max(0, Math.min(maxAge, maxAge * relativeX)));

    // Find milestones at similar positions for stacking
    const similarPositionMilestones = milestones.filter(m => 
      m.actualAge !== undefined && 
      Math.abs((m.actualAge || 0) - droppedAge) <= 2
    );

    // Calculate vertical offset based on existing milestones
    const verticalOffset = similarPositionMilestones.length;

    setMilestones(prev => prev.map(m => {
      if (m.id === event.active.id) {
        return {
          ...m,
          actualAge: droppedAge,
