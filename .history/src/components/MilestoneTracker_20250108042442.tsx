...
type DataOnlyCategory = 'language' | 'cognitive';
type DataCategoryType = CategoryType | DataOnlyCategory;
...
interface MilestoneWithUICategory extends Omit<Milestone, 'category'> {
...
const categoryInfo: CategoryInfoMap = {
  communication: {
    ...
    tooltips: {
      ...
      'Two-Word Phrases': 'Puts two or more words together meaningfully',
  ...
  social: {
    ...
    tooltips: {
      ...
      'Joint Attention': 'Shares attention and interest with others',
...
// Category mapping from data categories to UI categories
...
// Helper function to safely map categories
const mapCategory = (category: DataCategoryType): CategoryType => {
  if (category === 'language') return 'communication';
  if (category === 'cognitive') return 'social';
  return category;
};

// Convert milestone to use UI category
const convertToUIMilestone = (milestone: Milestone): MilestoneWithUICategory => ({
  ...milestone,
  category: mapCategory(milestone.category)
});
...
const DraggableMilestone: React.FC<{ 
  milestone: MilestoneWithUICategory;
  category: CategoryType;
...
export const MilestoneTracker: React.FC<{ data: any; onChange: (data: any) => void }> = ({ data, onChange }) => {
  const [activeMilestone, setActiveMilestone] = useState<MilestoneWithUICategory | null>(null);
  ...
  useEffect(() => {
    console.log('Data initialization check:', { data, hasData: !!data, hasMilestones: !!data?.milestones });
    if (!data || !data.milestones || data.milestones.length === 0) {
      console.log('Initializing with default milestones:', initialMilestones);
      onChange({
  ...
  }, [milestones, historyText, onChange]);
  ...
  }, [milestones, historyText, customConcernCount, onChange]);

  const handleDragStart = useCallback((event: DragEndEvent) => {
    const draggedMilestone = milestones.find((m: Milestone) => m.id === event.active.id);
    setActiveMilestone(draggedMilestone ? convertToUIMilestone(draggedMilestone) : null);
  }, [milestones]);
        ...
        <div className={styles.categoriesContainer}>
          {(Object.entries(categoryInfo) as [CategoryType, CategoryInfo][]).map(([category, info]) => {
            const categoryMilestones = milestones
              .filter((m: Milestone) => mapCategory(m.category) === category && !m.actualAge)
              .map(convertToUIMilestone);
            ...
            {activeMilestone && (
              <div 
                ...
                style={{ 
                  ...
                  backgroundColor: categoryInfo[mapCategory(activeMilestone.category)].color
                }}
              >
