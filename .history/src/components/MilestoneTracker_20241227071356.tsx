...
export const MilestoneTracker: React.FC = () => {
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    const droppedAge = Math.min(maxAge, parseInt(result.destination.index.toString()));
                            ...
                            >
                              <div className={styles.milestoneContent}>
                                <div className={styles.milestoneHeader}>
                                  ...
                                  <small>Expected: {milestone.expectedAge}m</small>
                                </div>
                                <div className={styles.milestoneDetails}>
                                  <p>{milestone.description}</p>
                                  <small className={styles.clinicalNotes}>{milestone.clinicalNotes}</small>
                                </div>
                              </div>
                            </div>
                          )}
          ...
          <div className={styles.expectedTimeline}>
            {milestones.map(milestone => (
              <div
                ...
                style={{
                  left: `${(milestone.expectedAge / maxAge) * 100}%`,
                  backgroundColor: categoryInfo[milestone.category].color + '40'
                }}
                title={`Expected: ${milestone.title} at ${milestone.expectedAge}m`}
              />
            ))}
          </div>
