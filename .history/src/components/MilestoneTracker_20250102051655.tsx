...
export const MilestoneTracker: React.FC = () => {
  ...
  const updateMilestoneProgress = useCallback(() => {
    const placedCount = milestones.filter((m: Milestone) => m.actualAge !== undefined).length;
    const progress = Math.min((placedCount / milestones.length) * 100, 10);
    
    console.log('Calculating progress:', {
      placed: placedCount,
      total: milestones.length,
      progress,
      milestones: milestones.map((m: Milestone) => ({id: m.id, placed: !!m.actualAge}))
    });
  ...
  // Calculate initial progress on mount and when milestones change
  useEffect(() => {
    updateMilestoneProgress();
  }, [updateMilestoneProgress]);

  const handleDeleteMilestone = useCallback((milestoneId: string) => {
    console.log('Attempting to delete milestone:', milestoneId);
    
    // Find the milestone to reset
    const milestoneToReset = milestones.find((m: Milestone) => m.id === milestoneId);
    if (!milestoneToReset) return;
    ...
    console.log('Resetting milestone:', {
      before: milestoneToReset,
      after: updatedMilestones.find((m: Milestone) => m.id === milestoneId)
    });
  ...
  }, [milestones, updateAssessment, updateMilestoneProgress]);
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    console.log('Placing milestone at month:', month);
