...
export const MilestoneTracker: React.FC<{ data: any; onChange: (data: any) => void }> = ({ data, onChange }) => {
  ...
  const isOverlapping = (milestone: Milestone, otherMilestones: Milestone[]): boolean => {
    ...
    return otherMilestones.some(other => {
      ...
      // Check if milestones are close enough to overlap horizontally
      const distance = Math.abs(milestone.actualAge - other.actualAge);
      const isClose = distance <= 6; // Slightly reduced from 8 to better match actual bubble width
      
      // Only check vertical overlap if they're horizontally close
      if (isClose) {
        ...
        // 3. They're horizontally close enough to overlap
        return thisStack < otherStack && 
               Math.abs(thisStack - otherStack) === 1 && 
               distance <= 6;
      }
      return false;
    });
                    ...
                    return (
                      <motion.div
                        ...
                        style={{
                          ...
                          opacity: hasOverlap ? 0.6 : 1
                        }}
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: hasOverlap ? 0.6 : 1, y: 0 }}
                        exit={{ opacity: 0, y: -20 }}
                      >
                        <DraggableMilestone
                          milestone={milestone}
