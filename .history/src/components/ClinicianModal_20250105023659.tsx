...
export const ClinicianModal: React.FC<ClinicianModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (clinicianInfo.name && clinicianInfo.email && clinicianInfo.clinicName) {
      ...
      const chataId = generateChataId(clinicianInfo.name, clinicianInfo.clinicName);
  ...
  const chataIdDialog = showChataIdInfo && (
    <div className={styles.blurryOverlay}>
      <div className={styles.dialogContent}>
        ...
        <p>
          Please write down your CHATA ID to retrieve your form later if needed:
        </p>
        <div className={styles.chataIdDisplay}>
          <span className={styles.blinkingId}>
            {generateChataId(clinicianInfo.name, clinicianInfo.clinicName)}
          </span>
        </div>
        <p className={styles.chataIdNote}>
          Your CHATA ID is built from:<br />
          • Your initials ({clinicianInfo.name.split(' ').map(n => n[0]).join('')})<br />
          • Clinic name ({clinicianInfo.clinicName.substring(0, 2).toUpperCase()})<br />
          • Unique number
        </p>
