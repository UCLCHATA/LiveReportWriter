...
export const AssessmentForm: React.FC<AssessmentFormProps> = ({ 
  ...
  const [submissionStage, setSubmissionStage] = useState<Stage>('submission');
  ...
  const handleStatusChange = async (newStatus: 'draft' | 'submitted') => {
    if (newStatus === 'submitted') {
      try {
        ...
        // Capture the combined radar chart
        const chartContainer = document.querySelector('.combined-radar-chart');
        let chartImage = null;
        if (chartContainer) {
          const canvas = await html2canvas(chartContainer);
          const base64Image = canvas.toDataURL('image/png');
          
          // Split base64 image into chunks if needed
          const chunkSize = 32000; // Adjust based on Sheety's limits
          const imageData = base64Image.split(',')[1];
          chartImage = {
            chunk1: imageData.slice(0, chunkSize) || '',
            chunk2: imageData.slice(chunkSize, 2 * chunkSize) || '',
            chunk3: imageData.slice(2 * chunkSize) || ''
          };
        }
        ...
        const formDataToSubmit = {
          ...
          milestoneTimelineData: JSON.stringify(globalState.milestoneTimeline || []),
          historyOfConcerns: globalState.historyOfConcerns || '',
          assessmentLogData: JSON.stringify(globalState.assessmentLog || [])
        };
