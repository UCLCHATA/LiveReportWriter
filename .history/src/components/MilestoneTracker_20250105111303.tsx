...
export const MilestoneTracker: React.FC<{ onChange?: (data: any) => void }> = ({ onChange }) => {
  ...
  // Get current state from globalState or use initial values
  const currentState = globalState.assessments?.milestones || {
    type: 'milestoneTracker',
    milestones: initialMilestones,
    history: ''
  };

  // Update both assessment and parent on any change
  ...
  }, [currentState, updateAssessment, onChange]);

  const handleHistoryChange = useCallback((text: string) => {
    updateState({ history: text });
  }, [updateState]);
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    const draggedMilestone = currentState.milestones.find((m: Milestone) => m.id === active.id);
    if (!draggedMilestone) return;

    const status = determineStatus(month, draggedMilestone.expectedAge);
    
    const updatedMilestones = currentState.milestones.map((m: Milestone) => 
      m.id === active.id ? {
        ...m,
        actualAge: month,
        stackPosition: getStackPosition(currentState.milestones, month, active.id.toString()),
        status
      } : m
    );
  ...
  const handleDragStart = useCallback((event: DragEndEvent) => {
    const draggedMilestone = currentState.milestones.find((m: Milestone) => m.id === event.active.id);
  ...
  const handleCustomConcern = useCallback((text: string) => {
    ...
    const updatedMilestones = [...currentState.milestones, newMilestone];
    updateState({ milestones: updatedMilestones });
    setCustomConcernCount(prev => prev + 1);
  }, [currentState, customConcernCount, updateState]);
