...
export const MilestoneTracker: React.FC<MilestoneTrackerProps> = React.memo(({ chataId, data, onChange }) => {
  ...
  const [activeMilestone, setActiveMilestone] = useState<Milestone | null>(data.activeMilestone);
  const [currentMonth, setCurrentMonth] = useState<number | null>(data.currentMonth);
