  ...
  const renderModal = () => {
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
      if (e.key === 'Enter') {
        ...
        if (bulletMatch && lastLine.trim() === bulletMatch[0].trim()) {
          const newValue = textBeforeCursor.substring(0, cursorPosition - lastLine.length) + textAfterCursor;
          handleModalContentChange({ target: { value: newValue } } as React.ChangeEvent<HTMLTextAreaElement>);
          target.selectionStart = cursorPosition - lastLine.length;
          target.selectionEnd = cursorPosition - lastLine.length;
        } else {
          const bullet = bulletMatch ? bulletMatch[0] : '• ';
      ...
      <div className={styles.modalOverlay} onClick={handleModalClose}>
        <div 
          ...
          style={{ 
            ...
            backgroundColor: `${getSectionColor()}05`
          }}
        >
          ...
          <textarea
            ...
            autoFocus
          />
          <div className={`${styles.modalLengthIndicator} ${styles[getTextLengthStatus(modalContent.value).status]}`}>
            {getTextLengthStatus(modalContent.value).message}
          </div>
        </div>
      </div>
    );
