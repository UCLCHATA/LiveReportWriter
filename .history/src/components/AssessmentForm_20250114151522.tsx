...
import TextAreaWithOverlay from './TextAreaWithOverlay';
  ...
  const handleStatusChange = async (newStatus: 'draft' | 'submitted') => {
    if (newStatus === 'submitted') {
      ...
      try {
        ...
        // Prepare form data for Sheety API
        const sheetyData = {
          r3Form: {
            ...
            oralScore: globalState.assessments.sensoryProfile.domains.oral?.value || '',
            oralObservations: globalState.assessments.sensoryProfile.domains.oral?.observations || '',
            
            // Social Communication Scores and Observations
            jointAttentionScore: globalState.assessments.socialCommunication.domains.jointAttention?.value || '',
            jointAttentionObservations: globalState.assessments.socialCommunication.domains.jointAttention?.observations || '',
            ...
            verbalCommunicationScore: globalState.assessments.socialCommunication.domains.verbalCommunication?.value || '',
            verbalCommunicationObservations: globalState.assessments.socialCommunication.domains.verbalCommunication?.observations || '',
            socialUnderstandingScore: globalState.assessments.socialCommunication.domains.socialUnderstanding?.value || '',
            socialUnderstandingObservations: globalState.assessments.socialCommunication.domains.socialUnderstanding?.observations || '',
            ...
            milestoneTimelineData: JSON.stringify(formData.milestones?.map(milestone => ({
              title: milestone.title,
              description: milestone.description,
              expectedMonth: milestone.isCustom ? 'NA' : milestone.expectedAge,
              actualMonth: milestone.actualAge,
              status: milestone.actualAge !== undefined && !milestone.isCustom
            ...
            selectedAssessments: globalState.assessments.assessmentLog.selectedAssessments
              .map(id => globalState.assessments.assessmentLog.entries[id]?.name || '')
              .join(', '),
            assessmentDates: globalState.assessments.assessmentLog.selectedAssessments
              .map(id => globalState.assessments.assessmentLog.entries[id]?.date || '')
              .join(', '),
            assessmentNotes: globalState.assessments.assessmentLog.selectedAssessments
              .map(id => globalState.assessments.assessmentLog.entries[id]?.notes || '')
              .join(' | '),
        ...
        console.log('Attempting to submit form data to Sheety API...', {
          ...
          r3Form: {
            ...sheetyData.r3Form,
            combinedRadarChart: radarChartImage ? '[BASE64_IMAGE]' : '',
            milestoneImage: formData.timelineImage ? '[BASE64_IMAGE]' : '',
            combinedGraphImage: radarChartImage ? '[BASE64_IMAGE]' : ''
          }
        });

        // Submit to Sheety API with retry logic
        const sheetyResponse = await submitToSheetyAPI(R3_FORM_API, sheetyData);
