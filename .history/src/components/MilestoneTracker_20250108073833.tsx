...
const calculateComponentProgress = (milestones: Milestone[], customMilestones: Milestone[]): number => {
  const totalMilestones = milestones.length + customMilestones.length;
  if (totalMilestones === 0) return 0;
  
  const placedMilestones = [...milestones, ...customMilestones].filter(
    m => typeof m.actualAge === 'number' && m.actualAge > 0
  ).length;
  
  return Math.min((placedMilestones / totalMilestones) * 100, 100);
};

const calculateFormContribution = (componentProgress: number): number => {
  // Each component can contribute up to 10% to the total form progress
  return Math.min((componentProgress / 100) * 10, 10);
};

const MAX_AGE = 72; // Maximum age in months

export const MilestoneTracker: React.FC<ComponentProps> = ({ data, onChange }) => {
  ...
  const [historyText, setHistoryText] = useState(data?.history || '');

  // Update progress whenever milestones change
  useEffect(() => {
    const componentProgress = calculateComponentProgress(milestones, customMilestones);
    const formContribution = calculateFormContribution(componentProgress);
    ...
    onChange({
      ...
      formProgress: formContribution,
      isComplete: componentProgress === 100,
      history: historyText
    });
  }, [milestones, customMilestones, onChange, historyText]);

  const handleDragStart = (event: DragEndEvent) => {
    const draggedMilestone = milestones.find(m => m.id === event.active.id) ||
                            customMilestones.find(m => m.id === event.active.id);
    setActiveMilestone(draggedMilestone || null);
  };

  const handleDragMove = (event: DragEndEvent) => {
    if (!event.over) return;
    const monthMatch = event.over.id.toString().match(/month-(\d+)/);
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const milestone = milestones.find(m => m.id === active.id) || 
                     customMilestones.find(m => m.id === active.id);

    if (milestone) {
      if (milestone.id.startsWith('custom-')) {
        setCustomMilestones(prev => prev.map(m => 
          m.id === milestone.id ? { ...m, actualAge: month } : m
        ));
      } else {
  ...
  const handleCustomConcern = (text: string) => {
    ...
    <div className={styles.container}>
      ...
      >
        <div className={styles.timeline}>
          {/* Timeline markers */}
          {Array.from({ length: MAX_AGE + 1 }, (_, i) => (
            <TimelineMonth key={i} month={i} maxAge={MAX_AGE} />
          ))}
          ...
          <div className={styles.milestonesContainer}>
            {Object.entries(categoryInfo).map(([category, info]) => (
              <div key={category} className={styles.categorySection}>
                <h3 className={styles.categoryTitle} style={{ color: info.color }}>
                  {info.title}
                </h3>
      ...
      <div className={styles.historySection}>
        <textarea
          ...
          placeholder="Add developmental history notes..."
          className={styles.historyTextarea}
        />
      </div>
    </div>
