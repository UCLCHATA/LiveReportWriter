...
export const MilestoneTracker: React.FC = () => {
  ...
  const maxAge = 72; // Maximum age in months
  const [placedMilestones, setPlacedMilestones] = useState<{
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    // Show expected timeline immediately
    const expectedLine = document.querySelector(`.${styles.expectedAgeIndicator}`);
    if (expectedLine && draggedMilestone) {
      expectedLine.style.left = `${(draggedMilestone.expectedAge / maxAge) * 100}%`;
      expectedLine.style.opacity = '1';
    }
  };
  ...
  const handleDragMove = (event: DragMoveEvent) => {
    ...
    if (event.over.id === 'timeline') {
      ...
      // Get exact position on timeline
      const timelineX = event.clientX - timelineRect.left;
      const month = Math.round(Math.max(0, Math.min(maxAge, (timelineX / timelineRect.width) * maxAge)));

      // Update current position indicator
      const currentAge = document.querySelector(`.${styles.currentAgeIndicator}`);
      if (currentAge) {
        currentAge.style.left = `${(month / maxAge) * 100}%`;
        currentAge.textContent = month >= 12 ? `${Math.floor(month/12)}y${month%12}m` : `${month}m`;
        currentAge.style.opacity = '1';
      }
    }
  };
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    // Use the same calculation as dragMove
    const mouseX = event.delta.x + event.active.rect.current.left;
    const relativeX = (mouseX - timelineRect.left);
    const month = Math.round(Math.max(0, Math.min(maxAge, (relativeX / timelineRect.width) * maxAge)));
        ...
        <Droppable id="timeline">
          <div className={styles.timeline}>
            ...
            {milestones
              ...
              .sort((a, b) => {
                // First sort by age
                if (a.actualAge !== b.actualAge) {
                  return (a.actualAge || 0) - (b.actualAge || 0);
                }
                // Then by category
              ...
              .map((milestone, index, array) => {
                ...
                const sameAgeCount = array.filter(m => m.actualAge === milestone.actualAge).length;
                const sameAgeIndex = array.filter(
                  m => m.actualAge === milestone.actualAge && m.id <= milestone.id
                ).length - 1;
                  ...
                  <Draggable key={milestone.id} id={milestone.id}>
                    <div
                      ...
                      style={{
                        ...
                        transform: `translate(-50%, ${-60 * (sameAgeCount - 1) + 120 * sameAgeIndex}%)`,
                        borderLeftColor: categoryInfo[milestone.category].color,
            ...
            {activeMilestone && (
              <>
                <div className={styles.expectedAgeIndicator} />
