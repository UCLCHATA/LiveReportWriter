...
export const AssessmentLogger: React.FC = () => {
  ...
  // Get the assessment log state from global state
  const assessmentLog = globalState.assessments?.assessmentLog || {
    type: 'assessmentLog',
    selectedAssessments: [],
    entries: {},
  ...
  useEffect(() => {
    if (!globalState.assessments?.assessmentLog) {
      updateAssessment('assessmentLog', assessmentLog);
    }
  }, []);

  // Calculate progress without updating state
  useEffect(() => {
    if (!selectedAssessments || !assessmentEntries) return;
    ...
    // We don't update the state here anymore - the progress is calculated in AssessmentCarousel
  ...
  const handleToggleAssessment = (assessment: Assessment) => {
    ...
    updateAssessment('assessmentLog', newState);
  };
