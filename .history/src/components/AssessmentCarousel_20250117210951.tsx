...
import type { AssessmentData } from '../types/formState';
...
interface ComponentProps {
  ...
  onChange: (data: AssessmentData[keyof AssessmentData]) => void;
}
...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = React.memo(({
  ...
  const handleSensoryProfileChange = (data: AssessmentData['sensoryProfile']) => {
    if (!globalState?.assessments) return;
    globalState.assessments.sensoryProfile = data;
    updateGlobalState(globalState);
  };

  const handleSocialCommunicationChange = (data: AssessmentData['socialCommunication']) => {
    if (!globalState?.assessments) return;
    globalState.assessments.socialCommunication = data;
    updateGlobalState(globalState);
  };

  const handleBehaviorInterestsChange = (data: AssessmentData['behaviorInterests']) => {
    if (!globalState?.assessments) return;
