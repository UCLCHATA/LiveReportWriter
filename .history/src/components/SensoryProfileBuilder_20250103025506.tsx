...
export const SensoryProfileBuilder: React.FC<SensoryProfileBuilderProps> = ({ data, onChange }) => {
  ...
  const handleTooltipPosition = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    // Determine if this is a right side grid item
    const isRightSide = e.currentTarget.closest(`.${styles.domainSlider}`)?.parentElement?.offsetLeft !== 0;

    // Calculate horizontal position
    let left: number;
    if (isRightSide) {
      // For right side items, position to the left of cursor
      left = cursorX - tooltipWidth - 8;
    } else {
      // For left side items, position to the right of cursor
      left = cursorX + 8;
    }

    // Calculate vertical position
    let top = cursorY + 8; // Start by positioning below cursor
    ...
    tooltip.style.transform = 'none'; // Remove any transform that might interfere
  };
