...
export const ChataIdInput: React.FC<ChataIdInputProps> = ({ onChange, onSubmit }) => {
  ...
  const handleSegmentChange = (index: number, value: string) => {
    ...
    // Only combine and send if all segments are valid
    if (newSegments.every(segment => segment.length === 3)) {
      const fullChataId = newSegments.join('-');
      console.log('ChataIdInput: All segments complete:', { segments: newSegments, fullChataId });
      onChange(fullChataId);
      // Auto-submit when all segments are complete
      onSubmit();
    } else {
      // For incomplete CHATA ID, still update the UI
      const fullChataId = newSegments.join('-');
      console.log('ChataIdInput: Partial update:', { segments: newSegments, fullChataId });
      onChange(fullChataId);
    }
