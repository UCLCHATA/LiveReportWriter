...
export const AssessmentLogger: React.FC = () => {
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-sm flex flex-col h-full">
      {/* Fixed height assessment selection area */}
      <div className="p-4 border-b">
        {/* Assessment Grid */}
        <div className="grid grid-cols-4 gap-4">
          {Object.entries(assessmentTools).map(([category, tools]) => (
            <div key={category}>
              <h3 className="text-[9px] font-medium text-gray-400 uppercase mb-3 text-center">
                {category.replace('_', ' ')}
              </h3>
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        {selectedAssessments.length > 0 && (
          <>
            <h3 className="text-[9px] font-medium text-gray-400 uppercase mb-2">
