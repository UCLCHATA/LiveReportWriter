...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = ({ onProgressUpdate }) => {
  ...
  }, [globalState.assessments, updateAssessments]);

  // Safely access assessment data
  const getSafeAssessmentData = (key: keyof AssessmentData) => {
    return globalState.assessments?.[key] || null;
  };

  // Update your render logic to use getSafeAssessmentData
  return (
    <div className={styles.carouselContainer}>
      {/* ... other carousel content ... */}
      <SensoryProfile 
        data={getSafeAssessmentData('sensoryProfile')}
        onUpdate={(data) => updateAssessments({ sensoryProfile: data })}
      />
      {/* ... other components ... */}
    </div>
  );
