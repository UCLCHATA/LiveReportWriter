...
export const SensoryProfileBuilder: React.FC<SensoryProfileBuilderProps> = ({ data, onChange }) => {
  ...
  useEffect(() => {
    if (!data?.domains) {
      onChange({
        domains: {
          visual: { name: 'Visual', value: 3, observations: [], label: 'Typical' },
          auditory: { name: 'Auditory', value: 3, observations: [], label: 'Typical' },
          tactile: { name: 'Tactile', value: 3, observations: [], label: 'Typical' },
          vestibular: { name: 'Vestibular', value: 3, observations: [], label: 'Typical' },
          proprioceptive: { name: 'Proprioceptive', value: 3, observations: [], label: 'Typical' },
          oral: { name: 'Oral', value: 3, observations: [], label: 'Typical' },
        }
      });
    }
  }, [data, onChange]);
  ...
  const handleTooltipPosition = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const sliderSection = document.querySelector(`.${styles.slidersGrid}`) as HTMLElement;
    if (!sliderSection) return;

    const sliderRect = sliderSection.getBoundingClientRect();
    
    // Get cursor position relative to the tooltip wrapper
    const cursorX = e.clientX - rect.left;
    const cursorY = e.clientY - rect.top;

    // Determine if this is a left or right grid item
    const isRightSide = e.currentTarget.closest(`.${styles.domainSlider}`)?.parentElement?.offsetLeft !== 0;

    // Calculate available space
    const spaceBelow = sliderRect.bottom - e.clientY;
    const tooltipHeight = tooltip.offsetHeight;

    if (isRightSide) {
      // For right side items, position tooltip to the left of the cursor
      tooltip.style.right = '100%';
      tooltip.style.left = 'auto';
      tooltip.style.transform = 'translateY(-50%)';
      tooltip.style.marginRight = '8px';
    } else {
      // For left side items, position tooltip to the right of the cursor
