...
import headerStyles from './ComponentHeader.module.css';
...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = ({ onProgressUpdate, initialProgress }) => {
  const { globalState, updateAssessment, setGlobalState } = useFormState();
  ...
  // Initialize progress from prop if provided
  useEffect(() => {
    if (initialProgress !== undefined && !isInitialized) {
      onProgressUpdate(initialProgress);
      setIsInitialized(true);
    }
  }, [initialProgress, isInitialized, onProgressUpdate]);
  ...
  useEffect(() => {
    ...
    const totalProgress = Math.round(
      componentProgresses.reduce((sum, progress) => sum + progress, 0) / tools.length
    );
    ...
    setGlobalState(prev => ({
      ...
      progress: {
        ...prev.progress,
        carousel: totalProgress,
        lastUpdated: Date.now()
      }
    }));
  }, [globalState.assessments, currentIndex, calculateComponentProgress, onProgressUpdate, setGlobalState]);
  ...
  const renderCompletionControls = () => {
    ...
    return (
      <div className={headerStyles.completionControls}>
        <button
          className={headerStyles.completeButton}
        ...
        {state?.progress > 0 && (
          <div className={headerStyles.progressIndicator}>
            <div 
  ...
  const renderCompletionStatus = () => {
    ...
    return (
      <div className={headerStyles.completionStatus}>
        <div className={headerStyles.statusBadge}>
          <CheckCircle2 size={14} />
          <span>Complete</span>
          <button 
            className={headerStyles.undoButton}
  ...
  const renderComponentIcon = () => {
    ...
    return (
      <img 
        src={iconSrc} 
        alt="" 
        className={headerStyles.componentIcon} 
      />
    );
  };
    ...
    <div className={carouselStyles.container}>
      <div className={carouselStyles.carouselContainer}>
        <div className={carouselStyles.carouselHeader}>
          <div className={carouselStyles.titleSection}>
            <h2 className={carouselStyles.title}>
              {renderComponentIcon()}
              {currentTool.title}
              <button className={carouselStyles.toolkitButton}>
                <HelpCircle size={16} />
                <div className={carouselStyles.toolkitTooltip}>
          ...
          <div className={carouselStyles.navigationSection}>
            {renderCompletionControls()}
            {renderCompletionStatus()}
            
            <button onClick={handlePrevious} className={carouselStyles.navButton}>
              <ChevronLeft size={20} />
            </button>
            ...
            <div className={carouselStyles.carouselIndicators}>
              {tools.map((_, index) => (
                <button
                  key={index}
                  className={`${carouselStyles.indicator} ${index === currentIndex ? carouselStyles.active : ''} ${
                    completionStates[tools[index].id]?.isComplete ? carouselStyles.complete : ''
                  }`}
            ...
            <button onClick={handleNext} className={carouselStyles.navButton}>
        ...
        <div className={carouselStyles.carouselContent}>
          {(currentTool.id === 'sensoryProfile' || 
            ...
            <div className={carouselStyles.componentWithGraph}>
              <div className={`${carouselStyles.graphSection} ${completionStates[currentTool.id]?.isComplete ? carouselStyles.completed : ''}`}>
                {currentTool.id === 'sensoryProfile' && (
                  <SensoryProfileGraph
              ...
              <div className={carouselStyles.sliderSection}>
                <div className={carouselStyles.sliderContainer}>
                  ...
                  {completionStates[currentTool.id]?.isComplete && (
                    <div className={carouselStyles.completionOverlay} />
                  )}
                </div>
              </div>
            </div>
          ) : (
            <div className={`${carouselStyles.componentContainer} ${currentTool.id === 'milestones' ? carouselStyles.milestoneTracker : ''}`}>
              <CurrentComponent
                data={globalState.assessments[currentTool.id]}
                onChange={handleComponentUpdate}
              />
