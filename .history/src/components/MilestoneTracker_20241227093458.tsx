...
const getStackPosition = (
  ...
  // Get all placed milestones except the current one
  const placedMilestones = milestones
    .filter(m => m.actualAge !== undefined && m.id !== currentId);

  // Check each row until we find a free spot
  let row = 0;
  let hasOverlap = true;

  while (hasOverlap) {
    hasOverlap = placedMilestones.some(m => {
      // Calculate horizontal distance between milestones in months
      const distance = Math.abs((m.actualAge || 0) - currentMonth);
      
      // Check if they're close enough to overlap (adjust the 3 value to change overlap sensitivity)
      const mightOverlap = distance <= 3;
      
      // Check if the other milestone is on the same row
...
export const MilestoneTracker: React.FC = () => {
  ...
  const handleDragMove = (event: any) => {
    ...
    const month = parseInt(event.over.id.replace('month-', ''));
  ...
  const handleDragEnd = (event: any) => {
    ...
    setMilestones(prev => {
      // First, find the stack position for the new milestone
      const updatedMilestones = [...prev];
      const draggedIndex = updatedMilestones.findIndex(m => m.id === active.id);
      
      if (draggedIndex !== -1) {
        const stackPosition = getStackPosition(updatedMilestones, month, active.id);
