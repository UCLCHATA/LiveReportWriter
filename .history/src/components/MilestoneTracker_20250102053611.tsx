...
export const MilestoneTracker: React.FC = () => {
  ...
  const handleDeleteMilestone = useCallback((milestoneId: string) => {
    ...
    // Create new array with the milestone removed from timeline
    const updatedMilestones = milestones.map((m: Milestone) => {
      if (m.id === milestoneId) {
        // Find the original milestone data
        const originalMilestone = initialMilestones.find(im => im.id === milestoneId);
        if (!originalMilestone) return m;
        
        // Return a fresh copy without timeline-specific properties
        return {
          ...originalMilestone,
          actualAge: undefined,
    ...
    // Update the global state with the milestone removed
