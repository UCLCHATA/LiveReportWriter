...
import { ChevronLeft, ChevronRight, HelpCircle, CheckCircle2, SkipForward, Trophy } from 'lucide-react';
...
const createConfetti = () => {
  ...
  confettiContainer.className = styles.celebrationContainer;
  ...
  for (let i = 0; i < 50; i++) {
    ...
    confetti.style.cssText = `
      ...
      animation: ${styles.confetti} 1s ease-out forwards;
...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = ({ onProgressUpdate }) => {
  ...
  const checkAndCelebrateMilestone = useCallback((progress: number) => {
    const milestones = [25, 50, 75, 100];
    const lastMilestone = Math.floor(milestoneState.lastProgress / 25) * 25;
    const currentMilestone = Math.floor(progress / 25) * 25;

    if (currentMilestone > lastMilestone && milestones.includes(currentMilestone)) {
      setMilestoneState({
      ...
      // Create confetti for 100% completion
  ...
  useEffect(() => {
    ...
    const componentProgresses = tools.map(tool => {
      ...
      return Math.min((state?.progress || 0) / 10, 10);
    });
    ...
    if (!isInitialized) {
      ...
      onProgressUpdate(totalProgress);
      checkAndCelebrateMilestone(totalProgress);
    }
  }, [completionStates, isInitialized, checkAndCelebrateMilestone]);
