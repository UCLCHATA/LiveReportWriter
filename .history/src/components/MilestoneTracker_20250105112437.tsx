...
export const MilestoneTracker: React.FC<{ onChange?: (data: any) => void }> = ({ onChange }) => {
  ...
  const historyOfConcerns = globalState.assessments?.milestones?.historyOfConcerns || '';

  const handleAddCustomConcern = () => {
    if (customConcernText.trim()) {
      const newMilestone: Milestone = {
        id: `custom-${Date.now()}`,
        title: customConcernText,
        category: 'concerns',
        expectedAge: 0,
        isCustom: true
      };
  ...
  }, [milestones, updateAssessment, onChange, historyOfConcerns]);
  ...
  const handleHistoryChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    updateAssessment('milestones', {
      type: 'milestoneTracker',
      milestones,
      historyOfConcerns: e.target.value
    });
  };
    ...
    <div className={styles.container}>
      ...
      >
        <div className={styles.categoriesContainer}>
          {/* History of Concerns text area */}
          <div className={styles.historySection}>
            <h3>History of Concerns</h3>
            <textarea
              className={styles.historyTextarea}
              value={historyOfConcerns}
              onChange={handleHistoryChange}
              placeholder="Enter any history of concerns..."
              rows={4}
            />
          </div>

          {/* Custom concerns section */}
          <div className={styles.category}>
            <h3>Custom Concerns</h3>
            <div className={styles.milestoneList}>
              ...
              {milestones
                .filter((m: Milestone) => m.category === 'concerns' && !m.actualAge)
