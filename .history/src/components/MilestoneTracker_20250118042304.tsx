...
import { setMilestoneImageHeader, clearMilestoneImageHeader } from '../services/api';
...
export const MilestoneTracker: React.FC<{ data: any; onChange: (data: any) => void }> = React.memo(({ data, onChange }) => {
  ...
  const processedMilestones = useMemo(() => {
    return milestones.map((milestone: Milestone) => ({
      ...milestone,
      stackPosition: currentMonth !== null ? getStackPosition(milestones, currentMonth, milestone.id) : 0,
  ...
  const handleCustomConcern = useCallback((text: string) => {
    ...
    const updatedMilestones = [...milestones.filter((m: Milestone) => m.id !== 'custom-concern'), newMilestone, {
      id: 'custom-concern',
      title: '+',
      category: 'concerns',
      expectedAge: 0
  ...
  const handleIncludeInReportToggle = useCallback(async () => {
    console.log('[MilestoneTracker] Include in report toggle clicked, current state:', includeInReport);
    setIsCapturingImage(true);
    setCaptureError(null);

    try {
      if (!includeInReport) {
        console.log('[MilestoneTracker] Starting timeline capture process');
        
        // Prepare timeline for capture
        if (wrapperRef.current) {
          console.log('[MilestoneTracker] Timeline wrapper found, preparing for capture');
          const originalStyles = {
            marginBottom: wrapperRef.current.style.marginBottom,
            height: wrapperRef.current.style.height
          };

          // Set capture styles
          wrapperRef.current.style.marginBottom = '0px';
          wrapperRef.current.style.height = '240px';

          console.log('[MilestoneTracker] Initiating html2canvas capture');
          const canvas = await html2canvas(wrapperRef.current, {
            backgroundColor: null,
            scale: 2,
            logging: true,
            ...
            useCORS: true,
            onclone: (clonedDoc) => {
              console.log('[MilestoneTracker] Cloning document for capture');
              const clonedWrapper = clonedDoc.querySelector(`.${styles.timelineWrapper}`);
              if (clonedWrapper) {
                (clonedWrapper as HTMLElement).style.marginBottom = '0px';
                (clonedWrapper as HTMLElement).style.height = '240px';
                ...
                const markers = clonedWrapper.querySelectorAll(`.${styles.monthMarker}`);
                console.log(`[MilestoneTracker] Processing ${markers.length} month markers`);
                markers.forEach((marker) => {
                  (marker as HTMLElement).style.opacity = '1';
                  const markerLine = marker.querySelector(`.${styles.markerLine}`);
                  if (markerLine) {
          ...
          // Restore original styles
          wrapperRef.current.style.marginBottom = originalStyles.marginBottom;
          wrapperRef.current.style.height = originalStyles.height;

          console.log('[MilestoneTracker] Canvas capture complete, converting to base64');
          const base64Data = canvas.toDataURL('image/png');
          
          // Check data size
          const sizeInBytes = new Blob([base64Data]).size;
          console.log(`[MilestoneTracker] Image size: ${sizeInBytes} bytes`);

          if (sizeInBytes > 45000) {
            console.warn('[MilestoneTracker] Image size exceeds 45K characters, will need chunking');
            // TODO: Implement chunking if needed
          }

          // Set custom header with image data
          console.log('[MilestoneTracker] Setting Milestone_Image_Chunk_1 header');
          const chunk: MilestoneImageChunk = {
            chunkNumber: 1,
            totalChunks: 1,
            data: base64Data
          };

          // You'll need to implement this function in your API service
          try {
            await setMilestoneImageHeader(chunk);
            console.log('[MilestoneTracker] Successfully set image header');
            setIncludeInReport(true);
          } catch (headerError) {
            console.error('[MilestoneTracker] Failed to set image header:', headerError);
            setCaptureError('Failed to set image header');
            throw headerError;
          }
        } else {
          console.error('[MilestoneTracker] Timeline wrapper ref not found');
          setCaptureError('Timeline element not found');
        }
      } else {
        console.log('[MilestoneTracker] Removing timeline from report');
        // Clear the header when removing from report
        try {
          await clearMilestoneImageHeader();
          console.log('[MilestoneTracker] Successfully cleared image header');
          setIncludeInReport(false);
        } catch (clearError) {
          console.error('[MilestoneTracker] Failed to clear image header:', clearError);
          setCaptureError('Failed to clear image header');
          throw clearError;
        }
      }
    } catch (error) {
      console.error('[MilestoneTracker] Error in capture process:', error);
      setCaptureError(error instanceof Error ? error.message : 'Unknown error occurred');
    } finally {
      setIsCapturingImage(false);
    }
  }, [includeInReport, wrapperRef]);

  // Add error effect
  useEffect(() => {
    if (captureError) {
      console.error('[MilestoneTracker] Capture error occurred:', captureError);
      ...
      >
        ...
        <div ref={wrapperRef} className={styles.timelineWrapper}>
          ...
          >
            ...
            {isCapturingImage ? "Processing..." : (includeInReport ? "Remove from Report" : "Include in Report")}
          </button>
