...
const RATING_SCALE_INFO = "Rate each domain from 1 (age appropriate) to 5 (significant challenge)";
...
export const AssessmentCarousel: React.FC = () => {
  ...
  const [socialMetrics, setSocialMetrics] = useState<Record<string, MetricInfo>>({
    reciprocity: {
      ...
      description: RATING_SCALE_INFO,
      clinicalDesc: "Ability to engage in back-and-forth social interactions and respond to social cues",
      value: 1
    },
    nonVerbalComm: {
      label: "Non-verbal Communication",
      description: RATING_SCALE_INFO,
      clinicalDesc: "Use and understanding of gestures, facial expressions, and body language",
      value: 1
    },
    relationships: {
      label: "Relationships",
      description: RATING_SCALE_INFO,
      clinicalDesc: "Ability to form and maintain peer relationships appropriate to developmental level",
      value: 1
    },
    emotionalRegulation: {
      label: "Emotional Regulation",
      description: RATING_SCALE_INFO,
      clinicalDesc: "Ability to manage and express emotions appropriately for age",
      value: 1
    },
    adaptiveFunctioning: {
      label: "Adaptive Functioning",
      description: RATING_SCALE_INFO,
      clinicalDesc: "Ability to handle daily activities and adjust to new situations",
      value: 1
    }
  });
  ...
  const radarData = Object.entries(socialMetrics).map(([key, info]) => ({
    category: info.label,
    value: 6 - info.value, // Invert for visualization (1 = small area, 5 = large area)
    fullMark: 5,
    originalValue: info.value,
    clinicalDesc: info.clinicalDesc
  }));
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="custom-tooltip">
          ...
          <p className="tooltip-desc">{data.clinicalDesc}</p>
        </div>
      );
    }
    return null;
  ...
  const SocialCommunicationChart = (
    <div className="assessment-card">
      <div className="assessment-header">
        ...
        <i className="material-icons info-icon" title={RATING_SCALE_INFO}>help_outline</i>
      </div>
          ...
          {Object.entries(socialMetrics).map(([key, info]) => (
            <div key={key} className="metric-control">
              <div className="metric-header">
                <label title={info.clinicalDesc}>
                  {info.label}
                  <div className="clinical-desc">{info.clinicalDesc}</div>
                </label>
