...
export const MilestoneTracker: React.FC<{ onChange?: (data: any) => void }> = ({ onChange }) => {
  ...
  // Handle history text changes with proper debouncing
  const handleHistoryChange = useCallback((text: string) => {
    setHistoryText(text);
    const timeoutId = setTimeout(() => debouncedUpdate(text), 300);
  ...
  useEffect(() => {
    return () => {
      const savedHistory = globalState.assessments?.milestones?.history;
      if (historyText !== savedHistory) {
        debouncedUpdate(historyText);
      }
    };
  }, [historyText, debouncedUpdate]);
