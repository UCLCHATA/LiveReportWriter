...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = ({ onProgressUpdate }) => {
  ...
  useEffect(() => {
    ...
    const sensoryProgress = calculateComponentProgress(globalState.sensoryProfile);
    const socialProgress = calculateComponentProgress(globalState.socialCommunication);
    const behaviorProgress = calculateComponentProgress(globalState.behaviorInterests);
    const milestoneProgress = calculateComponentProgress(globalState.milestoneTracker);
    const assessmentProgress = calculateComponentProgress(globalState.assessmentLog);

    progress = (sensoryProgress + socialProgress + behaviorProgress + milestoneProgress + assessmentProgress) * 10;
    onProgressUpdate(progress);
  }, [
    globalState.sensoryProfile,
    globalState.socialCommunication,
    globalState.behaviorInterests,
    globalState.milestoneTracker,
    globalState.assessmentLog,
    onProgressUpdate
  ]);
  ...
  const calculateComponentProgress = (component: any): number => {
    ...
    // For components with sliders and observations
    ...
    // For milestone tracker and assessment log
    if (component.items) {
      return component.items.length > 0 ? 1 : 0;
    }
