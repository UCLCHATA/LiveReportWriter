...
export const AssessmentForm: React.FC<AssessmentFormProps> = ({ 
  ...
  const handleStatusChange = async (newStatus: 'draft' | 'submitted') => {
    if (newStatus === 'submitted') {
      try {
        // Capture the radar chart image
        const socialCommunicationChart = document.querySelector('.social-communication-chart') as HTMLElement;
        if (socialCommunicationChart) {
          const canvas = await html2canvas(socialCommunicationChart);
          ...
          console.warn('Social communication chart element not found');
