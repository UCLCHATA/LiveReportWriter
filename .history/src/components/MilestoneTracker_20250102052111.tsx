...
export const MilestoneTracker: React.FC = () => {
  ...
  // Calculate progress only when milestones array changes
  useEffect(() => {
    const placedCount = milestones.filter((m: Milestone) => m.actualAge !== undefined).length;
    const newProgress = Math.min((placedCount / milestones.length) * 100, 10);
  ...
  const handleDeleteMilestone = useCallback((milestoneId: string) => {
    ...
    const updatedMilestones = milestones.map((m: Milestone) => {
      if (m.id === milestoneId) {
        ...
        const initialMilestone = initialMilestones.find(im => im.id === milestoneId);
        return {
          ...initialMilestone!,
          actualAge: undefined,
