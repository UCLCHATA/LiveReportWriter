...
interface Tool {
  id: 'sensoryProfile' | 'socialCommunication' | 'behaviorInterests';
  title: string;
...
interface ComponentProps {
  data: SensoryProfileData | SocialCommunicationData | BehaviorInterestsData;
  onChange: (data: SensoryProfileData | SocialCommunicationData | BehaviorInterestsData) => void;
}
...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = React.memo(({
  ...
  const [completionStates, setCompletionStates] = useState<Record<keyof AssessmentData, CompletionState>>(() => {
    ...
    if (globalState?.assessments) {
      ...
      // Initialize empty states if no saved state
      tools.forEach(tool => {
        states[tool.id] = {
          isComplete: false,
          isSkipped: false,
  ...
  const handleSensoryProfileChange = (data: SensoryProfileData) => {
    if (!globalState?.assessments) return;
    globalState.assessments.sensoryProfile = data;
    updateGlobalState(globalState);
  };

  const handleSocialCommunicationChange = (data: SocialCommunicationData) => {
    if (!globalState?.assessments) return;
    globalState.assessments.socialCommunication = data;
    updateGlobalState(globalState);
  };

  const handleBehaviorInterestsChange = (data: BehaviorInterestsData) => {
    if (!globalState?.assessments) return;
    globalState.assessments.behaviorInterests = data;
    updateGlobalState(globalState);
  };
      ...
      case 'socialCommunication':
        return (
          <SocialCommunication
            ...
            onChange={handleSocialCommunicationChange}
          />
        );
      case 'behaviorInterests':
        return (
          <BehaviorInterests
            ...
            onChange={handleBehaviorInterestsChange}
          />
        );
      default:
        return null;
