...
const categoryInfo: CategoryInfoMap = {
  ...
  concerns: {
    ...
    color: '#9F7AEA',
    tooltips: {
      'Repetitive behaviors': 'Repetitive movements or use of objects, insistence on sameness',
      'Social interaction': 'Limited reciprocal social interaction, reduced joint attention',
      'Communication patterns': 'Atypical communication patterns, echolalia, pronoun reversal'
    }
  }
};
...
const DraggableMilestone: React.FC<{ 
  ...
  const tooltipText = milestone.actualAge !== undefined 
    ...
    : milestone.id === 'custom-concern' ? 'Click to add custom concern' : undefined;
  ...
  if (milestone.id === 'custom-concern') {
    return (
      <motion.div
        className={`${styles.milestone} ${styles.customConcern}`}
