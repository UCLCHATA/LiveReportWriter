...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = ({ 
  ...
  useEffect(() => {
    ...
    if (savedForm?.assessmentState) {
      console.log('[AssessmentCarousel] Restoring assessment state:', savedForm.assessmentState);
      
      // Restore assessment state if it exists
      updateAssessments(savedForm.assessmentState);
      
      // Initialize completion states
      const newStates: Record<string, CompletionState> = {};
      tools.forEach(tool => {
        const component = savedForm.assessmentState?.[tool.id];
        if (component) {
  ...
  }, [globalState.chataId, isInitialized, updateAssessments]);
