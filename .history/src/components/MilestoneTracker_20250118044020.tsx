...
import { submitFormData, generateImage, chunkImageData } from '../services/api';
...
export const MilestoneTracker: React.FC<{ data: any; onChange: (data: any) => void }> = React.memo(({ data, onChange }) => {
    ...
    try {
      if (!includeInReport) {
        ...
        if (wrapperRef.current) {
          ...
          try {
            // Generate image using improved helper
            console.log('[MilestoneTracker] Generating timeline image');
            const base64Data = await generateImage(wrapperRef.current);
            
            if (!base64Data) {
              throw new Error('Failed to generate image within size limit');
            }

            // Use improved chunking with include flag
            console.log('[MilestoneTracker] Chunking image data with include flag');
            const chunks = chunkImageData(base64Data, true); // Pass true for include flag
            
            // Update data with image chunks and ensure includeImages is true
            const updatedData = {
              ...data,
              milestoneImage: {
                include: true,
            ...
            console.log('[MilestoneTracker] Updating form data with image chunks and flags:', {
              includeImages: true,
              includeTimelineInReport: true,
              chunkSizes: {
                chunk1: chunks.chunk1.length,
                chunk2: chunks.chunk2.length,
                chunk3: chunks.chunk3.length
              }
            });
        ...
        // Clear the image data using chunking helper with include=false
        const chunks = chunkImageData(null, false);
        const updatedData = {
          ...data,
          milestoneImage: {
            include: false,
        ...
        console.log('[MilestoneTracker] Updating form data to remove image:', {
          includeImages: false,
          includeTimelineInReport: false
        });
  ...
  }, [includeInReport, wrapperRef, data, onChange]);
