...
export const MilestoneTracker: React.FC = () => {
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    // Calculate the dropped age based on the drop position
    const timelineElement = document.querySelector(`.${styles.timeline}`);
    if (!timelineElement) return;

    const rect = timelineElement.getBoundingClientRect();
    const dropX = result.destination.clientOffset?.x || 0;
    const relativeX = dropX - rect.left;
    const droppedAge = Math.max(0, Math.min(maxAge, Math.round((relativeX / rect.width) * maxAge)));
