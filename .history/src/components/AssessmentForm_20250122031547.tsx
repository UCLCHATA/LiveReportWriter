...
export const AssessmentForm: React.FC<AssessmentFormProps> = ({ 
  ...
  // Function to check if all components are complete
  const areAllComponentsComplete = useCallback(() => {
    if (!globalState?.formData?.componentProgress) return false;
    return Object.values(globalState.formData.componentProgress).every(
      component => component.isComplete
    );
  }, [globalState?.formData?.componentProgress]);

  // Function to get validation items
  const getValidationItems = useCallback(() => {
    if (!globalState?.formData) return [];

    const textFields = {
      'Clinical Observations': globalState.formData.clinicalObservations || '',
      'Strengths & Abilities': globalState.formData.strengths || '',
      'Priority Support Areas': globalState.formData.priorityAreas || '',
      'Support Recommendations': globalState.formData.recommendations || ''
    };

    const items = [
      // Text fields validation
      ...Object.entries(textFields).map(([label, value]) => ({
        id: label.toLowerCase().replace(/[^a-z]/g, '-'),
        label: `${label} has good detail level`,
        isComplete: getWordCountState(value) === 'good' || getWordCountState(value) === 'excellent'
      })),

      // Status fields validation
      {
        id: 'asc-status',
        label: 'ASC Status is selected',
        isComplete: !!globalState.formData.ascStatus
      },
      {
        id: 'adhd-status',
        label: 'ADHD Status is selected',
        isComplete: !!globalState.formData.adhdStatus
      },

      // Components completion validation
      {
        id: 'components-complete',
        label: 'All assessment components are marked as complete',
        isComplete: areAllComponentsComplete()
      }
    ];
  ...
  // Update handleStatusChange to show validation dialog
  ...
  const handleValidationProceed = async () => {
    ...
    try {
      ...
      const result = await SubmissionService.submit(
        globalState as GlobalFormState, 
        true,
        chartRef.current
      );
