...
const HistoryTextBox: React.FC<HistoryTextBoxProps> = React.memo(({ value, onChange }) => {
...
export const MilestoneTracker: React.FC<{ data: any; onChange: (data: any) => void }> = React.memo(({ data, onChange }) => {
  ...
  const calculateProgress = (milestone: Milestone): number => {
    if (milestone.actualAge === undefined || milestone.actualAge === null) return 0;
    return milestone.actualAge;
  };

  const sortedMilestones = milestones.sort((a: Milestone, b: Milestone) => {
    const aAge = a.actualAge || 0;
    const bAge = b.actualAge || 0;
    return aAge - bAge;
  });
