    ...
    const updateData = async () => {
      if (includeInReport) {
        ...
        const formattedMilestones = milestones
          ...
          .map((m: Milestone) => {
            const difference = m.actualAge !== undefined ? m.actualAge - m.expectedAge : 0;
            let status = 'Not Assessed';
            if (m.actualAge !== undefined) {
              if (difference > 0) {
            ...
            return `${m.title}|${m.isCustom ? 'Custom milestone' : `Expected at ${m.expectedAge} months`}|${m.isCustom ? 'NA' : m.expectedAge}|${m.actualAge ?? 'Not Assessed'}|${status}`;
          })
          .join(' || ');
