...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = React.memo(({
  ...
  const handleToggleInclude = useCallback(async () => {
    ...
    try {
      ...
      if (chartElement) {
        console.log('Attempting to capture radar chart image...');
        ...
        console.log('Image captured successfully, length:', chartImage.length);
        
        // Always update the state with the image, just toggle includeInReport
        ...
        console.log('Updating assessment state with image data');
        updateAssessment('summary', summaryData);
      } else {
        console.error('Could not find radar chart element');
      }
    } catch (error) {
