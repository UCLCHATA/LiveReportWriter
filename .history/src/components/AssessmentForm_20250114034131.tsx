        ...
        const response = await fetch(R3_FORM_API, {
          ...
          body: JSON.stringify({
            r3Form: {
              ...
              // Add assessment data
              sensoryProfile: Object.entries(globalState.assessments.sensoryProfile.domains).map(([key, domain]) => ({
                domain: domain.name,
                value: domain.value,
                label: domain.label,
                observations: domain.observations
              })),
              socialCommunication: Object.entries(globalState.assessments.socialCommunication.domains).map(([key, domain]) => ({
                domain: domain.name,
                value: domain.value,
                label: domain.label,
                observations: domain.observations
              })),
              behaviorInterests: Object.entries(globalState.assessments.behaviorInterests.domains).map(([key, domain]) => ({
                domain: domain.name,
                value: domain.value,
                label: domain.label,
                observations: domain.observations
              ...
              ...(radarChartImage && { radarChartImage }),
              ...(formData.timelineImage && { milestoneTimelineImage: formData.timelineImage })
            }
          })
        });
