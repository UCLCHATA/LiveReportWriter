...
export const AssessmentSummary: React.FC<AssessmentSummaryProps> = ({ data, onChange }) => {
  ...
  const showSensoryData = data?.sensoryProfile?.domains && 
    Object.values(data.sensoryProfile.domains).some(domain => domain.value !== undefined && domain.value > 0);
  
  const showSocialData = data?.socialCommunication?.domains && 
    Object.values(data.socialCommunication.domains).some(domain => domain.value !== undefined && domain.value > 0);
  
  const showBehaviorData = data?.behaviorInterests?.domains && 
    Object.values(data.behaviorInterests.domains).some(domain => domain.value !== undefined && domain.value > 0);
      ...
      {showMilestones && (
        ...
        >
          ...
          <div className={styles.timeline}>
            <MilestoneTracker 
              data={data?.milestones || { milestones: [] }}
