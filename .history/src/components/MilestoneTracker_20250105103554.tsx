...
export const MilestoneTracker: React.FC<{ onChange?: (data: any) => void }> = ({ onChange }) => {
  ...
  // Initialize history text from global state
  const [historyText, setHistoryText] = useState(() => {
    return globalState.assessments?.milestones?.history || '';
  });

  // Load history text when global state changes
  useEffect(() => {
    const savedHistory = globalState.assessments?.milestones?.history;
    if (savedHistory !== undefined && savedHistory !== historyText) {
      setHistoryText(savedHistory);
    }
  }, [globalState.assessments?.milestones?.history]);
  ...
  const handleHistoryChange = useCallback((text: string) => {
    setHistoryText(text);
    // Ensure we're updating with all existing milestone data
  ...
  // Ensure history is saved when component unmounts
  useEffect(() => {
    return () => {
      if (historyText) {
        const currentMilestones = globalState.assessments?.milestones?.milestones || initialMilestones;
  ...
  }, [historyText, globalState.assessments?.milestones?.milestones, updateAssessment]);
