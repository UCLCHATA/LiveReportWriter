...
export const AssessmentForm: React.FC<AssessmentFormProps> = ({ onClear }) => {
  ...
  const handleModalContentChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    if (modalContent) {
      ...
      setModalContent(prev => prev ? { ...prev, value: newValue } : null);
  ...
  const renderModal = () => {
    ...
    const textLengthStatus = getTextLengthStatus(modalContent.value);

    return (
      <div className={styles.modalOverlay}>
        <div className={styles.modalContent}>
          <div className={styles.modalHeader}>
            {modalContent.field === 'clinicalObservations' && <Search className={styles.sectionIcon} size={18} />}
            {modalContent.field === 'priorityAreas' && <AlertTriangle className={styles.sectionIcon} size={18} />}
            {modalContent.field === 'strengths' && <Dumbbell className={styles.sectionIcon} size={18} />}
            {modalContent.field === 'recommendations' && <ThumbsUp className={styles.sectionIcon} size={18} />}
            <h3>{modalContent.title}</h3>
            <HelpCircle className={styles.helpIcon} size={16} />
            <button className={styles.modalCloseButton} onClick={handleModalClose}>×</button>
          </div>
          <textarea
            className={styles.modalTextArea}
            name={modalContent.field}
            value={modalContent.value}
            onChange={handleModalContentChange}
            placeholder={modalContent.placeholder}
            onKeyDown={handleTextAreaKeyDown}
          />
          <div className={`${styles.modalTextLengthIndicator} ${styles[textLengthStatus.status]}`}>
            {textLengthStatus.message}
          </div>
        </div>
      </div>
    );
