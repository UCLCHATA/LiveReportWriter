...
export const MilestoneTracker: React.FC<{ 
  ...
  // Update milestone data whenever it changes
  useEffect(() => {
    const formattedMilestones = milestones
      .filter(m => m.id !== 'custom-concern')
      .map((milestone: Milestone) => {
        const actualAge = milestone.actualAge;
        const expectedAge = milestone.expectedAge || 0;
        const status = actualAge !== undefined && expectedAge !== 0
          ? determineStatus(actualAge, expectedAge)
          : 'typical';
        const difference = actualAge !== undefined && expectedAge !== 0
    ...
    onChange({
      ...
      history: historyText,
      lastUpdated: new Date().toISOString(),
      timelineImage: includeInReport ? data?.timelineImage : '',
      includeTimelineInReport: includeInReport
    });
  }, [milestones, historyText]);

  // Handle image capture separately when includeInReport changes
  useEffect(() => {
    if (!includeInReport) {
      onChange({
        type: 'milestoneTracker',
