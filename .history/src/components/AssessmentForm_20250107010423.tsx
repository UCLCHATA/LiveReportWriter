...
export const AssessmentForm: React.FC<AssessmentFormProps> = ({ 
  ...
  useEffect(() => {
    if (globalState.chataId) {
      console.log('Attempting to restore form state for:', globalState.chataId);
      const savedState = localStorage.getItem(`chata-form-state-${globalState.chataId}`);
      if (savedState) {
        try {
          const parsedState = JSON.parse(savedState);
          console.log('Restored form state:', parsedState);
          updateFormData(parsedState.formData);
        } catch (error) {
  ...
  const handleSave = () => {
    ...
    // Update progress
    formPersistence.updateProgress(globalState.chataId, {
      formProgress: lastProgress.current,
      carouselProgress: 0 // This will be updated from the carousel component
    });
