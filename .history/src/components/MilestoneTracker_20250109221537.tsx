...
export const MilestoneTracker: React.FC<{ data: any; onChange: (data: any) => void }> = ({ data, onChange }) => {
  ...
  const isOverlapping = (milestone: Milestone, otherMilestones: Milestone[]): boolean => {
    if (milestone.actualAge === undefined) return false;
    
    return otherMilestones.some(other => {
      if (other.actualAge === undefined || other.id === milestone.id) return false;
      
      // Check if milestones are close enough to overlap
      const distance = Math.abs(milestone.actualAge - other.actualAge);
      const isClose = distance <= 8; // Adjust this value based on your bubble width
      
      // If they're close, check if they're in adjacent rows
      if (isClose) {
        const stackDiff = Math.abs((milestone.stackPosition || 0) - (other.stackPosition || 0));
        return stackDiff <= 1;
      }
      return false;
    });
            ...
            <div className={styles.placedMilestones}>
              <AnimatePresence>
                {milestones
                  ...
                  .map((milestone: Milestone) => {
                    if (milestone.actualAge === undefined) return null; // TypeScript guard
                    const hasOverlap = isOverlapping(milestone, milestones);
                    const isTopBubble = hasOverlap && milestone.stackPosition === 0;
                    ...
                    return (
                      <motion.div
                        ...
                        style={{
                          ...
                          opacity: isTopBubble ? 0.4 : 1
                        }}
                        initial={{ opacity: 0, y: 20 }}
                        animate={{ opacity: isTopBubble ? 0.4 : 1, y: 0 }}
                        exit={{ opacity: 0, y: -20 }}
                      >
                        <DraggableMilestone
                          milestone={milestone}
