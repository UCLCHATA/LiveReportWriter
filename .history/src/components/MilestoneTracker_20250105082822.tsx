...
const ExpectationLine: React.FC<{
  ...
  maxAge: number;
  isEarly: boolean;
}> = ({ actualAge, expectedAge, maxAge, isEarly }) => {
  const startX = (actualAge / maxAge) * 100;
  const endX = (expectedAge / maxAge) * 100;

  return (
    <div 
      className={styles.expectationLine}
      style={{
        left: `${isEarly ? startX : endX}%`,
        width: `${Math.abs(endX - startX)}%`,
        transform: `translateX(${isEarly ? '0' : '-100%'})`
      }}
    />
  );
};
