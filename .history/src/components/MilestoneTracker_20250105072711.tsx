...
const TimelineMonth: React.FC<{ 
  month: number; 
  maxAge: number;
  getCompressedPosition: (month: number) => number;
}> = ({ month, maxAge, getCompressedPosition }) => {
  ...
  return (
    <div
      ...
      style={{ 
        left: `${getCompressedPosition(month)}%`,
...
export const MilestoneTracker: React.FC<{ onChange?: (data: any) => void }> = ({ onChange }) => {
  ...
  // Calculate position with compression after 54 months
  const getCompressedPosition = (month: number) => {
    if (month <= 54) {
      return (month / 54) * 85; // First 54 months take up 85% of the width
    } else {
      const remainingMonths = month - 54;
      return 85 + ((remainingMonths / (maxAge - 54)) * 15); // Remaining months compressed into 15%
    }
  };
              ...
              {milestones
                ...
                .map((milestone: Milestone) => (
                  <motion.div
                    ...
                    style={{
                      left: `${getCompressedPosition(milestone.actualAge || 0)}%`,
