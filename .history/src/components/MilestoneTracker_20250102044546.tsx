...
export const MilestoneTracker: React.FC = () => {
  ...
  const handleDeleteMilestone = useCallback((milestoneId: string) => {
    ...
    // Create a new array with the milestone removed from timeline
    const updatedMilestones = milestones.map((m: Milestone) => {
      if (m.id === milestoneId) {
        // Reset the milestone to its original state
        return {
          ...m,
          actualAge: undefined,
    ...
    console.log('Updated milestones after delete:', updatedMilestones);
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    if (draggedIndex !== -1) {
      ...
      console.log('Updating milestone position:', {
        id: active.id,
        month,
        stackPosition
      });
