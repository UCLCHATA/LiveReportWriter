...
type AssessmentType = 'sensoryProfile' | 'socialCommunication' | 'behaviorInterests' | 'milestones' | 'assessmentLog' | 'summary';
...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = React.memo(({
  ...
  const handleUpdateAssessment = useCallback((id: AssessmentType, updates: any) => {
    const now = Date.now();
  ...
  const [completionStates, setCompletionStates] = useState<Record<AssessmentType, CompletionState>>(() => {
    // Initialize from saved state
    const states = {} as Record<AssessmentType, CompletionState>;
    tools.forEach(tool => {
    ...
    progressUpdateTimeoutRef.current = setTimeout(() => {
      if (progress !== initialProgress) {
        ...
        Object.entries(updates).forEach(([type, data]) => {
          updateAssessment(type as AssessmentType, data);
        });
      }
    ...
    switch (component.type) {
      ...
      case 'summary':
        ...
        const completedCount = otherComponents.filter(id => 
          completionStates[id as AssessmentType]?.isComplete
        ).length;
  ...
  // Handle completion overlay visibility
  useEffect(() => {
    const currentTool = tools[currentIndex];
    if (!currentTool) return;

    const isComplete = completionStates[currentTool.id]?.isComplete;
  ...
  }, [completionStates, currentIndex, tools]);

  const handleMarkComplete = useCallback((toolId: AssessmentType) => {
    ...
    // Then update the assessment state after a small delay
    setTimeout(() => {
  ...
  const handleMarkIncomplete = useCallback((toolId: AssessmentType) => {
    // First hide the overlay
    setShowOverlay(false);
    
    // Then update states after a small delay
    setTimeout(() => {
      setCompletionStates(prev => ({
        ...prev,
        [toolId]: {
      ...
      >
        {(currentTool?.id === 'sensoryProfile' || 
          ...
          <div className={styles.componentWithGraph}>
            <div className={`${styles.graphSection} ${completionStates[currentTool?.id]?.isComplete ? styles.completed : ''} ${showOverlay ? styles.visible : ''}`}>
            ...
            <div className={styles.sliderSection}>
              <div className={styles.sliderContainer}>
                ...
                />
                {completionStates[currentTool?.id]?.isComplete && (
                  <div className={`${styles.completionOverlay} ${showOverlay ? styles.visible : ''}`} />
                )}
              </div>
            </div>
          </div>
