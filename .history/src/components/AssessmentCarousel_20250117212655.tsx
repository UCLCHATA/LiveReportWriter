...
// Define tool types
type ToolId = keyof AssessmentData;

interface BaseComponentProps<T extends ToolId> {
  data: AssessmentData[T];
  onChange: (data: AssessmentData[T]) => void;
}

interface Tool<T extends ToolId> {
  id: T;
  title: string;
  component: React.ComponentType<BaseComponentProps<T>>;
  description: string;
  icon: string;
}
...
const tools = [
  {
    id: 'sensoryProfile' as const,
    title: 'Sensory Profile',
    component: SensoryProfileBuilder as React.ComponentType<BaseComponentProps<'sensoryProfile'>>,
    description: 'Evaluate sensory processing patterns',
    icon: sensoryIcon
  },
  {
    id: 'socialCommunication' as const,
    title: 'Social Communication',
    component: SocialCommunicationProfile as React.ComponentType<BaseComponentProps<'socialCommunication'>>,
    description: 'Assess social interaction and communication skills',
    icon: socialIcon
  },
  {
    id: 'behaviorInterests' as const,
    title: 'Behavior & Interests',
    component: BehaviorInterestsProfile as React.ComponentType<BaseComponentProps<'behaviorInterests'>>,
  ...
  },
  {
    ...
    component: MilestoneTracker as React.ComponentType<BaseComponentProps<'milestones'>>,
    description: 'Track developmental milestones',
    icon: developmentIcon
  },
  {
    id: 'assessmentLog' as const,
    title: 'Assessment Log',
    component: AssessmentLogger as React.ComponentType<BaseComponentProps<'assessmentLog'>>,
  ...
  },
  {
    id: 'summary' as const,
    title: 'Summary',
    component: AssessmentSummary as React.ComponentType<BaseComponentProps<'summary'>>,
    description: 'Review and summarize assessment findings',
    icon: summaryIcon
  }
] as const;
...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = React.memo(({
  ...
  const [completionStates, setCompletionStates] = useState<Record<ToolId, CompletionState>>(() => {
    // Initialize from saved state
    const states = {} as Record<ToolId, CompletionState>;
    
    // Only initialize if we have valid state
    if (globalState?.assessments) {
      ...
      // Initialize with default values if no state
      tools.forEach(tool => {
        states[tool.id] = {
          isComplete: false,
          isSkipped: false,
  ...
  // Batch progress updates with null checks
  const handleProgressUpdate = useCallback((progress: number) => {
    ...
    progressUpdateTimeoutRef.current = setTimeout(() => {
      if (progress !== initialProgress) {
        ...
        const updates = tools.reduce<Partial<Record<ToolId, AssessmentData[ToolId]>>>((acc, tool) => {
          const currentAssessment = globalState.assessments[tool.id];
        ...
        if (Object.keys(updates).length > 0) {
          Object.entries(updates).forEach(([type, data]) => {
            updateAssessment(type as ToolId, data);
          });
        }
      }
    }, 500);
  ...
  }, [currentIndex, globalState?.assessments, tools, calculateProgress]);
  ...
  // Render component with null checks
  const renderCurrentComponent = () => {
    if (!globalState?.assessments) return null;

    const currentTool = tools[currentIndex];
    const component = globalState.assessments[currentTool.id];
    if (!component) return null;

    const Component = currentTool.component;
    return (
      <Component 
        data={component} 
        onChange={(data: any) => {
          updateAssessment(currentTool.id, data);
        }}
      />
    );
  };
      ...
      {globalState?.assessments ? (
        <>
          <div className={styles.header}>
            ...
            <p>{tools[currentIndex].description}</p>
          </div>
