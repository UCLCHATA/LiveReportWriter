...
export const AssessmentForm: React.FC<AssessmentFormProps> = ({ onClear, onProgressUpdate }) => {
  ...
  useEffect(() => {
    ...
    const statusProgress = globalState.formData?.ascStatus ? 2.5 : 0;
    const adhdProgress = globalState.formData?.adhdStatus ? 2.5 : 0;

    // Referrals contribute 5% (optional)
    const hasReferrals = Object.values(globalState.formData?.referrals || {}).some(value => value);
    const referralsProgress = hasReferrals ? 5 : 0;
