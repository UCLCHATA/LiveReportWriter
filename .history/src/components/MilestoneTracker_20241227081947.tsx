...
export const MilestoneTracker: React.FC = () => {
  ...
  const handleDragMove = (event: DragMoveEvent) => {
    ...
    const cursorX = event.delta.x + event.active.rect.current.left;
    const timelineStart = timelineRect.left;
    const timelineWidth = timelineRect.width;
    
    // Calculate relative position
    const relativePosition = (cursorX - timelineStart) / timelineWidth;
    const month = Math.round(Math.max(0, Math.min(maxAge, maxAge * relativePosition)));

    // Update current position indicator
    const currentAge = document.querySelector(`.${styles.currentAgeIndicator}`);
    if (currentAge) {
      currentAge.style.left = `${(month / maxAge) * 100}%`;
      currentAge.textContent = `${month}m`;
    }
    ...
    if (expectedLine) {
      ...
      expectedLine.style.backgroundColor = categoryInfo[activeMilestone.category].color;
    }
  };
