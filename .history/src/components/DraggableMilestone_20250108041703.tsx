...
export const DraggableMilestone: React.FC<DraggableMilestoneProps> = ({ 
  ...
  useEffect(() => {
    console.log('DraggableMilestone mounted/updated:', {
      id: milestone.id,
      category,
  ...
  useEffect(() => {
    if (isHovered) {
      console.log('Milestone hover state:', {
        id: milestone.id,
        isHovered,
        differenceText,
        isDelayed,
  ...
  }, [isHovered, milestone.id, differenceText, isDelayed, isEarly]);

  const classNames = [
    styles.milestone,
    milestone.actualAge !== undefined ? styles.placedMilestone : '',
