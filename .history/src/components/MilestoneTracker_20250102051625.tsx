...
export const MilestoneTracker: React.FC = () => {
  ...
  const updateMilestoneProgress = useCallback(() => {
    const placedCount = milestones.filter(m => m.actualAge !== undefined).length;
    ...
    console.log('Calculating progress:', {
      ...
      progress,
      milestones: milestones.map(m => ({id: m.id, placed: !!m.actualAge}))
    });
  ...
  const handleDeleteMilestone = useCallback((milestoneId: string) => {
    ...
    const milestoneToReset = milestones.find(m => m.id === milestoneId);
    ...
    console.log('Resetting milestone:', {
      ...
      after: updatedMilestones.find(m => m.id === milestoneId)
    });
