...
const MilestoneTracker: React.FC<MilestoneTrackerProps> = ({
  ...
  if (state.error) {
    return (
      <div className={`milestone-tracker-error ${className}`}>
  ...
  if (state.isLoading) {
    return (
      <div className={`milestone-tracker-loading ${className}`}>
  ...
  return (
    <div className={`milestone-tracker ${isCollapsed ? 'collapsed' : ''} ${className}`}>
      <button 
        className="milestone-collapse-toggle"
        onClick={toggleCollapse}
        title={isCollapsed ? 'Show Milestones' : 'Hide Milestones'}
      >
        {isCollapsed ? '→' : '←'}
      </button>
      ...
      {!isCollapsed && (
        <>
          ...
          <div className="controls">
            ...
            <button onClick={exportData} title="Export milestone data">Export</button>
            <button onClick={resetComponent} title="Reset to default milestones">Reset</button>
          </div>
          ...
          {state.selectedMilestone && (
            <div className={`milestone-details ${detailsExpanded ? '' : 'collapsed'}`}>
              ...
              >
                <h3>{state.selectedMilestone.description}</h3>
                ...
                <div className="collapsible-content expanded">
                  <div className="milestone-info">
                    <div>
                      ...
                      <p>{state.selectedMilestone.expectedAgeRange.min} - {state.selectedMilestone.expectedAgeRange.max} months</p>
                    </div>
                    <div>
                      <label>Status</label>
                      <select
