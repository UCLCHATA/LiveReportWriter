...
export const MilestoneTracker: React.FC = () => {
  ...
  const updateMilestonesAndProgress = useCallback((updatedMilestones: Milestone[]) => {
    const placedCount = updatedMilestones.filter(m => m.actualAge !== undefined).length;
    const progress = Math.min(placedCount, 10);

    // Only update milestones, let the carousel handle progress
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    // Update milestones and progress together
    updateMilestonesAndProgress(updatedMilestones);

    setActiveMilestone(null);
    setCurrentMonth(null);
  }, [milestones, updateMilestonesAndProgress]);
