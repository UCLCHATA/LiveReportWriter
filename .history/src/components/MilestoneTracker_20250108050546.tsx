...
const DraggableMilestone: React.FC<{ 
  ...
  tooltipText: string;
}> = ({ milestone, category, onCustomConcern, tooltipText }) => {
...
// Helper functions
const getDifferenceText = (milestone: Milestone) => {
  if (milestone.actualAge === undefined || milestone.expectedAge === 0) return null;
  const diff = milestone.actualAge - milestone.expectedAge;
  if (diff === 0) return null;
  return diff > 0 ? `+${diff}` : `${diff}`;
...
const isDelayed = (milestone: Milestone) => {
  return milestone.actualAge !== undefined && 
         milestone.expectedAge !== 0 && 
         milestone.actualAge > milestone.expectedAge;
};

const isEarly = (milestone: Milestone) => {
  return milestone.actualAge !== undefined && 
         milestone.expectedAge !== 0 && 
         milestone.actualAge < milestone.expectedAge;
};

const mapCategory = (category: string): 'communication' | 'motor' | 'social' | 'concerns' => {
  switch (category) {
    case 'language':
    ...
    case 'social':
      return 'social';
    case 'cognitive':
      return 'social';  // Map cognitive to social for now
    default:
      return 'communication';
  }
};
...
export const MilestoneTracker: React.FC<{ data: any; onChange: (data: any) => void }> = ({ data, onChange }) => {
  ...
  // Map incoming milestones to the correct structure
  const mappedMilestones = React.useMemo(() => {
    if (!data?.milestones) return initialMilestones;
    
    return data.milestones.map((m: any) => ({
      id: m.id,
      title: m.name || m.title,  // Support both name and title
  ...
  useEffect(() => {
    ...
    if (!data || !data.milestones || data.milestones.length === 0) {
      ...
      console.log('Initializing with:', initialData);
      onChange(initialData);
    }
  }, [data, onChange]);

  const milestones = mappedMilestones;
                  ...
                  .map((milestone: Milestone) => {
                    const tooltipText = milestone.actualAge !== undefined 
                      ? `Placed at: ${milestone.actualAge}m${milestone.expectedAge ? ` (Expected: ${milestone.expectedAge}m)` : ''}${
                        getDifferenceText(milestone) ? ` - ${isDelayed(milestone) ? 'Delayed' : 'Advanced'} by ${Math.abs(milestone.actualAge - milestone.expectedAge)}m` : ''
                      }`
                      : categoryInfo[milestone.category as CategoryType].tooltips[milestone.title];
