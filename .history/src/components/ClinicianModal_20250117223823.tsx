...
const formState = new FormStateService();
...
export const ClinicianModal: React.FC<ClinicianModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const currentState = formState.getCurrentState();
    const existingForm = currentState?.clinician.name === formatted.name && 
                        currentState?.clinician.childFirstName === formatted.childFirstName;

    if (existingForm && formatted.childFirstName) {
      setChataIdInput(currentState!.chataId);
  ...
  const handleRetrieveClick = () => {
    ...
    // Try to restore form
    const currentState = formState.getCurrentState();
    if (currentState && currentState.clinician.name === clinicianInfo.name) {
      try {
        const info: ClinicianInfo = {
          name: currentState.clinician.name,
          email: currentState.clinician.email,
          clinicName: currentState.clinician.clinicName,
          childFirstName: currentState.clinician.childFirstName || '',
          childLastName: currentState.clinician.childLastName || '',
          childAge: currentState.clinician.childAge || '',
          childGender: currentState.clinician.childGender || '',
          chataId: currentState.chataId
        };
