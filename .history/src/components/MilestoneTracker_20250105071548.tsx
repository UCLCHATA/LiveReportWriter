...
const DraggableMilestone: React.FC<{ 
  ...
  const style = transform ? {
    ...
    top: milestone.stackPosition ? `${milestone.stackPosition * 40}px` : undefined
  } : {
    top: milestone.stackPosition ? `${milestone.stackPosition * 40}px` : undefined
  };
...
const getStackPosition = (
  ...
  const placedMilestones = milestones
    ...
    .sort((a, b) => (a.actualAge || 0) - (b.actualAge || 0));

  let row = 0;
  let maxRows = 3; // Maximum number of rows to prevent going too far below timeline
  let hasOverlap = true;
  ...
  while (hasOverlap && row < maxRows) {
    hasOverlap = placedMilestones.some((m: Milestone) => {
      ...
      const mightOverlap = distance <= 15; // Reduced overlap threshold
  ...
  // If we've exceeded maxRows, find the least crowded row
  if (row >= maxRows) {
