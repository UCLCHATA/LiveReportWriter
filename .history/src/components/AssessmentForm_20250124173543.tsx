...
export const AssessmentForm: React.FC<AssessmentFormProps> = ({ 
  ...
  const handleStatusChange = async (newStatus: 'draft' | 'submitted') => {
    if (newStatus === 'submitted' && globalState) {
      try {
        ...
        const result = await SubmissionService.submit(globalState);

        if (result.success) {
          // Update submission progress and stage
          setSubmissionProgress(100);
          setSubmissionStage('complete');
          setSubmissionDetails({
            documentUrl: result.submittedData?.documentUrl,
            emailStatus: result.submittedData?.emailStatus
          });
