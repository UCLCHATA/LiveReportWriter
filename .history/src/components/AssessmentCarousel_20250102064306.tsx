    ...
    switch (component.type) {
      ...
      case 'assessmentLog':
        ...
        selectedAssessments.forEach((assessment: any) => {
          ...
          if (entry?.notes) {
            // Get typing indicator level (0-3, where 3 is "Excellent detail")
            const typingLevel = entry.typingIndicator || 0;
            // Each level contributes 2.5% up to 10% for excellent detail
            progress += (typingLevel / 3) * 10;
          }
        });
      ...
      case 'behaviorInterests':
        if (component.domains) {
          ...
          // Count completed sliders
          const completedSliders = domains.filter((domain: any) => 
            typeof domain.value === 'number' && domain.value !== 3
          ).length;
          
          // Count domains with observations and their typing levels
          let observationProgress = 0;
          domains.forEach((domain: any) => {
            if (domain.observations?.length > 0) {
              // Get the highest typing indicator level for this domain's observations
              const maxTypingLevel = Math.max(...domain.observations.map((obs: any) => obs.typingIndicator || 0));
              // Each level contributes 2.5% up to 10% for excellent detail
              observationProgress += (maxTypingLevel / 3) * 10;
            }
          });
          
          // Sliders contribute 60% of progress
          const sliderProgress = (completedSliders / domainCount) * 6;
          // Observations (with typing quality) contribute 40%
          observationProgress = Math.min(observationProgress / domainCount, 4);
