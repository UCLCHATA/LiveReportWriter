          ...
          <RadarChart cx="50%" cy="50%" outerRadius="80%" data={data}>
            ...
            <PolarAngleAxis 
              ...
              tick={(props) => {
                ...
                // Normalize the domain name for comparison
                const domainName = payload.value;
                const centerX = Number(props.cx); // Get center x coordinate
                
                // Check which dataset this domain belongs to
                let color = '#4b5563'; // default
                
                if (sensoryData?.domains && Object.values(sensoryData.domains).some(d => d.name === domainName)) {
                  color = '#be185d'; // pink/maroon for sensory
                } else if (socialData?.domains && Object.values(socialData.domains).some(d => d.name === domainName)) {
                  color = '#10b981'; // green for social
                } else if (behaviorData?.domains && Object.values(behaviorData.domains).some(d => d.name === domainName)) {
                ...
                return (
                  <text 
                    ...
                    fontSize={12}
                    dy={3}
                    dx={x > centerX ? 5 : x < centerX ? -5 : 0} // Add horizontal offset based on position
                  >
                    {payload.value}
                  </text>
                );
