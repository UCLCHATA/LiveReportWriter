...
export const AssessmentForm: React.FC<AssessmentFormProps> = ({ 
  ...
  const { globalState, updateFormData, clearState, setChataId } = useFormState();
  ...
  // Initialize CHATA ID when component mounts
  useEffect(() => {
    console.log('Initializing AssessmentForm with CHATA ID:', chataId);
    if (chataId && chataId !== globalState.chataId) {
      setChataId(chataId);
    }
  }, [chataId, setChataId]);
  ...
  // Add effect to restore saved form state when mounted
  useEffect(() => {
    if (chataId) {
      console.log('AssessmentForm: Attempting to restore form state for:', chataId);
      const savedForm = formPersistence.getForm(chataId);
      console.log('AssessmentForm: Retrieved saved form:', savedForm);
      
      if (savedForm?.formState) {
        console.log('AssessmentForm: Found form state to restore:', {
          chataId,
          formState: savedForm.formState,
          hasClinicianInfo: !!savedForm.clinicianInfo,
          hasAssessmentState: !!savedForm.assessmentState
        });
        
        // Update the global state with the CHATA ID first
        setChataId(chataId);
        
        // Then update the form data with the saved state
        const restoredState = {
        ...
        console.log('AssessmentForm: Restoring form with state:', restoredState);
        updateFormData(restoredState);
        console.log('AssessmentForm: Form state restored successfully');
      } else {
        console.warn('AssessmentForm: No saved form state found for:', chataId);
      }
    } else {
      console.log('AssessmentForm: No CHATA ID provided for restoration');
    }
  }, [chataId, setChataId, updateFormData]);

  // Add validation for form data updates
  useEffect(() => {
    if (globalState.formData) {
      console.log('AssessmentForm: Form data updated:', {
        chataId: globalState.chataId,
        formData: globalState.formData
      });
    }
  }, [globalState.formData, globalState.chataId]);
  ...
  useEffect(() => {
    if (!globalState.formData) {
      ...
      // Update form data if referrals have changed
  ...
  const handleCheckboxChange = (field: keyof typeof formData.referrals) => (
    ...
    // Update both the referrals object
    updateFormData({
      referrals: updatedReferrals
    });
  };
  ...
  const handleSave = () => {
    if (!globalState.chataId || !globalState.formData) {
      console.error('No CHATA ID or form data available for saving');
      return;
    }

    // Get the current form from storage
    const currentForm = formPersistence.getForm(globalState.chataId);
    if (!currentForm) {
      console.error('No form found for CHATA ID:', globalState.chataId);
      return;
    }

    // Update form state
    formPersistence.updateFormState(globalState.chataId, {
      status: globalState.formData.status,
