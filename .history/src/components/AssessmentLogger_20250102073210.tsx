...
import { X, Search, AlertTriangle, Dumbbell, ThumbsUp, Check, RefreshCw } from 'lucide-react';
...
const AssessmentEntry: React.FC<{
  ...
  // Check if assessment is complete based on content
  const isContentComplete = Boolean(
    assessment.date && 
    assessment.notes?.trim().length >= 50
  );

  // Overall completion status
  const isComplete = assessment.completed || assessment.skipped;
  ...
  const handleToggleCompletion = () => {
    if (assessment.completed) {
      onUpdate(assessment.id, { completed: false, skipped: false });
    } else if (assessment.skipped) {
      onUpdate(assessment.id, { completed: true, skipped: false });
    } else {
      onUpdate(assessment.id, { completed: true, skipped: false });
    }
  };
  ...
  const handleSkip = () => {
    onUpdate(assessment.id, { completed: false, skipped: true });
  };
  ...
  return (
    <>
      <motion.div
        ...
        exit={{ opacity: 0, y: -5 }}
        className={`${styles.assessmentEntry} ${isComplete ? styles.completed : ''}`}
      >
        <div className={styles.entryHeader}>
          <div className={styles.entryTitle}>
            ...
            {isContentComplete && !isComplete && (
              <div className={styles.readyToComplete}>
                <Check size={14} />
              ...
              {!isComplete ? (
                <>
                  <button
                    ...
                    className={`${styles.completionButton} ${styles.complete}`}
                  ...
                  <button
                    onClick={handleSkip}
                    className={`${styles.completionButton} ${styles.skip}`}
                    title="Skip for now"
                  >
                    Skip
                  </button>
                ...
                <button
                  onClick={handleToggleCompletion}
                  className={`${styles.completionButton} ${styles.undo}`}
          ...
          <div className={styles.modalContent} onClick={e => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h3 className={styles.modalTitle}>
                ...
                {isComplete && (
                  <span className={styles.modalCompletionStatus}>
                    {assessment.completed ? 'Completed' : 'Skipped'}
                  </span>
                )}
              </h3>
