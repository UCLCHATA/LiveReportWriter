...
export const ClinicianModal: React.FC<ClinicianModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (clinicianInfo.name && clinicianInfo.email && clinicianInfo.clinicName) {
      ...
      const chataId = generateChataId(clinicianInfo.name, clinicianInfo.childName);
      ...
      // Show CHATA ID info after a brief delay to allow modal transition
  ...
  // Move the CHATA ID dialog outside the modal
  const chataIdDialog = showChataIdInfo && (
      ...
      <div className={styles.dialogContent}>
        ...
        <div className={styles.chataIdDisplay}>
          <span className={styles.blinkingId}>
            {generateChataId(clinicianInfo.name, clinicianInfo.childName)}
          </span>
        </div>
        <p className={styles.chataIdNote}>
          Your CHATA ID breaks down as:<br />
          • Your code: {clinicianInfo.name.split(' ')[0].substring(0, 3).toUpperCase()}<br />
          • Child's code: {clinicianInfo.childName ? clinicianInfo.childName.split(' ')[0].substring(0, 3).toUpperCase() : 'XXX'}<br />
          • Unique number: {generateChataId(clinicianInfo.name, clinicianInfo.childName).split('-')[2]}
        </p>
