...
const DraggableMilestone: React.FC<{ 
  ...
  return (
    <motion.div
      ref={milestone.id === 'custom-concern' ? undefined : setNodeRef}
      {...(milestone.id === 'custom-concern' ? {} : { ...attributes, ...listeners })}
...
export const MilestoneTracker: React.FC<{ onChange?: (data: any) => void }> = ({ onChange }) => {
  ...
  // Update both assessment and parent when history changes
  ...
  }, [globalState.assessments?.milestones?.milestones, updateAssessment, onChange]);
  ...
  }, [milestones, updateAssessment, onChange, globalState.assessments?.milestones?.history]);
  ...
  const handleCustomConcern = useCallback((text: string) => {
    ...
    const currentMilestones = [...milestones, newMilestone];
  ...
  }, [milestones, customConcernCount, updateAssessment, onChange, globalState.assessments?.milestones?.history]);
                ...
                {milestones
                  ...
                  .map((milestone: Milestone) => (
                    <DraggableMilestone
                      ...
                      milestone={milestone}
                      category={category}
                      onCustomConcern={category === 'concerns' ? handleCustomConcern : undefined}
                    />
                  ))}
                {category === 'concerns' && (
                  <DraggableMilestone
                    milestone={{ id: 'custom-concern', title: '+', category: 'concerns', expectedAge: 0 }}
                    category="concerns"
