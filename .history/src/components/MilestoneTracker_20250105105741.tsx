...
export const MilestoneTracker: React.FC<{ onChange?: (data: any) => void }> = ({ onChange }) => {
  ...
  const currentMilestones = globalState.assessments?.milestones?.milestones || initialMilestones;
  const currentHistory = globalState.assessments?.milestones?.history || '';

  // Update assessment with both milestones and history
  const updateMilestoneState = useCallback((newMilestones: Milestone[], newHistory: string = currentHistory) => {
  ...
  }, [updateAssessment, onChange, currentHistory]);

  // Handle history text changes
  const handleHistoryChange = useCallback((text: string) => {
    updateMilestoneState(currentMilestones, text);
  }, [currentMilestones, updateMilestoneState]);
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    const draggedMilestone = currentMilestones.find((m: Milestone) => m.id === active.id);
    if (!draggedMilestone) return;

    const status = determineStatus(month, draggedMilestone.expectedAge);
    
    const updatedMilestones = currentMilestones.map((m: Milestone) => 
      m.id === active.id ? {
        ...m,
        actualAge: month,
        stackPosition: getStackPosition(currentMilestones, month, active.id.toString()),
        status
      } : m
    );

    updateMilestoneState(updatedMilestones);
    setActiveMilestone(null);
    setCurrentMonth(null);
  }, [currentMilestones, updateMilestoneState]);

  const handleDragStart = useCallback((event: DragEndEvent) => {
    const draggedMilestone = currentMilestones.find((m: Milestone) => m.id === event.active.id);
  ...
  }, [currentMilestones, customConcernCount, updateMilestoneState]);
