...
const getStackPosition = (
  ...
  const placedMilestones = milestones
    ...
    .filter(m => (m.actualAge || 0) > (m.expectedAge || 0) === isDelayed);

  let row = isDelayed ? 0 : 4; // Reduced max rows due to increased spacing
  let hasOverlap = true;
  const maxRows = 5; // Reduced max rows
  const direction = isDelayed ? 1 : -1;
...
const ExpectationLine: React.FC<{
  ...
  category: CategoryType;
  stackPosition: number;
}> = ({ actualAge, expectedAge, maxAge, category, stackPosition }) => {
  ...
  const verticalOffset = stackPosition * 40; // Increased vertical spacing between bubbles
  ...
  return (
    <div 
      ...
      style={{
        position: 'absolute',
        left: `${Math.min(actualX, expectedX)}%`,
        top: `${verticalOffset + 20}px`, // Center align with bubble
        width: `${Math.abs(expectedX - actualX)}%`,
                      ...
                      >
                        ...
                        {milestone.expectedAge !== 0 && (
                          <ExpectationLine
                            ...
                            maxAge={maxAge}
                            category={milestone.category}
                            stackPosition={milestone.stackPosition || 0}
                          />
                        )}
                      </motion.div>
                    );
