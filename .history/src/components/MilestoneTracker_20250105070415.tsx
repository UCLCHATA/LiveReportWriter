...
const TimelineContainer = styled.div`
  ...
  .timeline-inner {
    width: ${MAX_MONTHS * 20}px;  // Adjust pixel per month ratio
    min-width: 100%;
    position: relative;
    height: 150px;  // Fixed height for timeline
  }
`;
...
export const MilestoneTracker: React.FC<{ onChange?: (data: any) => void }> = ({ onChange }) => {
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => {
    ...
    const monthPosition = Math.round((dropX / timelineRect.width) * MAX_MONTHS);
    
    // Ensure vertical positioning is always on the timeline
    const yPosition = timelineRect.top + (timelineRect.height / 2);

    // Update milestone position
    const milestoneData = JSON.parse(event.dataTransfer.getData('milestone'));
    const newMilestone: TimelineItem = {
      ...
      style: {
        left: `${(monthPosition / MAX_MONTHS) * 100}%`,
