...
export const MilestoneTracker: React.FC<{ data: any; onChange: (data: any) => void }> = ({ data, onChange }) => {
  ...
  const captureTimelineImage = async () => {
    if (timelineRef.current) {
      ...
      timelineRef.current.style.paddingBottom = '30px';
      ...
      const canvas = await html2canvas(timelineRef.current, {
        ...
        useCORS: true,
        onclone: (clonedDoc) => {
          // Ensure all absolutely positioned elements are visible in the clone
          const clonedTimeline = clonedDoc.querySelector(`.${styles.timeline}`);
          if (clonedTimeline) {
            // Make sure the timeline container has a proper height
            (clonedTimeline as HTMLElement).style.height = '180px';
            // Make all month markers visible
            const markers = clonedTimeline.querySelectorAll(`.${styles.monthMarker}`);
            markers.forEach((marker) => {
              ...
              // Make sure the month labels are visible
      ...
      timelineRef.current.style.paddingBottom = '';
