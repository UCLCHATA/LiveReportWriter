        ...
        <RadarChart cx="50%" cy="50%" outerRadius="80%" data={data}>
          ...
          <PolarAngleAxis 
            ...
            tick={(props) => {
              ...
              if (sensoryData?.domains?.[payload.value.toLowerCase()]) {
                color = '#be185d'; // pink/maroon for sensory
              } else if (socialData?.domains?.[payload.value.toLowerCase()]) {
                color = '#10b981'; // green for social
              } else if (behaviorData?.domains?.[payload.value.toLowerCase()]) {
                color = '#f59e0b'; // amber for behavior
              }
              return (
                <text x={x} y={y} fill={color} textAnchor="middle" fontSize={12}>
                  {payload.value}
                </text>
              );
            }}
