...
const DraggableMilestone: React.FC<{ 
  ...
  const getTooltipText = () => {
    if (milestone.actualAge === undefined) {
      return categoryInfo[category].tooltips[milestone.title];
    }
    const diff = milestone.actualAge - milestone.expectedAge;
    const diffText = diff !== 0 
      ? `\n${diff > 0 ? 'Delayed' : 'Advanced'} by ${Math.abs(diff)} months`
      : '';
    return `Placed at: ${milestone.actualAge}m${milestone.expectedAge ? ` (Expected: ${milestone.expectedAge}m)${diffText}` : ''}`;
  };
  ...
  return (
    ...
    >
      {isEditing ? (
        ...
        <>
          {differenceText && milestone.actualAge !== undefined && (
            <div 
              className={styles.differenceCircle}
