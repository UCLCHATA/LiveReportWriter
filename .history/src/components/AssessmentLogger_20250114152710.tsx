...
const AssessmentEntry: React.FC<{
  ...
  const handleToggleComplete = (e: React.MouseEvent) => {
    ...
    onUpdate(assessment.id, { status: newStatus });
  };
        ...
        <div className={styles.entryHeader}>
          ...
          <div className={styles.entryControls}>
            ...
            <button
              ...
              className={`${styles.completeButton} ${assessment.status === 'completed' ? styles.completed : ''}`}
              title={assessment.status === 'completed' ? 'Mark as incomplete' : 'Mark as complete'}
            >
              <ThumbsUp size={12} />
            </button>
            <button
...
export const AssessmentLogger: React.FC<ComponentProps> = ({ data, onChange }) => {
  ...
  const handleUpdateAssessment = useCallback((id: string, updates: Partial<Assessment>) => {
    ...
    const newState = {
      ...assessmentLog,
      selectedAssessments: selectedAssessments.map((assessment: Assessment) => 
        assessment.id === id 
          ? { ...assessment, ...updates }
          : assessment
      ),
