    ...
    switch (componentId) {
      ...
      case 'behaviorInterests':
        if (isBaseAssessment(currentAssessment)) {
          ...
          // Update domain values while preserving labels
          if (data.domains) {
            Object.entries(data.domains).forEach(([key, newDomain]) => {
              if (key in currentAssessment.domains) {
                baseData.domains[key] = {
                  ...newDomain,
                  label: currentAssessment.domains[key].label
                };
              }
            });
          }
          
          updateAssessment(componentId, baseData as typeof currentAssessment);
        }
        break;
