      ...
      try {
        ...
        const sheetyData = {
          r3Form: {
            ...
            name: globalState.clinician?.childFirstName,
            ...
            sensoryProfile: JSON.stringify(Object.entries(globalState.assessments.sensoryProfile.domains).map(([_, domain]) => ({
              domain: domain.name,
              value: domain.value,
              label: domain.label,
              observations: domain.observations
            }))),
            socialCommunication: JSON.stringify(Object.entries(globalState.assessments.socialCommunication.domains).map(([_, domain]) => ({
              domain: domain.name,
              value: domain.value,
              label: domain.label,
              observations: domain.observations
            }))),
            behaviorInterests: JSON.stringify(Object.entries(globalState.assessments.behaviorInterests.domains).map(([_, domain]) => ({
              domain: domain.name,
              value: domain.value,
              label: domain.label,
              observations: domain.observations
            }))),
            assessmentLog: JSON.stringify({
        ...
        console.log('Submitting form data to Sheety API:', {
          ...sheetyData,
          r3Form: {
            ...sheetyData.r3Form,
            radarChartImage: radarChartImage ? '[BASE64_IMAGE]' : '',
            timelineImage: formData.timelineImage ? '[BASE64_IMAGE]' : ''
          }
        });

        // 1. Submit to Sheety API
        ...
        const sheetyResponse = await response.json();
        console.log('Sheety API Response:', sheetyResponse);
        ...
        // 2. After successful Sheety submission, trigger Apps Script workflow
        ...
        const analysisResult = await makeAppsScriptCall(
          APPS_SCRIPT_URLS.analysis,
          globalState.clinician?.chataId
        );

        if (!analysisResult.success) {
          console.error('Analysis stage failed:', analysisResult.error);
          throw new Error(`Analysis stage failed: ${analysisResult.error || 'Unknown error'}`);
        }

        console.log('Analysis stage completed successfully');
        setSubmissionProgress(90);
        setSubmissionStage('report');

        const result = await makeAppsScriptCall(
          APPS_SCRIPT_URLS.report,
          globalState.clinician?.chataId
        );
        ...
        alert(`Submission failed: ${error instanceof Error ? error.message : 'Unknown error occurred'}`);
      }
    } else {
