...
const TimelineIndicators: React.FC<{ 
  milestones: Milestone[];
  activeMilestone: Milestone | null;
}> = ({ milestones, activeMilestone }) => {
  return (
    <>
      {/* Only show expected age for active milestone */}
      {activeMilestone && (
        <div 
          className={styles.expectedAgeIndicator}
          style={{ left: `${(activeMilestone.expectedAge / 72) * 100}%` }}
        >
      ...
      {/* Show actual age indicators for placed milestones */}
      {milestones
        .filter(m => m.actualAge !== null)
        .map(milestone => (
          <div 
            key={milestone.id}
            className={styles.actualAgeIndicator}
            style={{ 
              left: `${(milestone.actualAge || 0) / 72 * 100}%`,
