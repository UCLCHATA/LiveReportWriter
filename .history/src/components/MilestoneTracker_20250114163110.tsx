      ...
      if (includeInReport) {
        ...
        const formattedMilestones = milestones
          ...
          .map((milestone: Milestone) => {
            const status = formatMilestoneStatus(milestone);
            const description = milestone.category === 'concerns' 
              ? 'Custom milestone' 
              : `Expected at ${milestone.expectedAge} months`;

            return `${milestone.title}|${description}|${milestone.category === 'concerns' ? 'NA' : milestone.expectedAge}|${milestone.actualAge ?? 'Not Assessed'}|${status}`;
          })
          .join(' || ');
