...
const getStackPosition = (
  ...
  const milestonesAtMonth = milestones.filter(
    (milestone: Milestone) => milestone.actualAge === currentMonth && milestone.id !== currentId
  );
  return milestonesAtMonth.length;
};

const getTimelinePosition = (milestone: Milestone, maxAge: number): { left: string; top: string } => {
  if (!milestone.actualAge) return { left: '0%', top: '0px' };
  
  const horizontalPosition = `${(milestone.actualAge / maxAge) * 100}%`;
  const stackPosition = milestone.stackPosition || 0;
  const verticalPosition = `${stackPosition * 30}px`;
...
const formatMilestoneStatus = (milestone: Milestone): string => {
  if (milestone.actualAge === undefined) return 'Not Assessed';
  const status = determineStatus(milestone.actualAge, milestone.expectedAge);
  const difference = milestone.actualAge - milestone.expectedAge;
  
  if (difference > 0) return `Delayed by ${difference} months`;
  if (difference < 0) return `Advanced by ${Math.abs(difference)} months`;
  return 'On Track';
};
...
export const MilestoneTracker: React.FC<{ 
  ...
  const [includeInReport, setIncludeInReport] = useState(data?.includeTimelineInReport || false);
  ...
  // Update form data whenever milestones change
  useEffect(() => {
    if (!includeInReport) return;

    const formattedMilestones = milestones
      .filter(m => m.actualAge !== undefined && m.id !== 'custom-concern')
      .map((milestone: Milestone) => ({
        ...
        status: milestone.actualAge !== undefined && milestone.expectedAge !== 0
          ? determineStatus(milestone.actualAge, milestone.expectedAge)
          : 'typical',
        isCustom: milestone.id.startsWith('custom-concern-')
      }));

    console.log('Formatted milestones for form:', formattedMilestones);

    // Capture timeline image if needed
    const updateFormWithImage = async () => {
      let timelineImage = '';
      try {
        const capturedImage = await captureTimelineImage();
        timelineImage = capturedImage || '';
      } catch (error) {
        console.error('Failed to capture timeline image:', error);
      }

      // Update form data with milestone information
      onChange({
        type: 'milestoneTracker',
    ...
    updateFormWithImage();
  }, [includeInReport, milestones, historyText]);

  const handleIncludeInReportToggle = () => {
    const newIncludeInReport = !includeInReport;
    setIncludeInReport(newIncludeInReport);
    
    if (!newIncludeInReport) {
      // If removing from report, clear milestone data
      onChange({
        type: 'milestoneTracker',
            ...
            <div className={styles.placedMilestones}>
              <AnimatePresence>
                {milestones
                  ...
                  .map((milestone: Milestone) => {
                    const position = getTimelinePosition(milestone, maxAge);
                    return (
                      <motion.div
                        key={milestone.id}
