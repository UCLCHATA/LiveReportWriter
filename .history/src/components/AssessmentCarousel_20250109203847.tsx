...
interface Tool {
  id: 'sensoryProfile' | 'socialCommunication' | 'behaviorInterests' | 'milestones' | 'assessmentLog' | 'summary';
  title: string;
...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = ({
  ...
  // Add logging for summary component data
  useEffect(() => {
    if (currentTool.id === 'summary') {
      console.log('Rendering AssessmentSummary:', {
        currentIndex,
        hasData: !!globalState.assessments?.milestoneTracker?.milestones,
        milestoneCount: globalState.assessments?.milestoneTracker?.milestones?.length,
        milestoneData: globalState.assessments?.milestoneTracker
      });
    }
  }, [currentTool.id, currentIndex, globalState.assessments?.milestoneTracker]);
