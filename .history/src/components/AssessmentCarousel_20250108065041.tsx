    ...
    switch (componentId) {
      ...
      case 'behaviorInterests':
        if (isBaseAssessment(currentAssessment)) {
          ...
          // Safely update domain values
          if (data.domains) {
            try {
              Object.keys(data.domains).forEach(key => {
                const domain = data.domains[key];
                const existingDomain = updatedAssessment.domains[key];
                
                if (existingDomain && domain) {
                  if (typeof domain.value === 'number') {
                    existingDomain.value = domain.value;
                  }
                  if (Array.isArray(domain.observations)) {
                    existingDomain.observations = [...domain.observations];
                  }
                }
              });
            } catch (error) {
              console.error('Error updating domain values:', error);
            }
          }
          
          updateAssessment(componentId, updatedAssessment);
        }
        break;
