...
import type { GlobalFormState, AssessmentData } from '../types/index';
...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = ({ 
    ...
    switch (component.type) {
      ...
      case 'behaviorInterests':
        if (component.domains) {
          ...
          const sliderValues = domains.map((domain: any) => 
            typeof domain.value === 'number' ? domain.value : 0
          );
          const sliderProgress = (sliderValues.reduce((sum: number, value: number) => sum + value, 0) / (domainCount * 4)) * 70;
  ...
  useEffect(() => {
    if (!isInitialized) {
      setCompletionStates(prev => {
        ...
        tools.forEach(tool => {
          const assessment = globalState.assessments[tool.id];
          newStates[tool.id] = {
  ...
  }, [isInitialized, globalState.assessments]);
  ...
  useEffect(() => {
    ...
    // Update completion state based on saved data
    if (assessment) {
      ...
      setCompletionStates(prev => ({
        ...
        [tool.id]: {
          ...prev[tool.id],
          progress,
          isComplete: assessment.isComplete || isComplete,
          autoDetected: isComplete && !assessment.isComplete
        }
      }));
  ...
  const handleMarkComplete = (toolId: keyof AssessmentData) => {
    ...
    // Update assessment completion state
    updateAssessment(toolId, { isComplete: true });
  };
  ...
  const handleMarkIncomplete = (toolId: keyof AssessmentData) => {
    ...
    // Update assessment completion state
    updateAssessment(toolId, { isComplete: false });
  };
  ...
  const handleComponentUpdate = (data: any) => {
    ...
    if (componentId === 'milestones') {
      ...
      updateAssessment(componentId, data);
    }
  };
  ...
  useEffect(() => {
    ...
    const hasChanges = tools.some(tool => {
      const current = completionStates[tool.id];
      const next = newStates[tool.id];
      return !current || 
        current.progress !== next.progress ||
        current.isComplete !== next.isComplete ||
        current.isSkipped !== next.isSkipped ||
        current.autoDetected !== next.autoDetected;
    });

    if (hasChanges) {
      setCompletionStates(newStates);
    }
  }, [globalState.assessments, completionStates, calculateComponentProgress]);
