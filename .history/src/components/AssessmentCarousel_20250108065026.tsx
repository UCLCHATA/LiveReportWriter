...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = ({ 
  ...
  const [completionStates, setCompletionStates] = useState<Record<AssessmentToolId, CompletionState>>(() => {
    const initial: Record<AssessmentToolId, CompletionState> = {} as Record<AssessmentToolId, CompletionState>;
    tools.forEach(tool => {
      initial[tool.id] = {
        isComplete: false,
        isSkipped: false,
    ...
    switch (component.type) {
      ...
      case 'behaviorInterests':
        if (component.domains) {
          ...
          // Calculate slider progress (max 70%)
          const sliderProgress = domains.reduce((acc: number, domain: any) => {
            // Consider a slider "complete" if it has any non-zero value
            if (typeof domain.value === 'number' && domain.value !== 0) {
              return acc + (70 / domainCount);
            }
            return acc;
          }, 0);
          
          // Calculate observation progress (max 30%)
          const observationProgress = domains.reduce((acc: number, domain: any) => {
            if (domain.observations?.length > 0 && 
                domain.observations.some((obs: string) => obs.trim().length > 0)) {
              return acc + (30 / domainCount);
            }
            return acc;
          }, 0);
  ...
  const handleComponentUpdate = (data: any) => {
    ...
    const currentAssessment = globalState.assessments[componentId];
    
    switch (componentId) {
      case 'milestones':
        updateAssessment(componentId, {
          type: 'milestoneTracker' as const,
          milestones: data.milestones || [],
          history: data.history || '',
          progress: data.progress || currentAssessment.progress || 0
        });
        break;
      ...
      case 'assessmentLog':
        updateAssessment(componentId, {
          ...
          selectedAssessments: data.selectedAssessments || [],
      ...
      case 'socialCommunication':
      case 'behaviorInterests':
        if (isBaseAssessment(currentAssessment)) {
          // Create a new object with the same shape as the current assessment
          const baseData = {
            ...currentAssessment,
            progress: data.progress ?? currentAssessment.progress,
            isComplete: data.isComplete ?? currentAssessment.isComplete
          };
          
          // Only update values from domains, keeping existing labels
          if (data.domains) {
            for (const key in data.domains) {
              if (key in baseData.domains) {
                baseData.domains[key as keyof typeof baseData.domains].value = 
                  data.domains[key].value;
                if (data.domains[key].observations) {
                  baseData.domains[key as keyof typeof baseData.domains].observations = 
                    data.domains[key].observations;
                }
              }
            }
          }
          
          updateAssessment(componentId, baseData);
        }
        break;
  ...
  useEffect(() => {
    ...
    const componentProgresses = tools.map(tool => {
      ...
      if (state?.isComplete) return 20; // Award 20 points for completed components
      
      // Calculate weighted progress
      const { progress } = calculateComponentProgress(component);
      // Only count progress if it's above a minimal threshold (5%)
      return progress > 5 ? Math.min((progress / 100) * 20, 20) : 0;
    });
  ...
  }, [completionStates, globalState.assessments, calculateComponentProgress, isInitialized, onProgressUpdate]);
