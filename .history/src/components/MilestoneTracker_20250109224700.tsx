...
export const MilestoneTracker: React.FC<{ data: any; onChange: (data: any) => void }> = ({ data, onChange }) => {
  ...
  const isOverlapping = (milestone: Milestone & { actualAge: number }, otherMilestones: Milestone[]): boolean => {
    return otherMilestones.some(other => {
      ...
      // Check if the main bubbles overlap horizontally
      // Ignore the number rectangle (which is positioned at right: -8px)
      const distance = Math.abs(milestone.actualAge - other.actualAge);
      const isClose = distance <= 6; // Width of the main bubble body
