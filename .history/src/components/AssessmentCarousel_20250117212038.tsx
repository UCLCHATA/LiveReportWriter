...
export const AssessmentCarousel: React.FC<AssessmentCarouselProps> = React.memo(({
  ...
  const handleProgressUpdate = useCallback((progress: number) => {
    ...
    progressUpdateTimeoutRef.current = setTimeout(() => {
      if (progress !== initialProgress) {
        ...
        const updates = tools.reduce<Partial<AssessmentData>>((acc, tool) => {
          const currentAssessment = globalState.assessments[tool.id as keyof AssessmentData];
          if (currentAssessment) {
            acc[tool.id as keyof AssessmentData] = {
              ...currentAssessment,
              progress,
              lastUpdated: new Date().toISOString()
            };
  ...
  useEffect(() => {
    ...
    const component = globalState.assessments[currentTool.id as keyof AssessmentData];
    if (!component) return;
