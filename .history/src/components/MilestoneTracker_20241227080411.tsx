...
export const MilestoneTracker: React.FC = () => {
  ...
  const handleDragMove = (event: DragMoveEvent) => {
    ...
    if (event.over.id === 'timeline') {
      ...
      const relativePosition = mouseX / timelineWidth;
      const age = Math.max(0, Math.min(maxAge, maxAge * relativePosition));

      // Update UI to show expected age indicator
      const indicator = document.querySelector(`.${styles.expectedAgeIndicator}`);
      if (indicator) {
        indicator.style.left = `${(activeMilestone.expectedAge / maxAge) * 100}%`;
      }
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && over.id === 'timeline') {
      ...
      // Calculate drop position relative to timeline's left edge
      const mouseX = event.delta.x;
      const timelineWidth = timelineRect.width;
      const relativePosition = mouseX / timelineWidth;
      const age = Math.max(0, Math.min(maxAge, maxAge * relativePosition));
