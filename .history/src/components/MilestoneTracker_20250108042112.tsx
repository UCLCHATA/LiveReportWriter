...
type AllCategoryTypes = CategoryType | 'language' | 'cognitive';
...
// Update category mapping to handle both directions
const categoryMapping: { [K in AllCategoryTypes]: CategoryType } = {
...
const mapCategory = (category: AllCategoryTypes): CategoryType => 
  categoryMapping[category] || category as CategoryType;
        ...
        <div className={styles.categoriesContainer}>
          {(Object.entries(categoryInfo) as [CategoryType, CategoryInfo][]).map(([category, info]) => {
            const categoryMilestones = milestones
              .filter((m: Milestone) => {
                const mappedCategory = mapCategory(m.category);
                return mappedCategory === category && !m.actualAge;
              });
