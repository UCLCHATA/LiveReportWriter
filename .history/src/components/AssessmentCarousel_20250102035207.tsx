  ...
  const calculateComponentProgress = (component: any) => {
    ...
    switch (component.type) {
      case 'assessmentLog':
        ...
        // 1% for selecting assessments (if any are selected)
        if (selectedAssessments.length > 0) {
          progress += 1;
        }

        // Up to 9% for entries (2% for each entry with notes)
      ...
      case 'milestoneTracker':
        if (component.milestones) {
          // Count milestones that have been placed (have an actualAge)
          const placedMilestones = component.milestones.filter((m: any) => 
            m.actualAge !== null && m.actualAge !== undefined
          ).length;
          
          // Each placed milestone contributes 1% up to 10%
          progress = Math.min(placedMilestones, 10);
          
          // Log for debugging
          console.log('Milestone Progress:', {
            total: component.milestones.length,
            placed: placedMilestones,
            progress
          });
        }
        break;
    ...
    const calculateProgress = () => {
      ...
      Object.entries(components).forEach(([key, component]) => {
        if (component) {
          ...
          // Log individual component progress for debugging
