...
export const ClinicianModal: React.FC<ClinicianModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Clean up any unsubmitted forms for this clinician before creating a new one
    const existingForms = formPersistence.getAllUnsubmittedForms();
    existingForms.forEach(form => {
      if (form.clinicianInfo.email.toLowerCase() === clinicianInfo.email.toLowerCase()) {
        console.log('Cleaning up unsubmitted form:', form.chataId);
    ...
    console.log('Checking for existing form by email:', { 
    ...
    if (existingForm && 
      ...
      console.log('Found matching existing form:', { 
        chataId: existingForm.chataId,
        clinicianName: existingForm.clinicianInfo.name,
  ...
  const handleChataIdSubmit = () => {
    ...
    if (form.assessmentState) {
      ...
      if (storedAssessments) {
        try {
          ...
          // Only initialize with null values if we couldn't find any existing data
          formPersistence.updateAssessmentState(normalizedChataId, {
            sensoryProfile: null,
        ...
        console.log('No assessment data found in any storage:', { chataId: normalizedChataId });
        // Only initialize with null values if we couldn't find any existing data
        formPersistence.updateAssessmentState(normalizedChataId, {
          sensoryProfile: null,
          socialCommunication: null,
    ...
    const progress = localStorage.getItem(`chata_form_progress_${normalizedChataId}`);
    if (progress) {
      console.log('Found progress data:', { chataId: normalizedChataId });
      try {
        const parsedProgress = JSON.parse(progress);
        formPersistence.updateProgress(normalizedChataId, parsedProgress);
      } catch (e) {
        console.error('Error parsing progress data:', e);
      }
    } else {
      console.log('No progress data found:', { chataId: normalizedChataId });
    }
