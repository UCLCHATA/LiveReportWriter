...
export const SocialCommunicationGraph: React.FC<SocialCommunicationGraphProps> = ({ data }) => {
  ...
  const handlePreviewClick = async () => {
    try {
      const chartElement = document.querySelector('.social-profile-chart') as HTMLElement;
      if (chartElement) {
        ...
        if (newTab) {
          newTab.document.write(`<img src="${chartImage}" alt="Social Communication Chart" />`);
        }
      }
    } catch (error) {
    ...
    <div className={styles.graphContainer}>
      <ResponsiveContainer width="100%" height="100%">
        <RadarChart 
          ...
          outerRadius="80%" 
          data={chartData}
          className="social-profile-chart"
        >
