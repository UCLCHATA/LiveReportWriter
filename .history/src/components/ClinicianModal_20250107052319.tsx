...
export const ClinicianModal: React.FC<ClinicianModalProps> = ({
  ...
  const handleChataIdSubmit = async (chataId: string) => {
    ...
    try {
      ...
      const form = formPersistence.getForm(normalizedId);
      console.log('Form retrieval result:', form ? 'Found' : 'Not found', { chataId: normalizedId });
      ...
      if (!form.clinicianInfo) {
        console.log('Form found but missing clinician info:', { chataId: normalizedId });
      ...
      existingForms.forEach(existingForm => {
        if (existingForm.chataId !== normalizedId && 
      ...
      // Set current CHATA ID
      setCurrentChataId(normalizedId);

      // Update clinician info from the form
      ...
      console.log('Submitting restored form with CHATA ID:', normalizedId);
