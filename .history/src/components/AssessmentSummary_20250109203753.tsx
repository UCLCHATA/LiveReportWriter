    ...
    <div className={styles.timelineViewer}>
      ...
      <div className={styles.milestonesLayer}>
        {placedMilestones.map((milestone) => (
          <div
            key={milestone.id}
            className={`${styles.timelineMilestone} ${getStatusClass(milestone.status)}`}
            style={{
...
export const AssessmentSummary: React.FC<AssessmentSummaryProps> = ({ data, onChange, currentStep }) => {
  ...
  console.log('AssessmentSummary currentStep:', currentStep, 'isFinalStep:', isFinalStep);
  console.log('Has milestone data:', !!data?.milestoneTracker?.milestones);
