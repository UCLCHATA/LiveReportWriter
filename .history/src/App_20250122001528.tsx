...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    const totalProgress = componentProgress + formProgress;
    setProgress(totalProgress);
  }, [componentProgress, formProgress]);

  const handleComponentProgressUpdate = useCallback((newProgress: number) => {
    setComponentProgress(newProgress);
  }, []);

  const handleFormProgressUpdate = useCallback((newProgress: number) => {
  ...
  return (
    <div className="app">
      ...
      {isFormInputStage ? (
        <main>
          <AssessmentCarousel 
            onProgressUpdate={handleComponentProgressUpdate}
            initialProgress={componentProgress}
          />
          <AssessmentForm 
