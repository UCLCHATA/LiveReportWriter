class TextBox {
    ...
    addCharacterCount() {
        ...
        const charCountProgress = document.createElement('div');
        charCountProgress.className = 'char-count-progress';
        
        const charCountLabel = document.createElement('div');
        charCountLabel.className = 'char-count-label';
        
        charCountContainer.appendChild(charCountProgress);
        charCountContainer.appendChild(charCountLabel);
        
        this.textArea.parentNode.insertBefore(charCountContainer, this.textArea.nextSibling);
        
        this.charCountProgress = charCountProgress;
        this.charCountLabel = charCountLabel;
    }
    
    updateCharacterCount() {
        const charCount = this.textArea.value.length;
        const { minChars, optimalChars, maxChars } = this.options;
        
        // Calculate progress percentage
        let progress;
        if (charCount < minChars) {
            progress = (charCount / minChars) * 40;
        } else if (charCount < optimalChars) {
            progress = 40 + ((charCount - minChars) / (optimalChars - minChars)) * 40;
        } else {
            progress = 80 + ((charCount - optimalChars) / (maxChars - optimalChars)) * 20;
        }
        progress = Math.min(100, Math.max(0, progress));
        
        // Update progress bar
        this.charCountProgress.style.width = `${progress}%`;
        
        // Update label
        this.charCountLabel.textContent = `${charCount}/${maxChars}`;
        
        // Update state class
        this.charCountProgress.classList.remove('char-count-low', 'char-count-medium', 'char-count-good');
        if (charCount < minChars) {
            this.charCountProgress.classList.add('char-count-low');
        } else if (charCount < optimalChars) {
            this.charCountProgress.classList.add('char-count-medium');
        } else {
            this.charCountProgress.classList.add('char-count-good');
        }
    }
